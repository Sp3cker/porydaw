...
typedef struct {                       /* structured diagnostic, one block */
    ...
    size_t end_line, end_column;       /* 1-based, inclusive; 0 = no range */
...
legacy single-file loader and modular loader never link together under the
same target while they expose colliding symbols. Waves 0–7 keep the legacy
Porydaw target intact while the new module is built and exercised by its own
harness; the Wave 8 atomic cutover switches the target, include paths, and all
legacy callers together. No intermediate commit may contain duplicate runtime
types, duplicate loader symbols, or a half-migrated caller set.
...
| `sound/voicegroups.inc` alt spelling | `voicegroup_loader.c:2063-2103,2825-2828` | **gate**: add to Rust or declared-unsupported fixture |
| `sound/keysplit_tables.s` | `voicegroup_loader.c:548-550,616-619` | **gate** (same) |
| `_keysplit` / `_drumset` suffix | `voicegroup_loader.c:1889-1982` | **gate** (same) |
| `vg_` eventide prefix | `voicegroup_loader.c:1985-1999` | **gate** (same) |
| deep scan of `sound/` depth 3 | `voicegroup_loader.c:593-684,843-854` | **gate** (same) |
| `.s` voicegroup files | legacy `find_voicegroup_probe` | **gate** (same) |
| config overrides | `voicegroup_loader.c:746-784` | **gate** (same) |
| synth data file / inline synth | `voicegroup_loader.c:786-796,963-969` | added (Wave 1, §7) |
| **ROM-contiguity overflow** | `contiguousFill`+`noSubRecurse`, `voicegroup_loader.c:2145-2171,2221-2239,2542,2560` | **preserve by default** (§13.3) |
...
Decide at the pre-freeze gate between unbounded (legacy semantics) and the fixed
128 cap with a documented, tested hard bound; the choice is recorded in the
frozen-commit manifest.
...
- Configure-time checks compare the selected manifest's `header_sha256` and
  ...
  error.
- The Wave 8 atomic cutover switches the engine target from the legacy single
  file (`plugin/porydaw/CMakeLists.txt:17-20` builds
  `../voicegroup_loader.c`) to the modular `voicegroup` library and links
  `voicegroup_core_static`; Waves 0–7 do not link both loader variants.
...
freeze → vendor → CI chain (Waves 5–6) is **not parallelizable**; Wave 8 is
the atomic Porydaw target/caller cutover. Each wave lists dependency, target
modules, available subagent type, acceptance criteria, and verification
command. Use `scout` for read-only discovery, `task` for implementation,
`sonic` only for mechanical edits, and `reviewer` for gates. Formatters,
...
legacy Porydaw engine source list and all legacy callers intact until the Wave
8 atomic cutover; only the modular harness consumes the imported library in
this wave. No submodule file is touched here.
...
### Wave 8 — MainWindow adopts the project handle + picker adapter cutover (Porydaw) — `task`

**Dependency:** Wave 7.

**Targets.** `src/mainwindow.{h,cpp}`.
...
**Targets.** `src/mainwindow.cpp` (`maybeRefreshVoicegroup`, `refreshSessionsAfterVgSave`,
`loadVoicegroupFor`, `saveSession`, `reloadVoicegroupPreview`,
`synthDescForSymbol`, `applyPendingSynthTones`), `src/ui/voicegroupbrowser.{h,cpp}`
(new **Reload Voicegroup** action).

**Change.** Route `loadVoicegroupFor` through the module (`voicegroup_project_load`
→ `take`; on failure keep the current group); preserve dirty-sibling skip and
transactional `swapVoicegroup`; replace `reloadVoicegroupPreview`'s
`.porydaw/vgpreview` temp-file shadow (`:2558-2587`) with `VG_LOAD_SOURCE`;
remove `cleanupVgPreview` (`:2674-2679`). Add the **Reload Voicegroup** browser
action that calls `voicegroup_project_refresh` to re-arm a `RefreshFailed`/stale
module, rebuilds transactionally, and swaps only on a successful materialization
(§11). Build the transient synth overlay from `m_pendingSynths` for
preview/undo/redo loads; implement the save-time graduation (write definitions →
`mark_stale()` → refresh → clear pending only after success, §7).

**Acceptance.** `deno task verify --filter tabcheck` covers external-edit
no-hot-swap, clean-sibling reload, dirty-sibling retention, broken-reload keeps
sound; no `.porydaw/vgpreview` written during preview; an unsaved synth-param
edit minting a new symbol resolves through the overlay for each of the six
Golden Sun spellings (§7) in preview and clears from `m_pendingSynths` only
after a successful save graduation; **Reload Voicegroup** exits `RefreshFailed`
after a successful rebuild and keeps the last-good bank on failure.

**Verification.** `deno task verify --filter tabcheck`.
...
**Change.** Rework the four preserved scenarios (project switching, stale source
changes, nested discovery, invalidation) against the new seam; keep
`tools/porydaw_loadbench_cli.cpp` as the **old** reference; add the **new**
benchmark CLI targeting the modular loader and driving the four §17.5
new-surface analogs (cold open+first refresh, warm no-change refresh,
`VG_LOAD_SOURCE` preview materialization, one-symbol `load_asset`); add a
...
fingerprint differs from the Wave 0 recording, re-run the legacy side on the new
fixture before comparing.**
...
| Module ownership + picker + save/reload routing | `src/mainwindow.{h,cpp}` |
...
| Loader header includes | `src/project/voicegroupsource.h`, `src/audio/audioengine.h`, `src/audio/wavexport.h` |
| Diagnostics UI + picker | `src/ui/voicegroupbrowser.{h,cpp}`, `src/ui/samplepicker.*` |
| Harnesses | `src/checks/{contextcheck.cpp,vgsavecheck.cpp,tabcheck.cpp,vgcheck.cpp,projectindexcheck.cpp,checkcatalog.cpp,checkregistry.cpp}` |
...
|---|---|---|---|---|---|
| cold saved bank | 0.31 / 0.37 / 0.74 / 2.31 | cache disabled, env unset | open + first refresh + saved `load` | new handle, no generation | ≤ 1.11 ms mean |
| warm saved bank | 0.29 / 0.30 / 0.30 / 0.32 | cache enabled after warmup | saved `load` after warmup | steady-state Fresh | ≤ 0.45 ms mean |
| preview materialization | 0.35 / 0.39 / 0.41 / 0.52 | cache enabled after warmup | `VG_LOAD_SOURCE` load | steady-state Fresh | ≤ 0.615 ms mean |
| first picker row | 0.12 / 0.13 / 0.14 / 0.16 | cache enabled after warmup | one-symbol `load_asset` after refresh | Fresh, empty asset cache | ≤ 0.21 ms mean |
