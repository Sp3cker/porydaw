...
| R29 | pending | 2026-09-25 ED06/ED07/ED08 presenter-contract divergence: authored portable `*_gap.swift` assertions for the velocity, automation and voice-change presenter surfaces were brought into this tree, compiled and wired into `runProjectSessionSuite`; they produced 241 failing assertions — concentrated in `VelocityPageTest::portableGapPredicates` (104), `voicePortableGapPredicates` (101) and one `publishedLayoutAndZoom` gutter mismatch (expected 584, actual 640). The page APIs exist (`publishedMarkers`, `pickerOpen`, `hoverTick`, `setPickerFilter` on VoiceChangesPage/AutomationPage); the failures are a publication-contract divergence from the original C++ behavior, not a missing surface. The attempt was reverted the same day pending presenter-parity repair: fix the velocity/automation/voice-change presenter publication contract, then land the portable assertions so ED06/ED07/ED08 can go green. |
...
| ED06 | pending | Velocity scope/grouping/PSG detents/prompts/history. See R29 (2026-09-25): presenter-contract divergence must be repaired before its portable assertions can land. |
| ED07 | pending | Automation/strokes/clipboard/delete/tap tempo/opaque events. See R29 (2026-09-25): presenter-contract divergence must be repaired before its portable assertions can land. |
| ED08 | pending | Voice-change picker/preview/commit/cancel/remaps/persistence. See R29 (2026-09-25): presenter-contract divergence must be repaired before its portable assertions can land. |
