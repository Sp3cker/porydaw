...
- `src/swift/app/module.modulemap.in` — add module `PorydawSampleProcessing` with header `@CMAKE_SOURCE_DIR@/src/audio/swift_sample_processing.h`; `src/checks/workspace/SessionChecks.swift` and `src/checks/CMakeLists.txt` — register `runSampleProcessingChecks` for 29 new predicates and compile the retained fixture/check sources. The already-extended `src/checks/audio/AudioAuditionChecks.swift` and test-only `src/checks/support/corecheck/native_check.{h,cpp}` are read-only baselines here; do not add a duplicate lifecycle registration.
...
- `auditionSlotLifecycle` already runs under `samplecheck/SampleProcessingTest::auditionSlotLifecycle` from `src/checks/audio/AudioAuditionChecks.swift`. Its 32768 Hz cold engine reinitialization, pool reset, and post-reset playback now cover `proof.analysis.txt` A022–A035 with a green SwiftCore run. Preserve that exact check and the test-only `pdc_playback_engine_reinitialize` ownership; do not duplicate the `cppID` or registration.
