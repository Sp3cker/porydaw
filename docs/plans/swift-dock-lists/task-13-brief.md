...
- After the task-owned check sources are frozen, replace each header's `Swift counterpart: none identified`/`Swift SHA-256: n/a` with the exact supporting Swift, SwiftRollGated, or visual-check source and its SHA-256. When one ledger spans multiple sources, record the primary source and add every other source as an `Additional Swift counterpart` with its own `Swift SHA-256`, following the existing `proof.xcmd.txt` convention. Preserve `Original SHA-256` and all reference-revision fields unchanged.
