...
| 11 | Document track-suite file: duplicationOwnership, trackRemap publication + globalMetadata reclassification (A145-A158) | task-11-brief.md | new `DocumentTrackChecks.swift`, `EventChecks.swift` (2 calls), `src/checks/CMakeLists.txt` (1 line), `proof.tst_songdocument_document.txt` (A145-A158 only) | 10 (CMake chain) + C2 (EventChecks reuse) | `--qt eventEdits` |
