...
| `documentRemapsAndRaw` | A073-A100 | Direct `DocumentChange.trackRemap` payloads A074-A088 and nil-remap raw edit A089-A090; A098-A100 were BEHAVIOR-GAP, now closed by `trackRemapPublicationContract` S130-S132 (rejected same-slot move, unchanged revision, zero publication under the original `cppID`; the old `trackMoveContract` assertion used a different `cppID`). RETIRED-REPRESENTATION for staging and A091-A097 dynamic conductor promotion on raw events — Swift `insertRawEvent` never remaps; explicit track ops only. |
