...
- Voice restoration, shared hover lifecycle and canonical command availability: dedicated reviews completed. Pointer audition, filter-triggered note-off, picker navigation, captured menu targets, outside-release hint recovery and menu/window Paste availability pass their covering checks.
- Sibling-worktree behavior and visual oracles were reused. Four capture profiles (DPR 1/2 × font 12/16) pass. These are geometry/style and interaction checks, not an unsupported claim of pixel-diff parity for historical prompts or menus.
- The narrow hover addition to `VoiceChangesPage.swift` (1229 lines) has an explicit accepted cohesion exception: it publishes the existing interaction owner's profile without adding another responsibility. The checks review retained existing behavior-suite exceptions: new cases reuse the process-owned production fixture and existing behavior groups, without duplicate setup or arbitrary file splitting.

## Final verification and unresolved acceptance

- `deno task verify --filter swiftcore --filter swiftqtml --verbose`: both selected harnesses pass.
- `deno task build:app`: passes. Existing Qt importer and unused-variable warnings remain visible.
- `deno task verify:qml --filter editorqml-drawer --verbose`: **35 passed, 2 failed, 11 skipped**. Only `test_numericFieldWindowShortcutPriority` fails, once for velocity and once for automation. Neither row is suppressed or marked expected-failure.
- Separate focused runs pass the container resize/toggle/preferences/focus/playhead cases, automation and voice menu cases, both hover lifecycle regressions, and all four capture profiles. Those runs do not turn the full-suite result green.
- The full QML review completed the Python scanner, system `qmllint`, and six independent analysis domains. The final report records 224 mechanical observations, 60 static missing-property warnings, one confirmed shortcut blocker, and two investigation targets. `qmllint` exits 0 with no remaining unqualified-lookup warnings; the remaining warnings are not hidden.
- Native PID-window captures confirm right-aligned velocity graduations, hover instructions after Quick focus, and readable point/lane menus with the range arrow. Startup hover before the first Quick-host click remains an activation-scope investigation. Final native keyboard injection was inconclusive: Secure Input was enabled and injected Escape did not dismiss the menu. This observation is not attributed to application code without further evidence.
- Passing voice cases still emit `DelegateModel::cancel` warnings. The delegate review found no proven invalid product access; the precise Qt incubation/reset origin remains uninstrumented. No guard or fallback was introduced.
- Temporary QML diagnostics and the private native input probe/fixture were removed. Captures and the complete review report were retained as session evidence. Every completed todo checkpoint was committed and pushed.

**Overall acceptance remains blocked**, not complete. The missing prerequisite is an approved scoped Qt host/event-filter capability usable from Swift, so numeric fields can yield Space to the existing window action before `QQuickTextInput` consumes it. The no-new-C++, no-forwarding and no-global-hook rulings remain intact. Preexisting project/window C++ owners listed above remain outstanding; this restoration does not certify a C++-free application.
