# Task 30: Remove obsolete SongView lane-visibility facades
...
Completed/reviewed tasks: 7, 12, 23, 26, 28, 29.

## Interface contract

Delete only SongView::addEmptyLane/removeEmptyLane declarations and definitions after references have migrated. AutomationPage owns independent implementations, not SongView forwarding; its header and definitions retire together in task 35. Preserve setLaneRange and retained EditorViewState synchronization.

## Implementation steps

### Step 1

Refresh LSP references for the SongView add/remove empty-lane methods. Remove their SongView declarations and definitions together. Leave AutomationPage declarations and definitions intact for the bounded task 35 cutover. All UI discovery now uses the intrinsic catalog.

### Step 2

Keep range changes, outer drawer state, shared selection, track remap and document refresh publication unchanged. Do not replace removed functions with no-ops or move them to another facade.

### Step 3

Use the reference map to ensure that any remaining test or production caller is assigned and migrated before declaration deletion. An unexpected semantic caller outside these files requires a small additional controller brief, not silent behavior loss.

## Acceptance predicate

SongView no longer exposes or implements empty-row visibility mutations, while retained range/state publication and shared command routes remain intact.

## Controller verification

Controller: `deno task build:app`; complete settings and page deletion tasks before building the final checks target.
