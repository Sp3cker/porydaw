...
5. Existing gutter only. One full-width column of full labels — revised by the user from the original at-most-three-column design; spec.md is authoritative. Smaller typography permitted. Derive automation's minimum from the grid without changing shared minimums, Voice Changes/Velocity rules or the canonical split.
...
11. Prefer built-in Qt behavior over custom controls and their checks. Adding Qt modules is approved where it removes implementation/test burden. Task 5 uses native TabButton checking/input, GridLayout, Text.HorizontalFit, Binding and ContextMenu; the FontMetrics minimum-cell-width mandate was dropped with the three-column grid when the selector became one full-width column. Local code supplies only styling, the single-column policy, domain/menu wiring and a narrow Enter/Return extension.
