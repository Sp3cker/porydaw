# Task 31: Remove automation scrollbar chrome end to end
...
Completed/reviewed tasks: 4, 6, 17, 19, 21, 22, 27, 30.

## Interface contract

Delete only automationScrollbarRect/Visible, automationScrollY/ContentHeight/ViewportHeight/MaximumScrollY, setAutomationScrollY/scrollAutomationByWheel and their signal/forwarding chain. Shared scrollbar colors/metrics used by TimelineCanvas remain.

## Implementation steps

### Step 1

Remove the drawerAutomationScrollBar component from DrawerChromeLayer.qml and its drag connections; preserve inline value prompts, handles, section toggles and detent.

### Step 2

Remove the automationScrollbarRect snapshot field and its Q_PROPERTY/accessors, automation scroll properties/invokable definitions, constructor page-scroll signal connection and scrollChanged signal. Refresh LSP references first; task 4 already removed its TimelineQuickView connection.

### Step 3

Keep scrollbarHandle/scrollbarHandleHover and any scrollbar metric still referenced by the actual piano-roll vertical or timeline horizontal scrollbar. Do not remove the unrelated TimelineQuickView::verticalScroll properties just because their names match.

## Acceptance predicate

No automation scrollbar QML item, geometry field, scroll property, mutator or signal remains, while remaining drawer controls and piano-roll/timeline scrollbar chrome still work.

## Controller verification

Controller: `deno task verify --filter scrollbar --verbose` and `deno task verify --filter editor-drawer --verbose` at the final barrier.
