# Task 33: Reduce TempoLane to its unchanged musical adapter
...
Completed/reviewed tasks: 3, 13, 16, 29, 32.

## Interface contract

Retain the NodeLane adapter methods and existing document/AutomationPage construction needed for document/history access. Remove updateLayout/totalHeight/bodyRect/headerRect/expanded/containsHeader/toggleExpanded/cancel only when no callers remain.

## Implementation steps

### Step 1

Refresh LSP references for each TempoLane layout symbol. Delete layout functions and m_header/m_body/m_expanded fields, collapsedHeight/bodyHeight helpers and their sole-use Qt geometry includes.

### Step 2

Remove the obsolete canvas call to TempoLane::cancel if it only reset expansion/header interaction state. Do not change TempoLane::points/leadIn/minimumValue/maximumValue/replaceSpan/valuePrompt or the separate tempo adapter implementation.

### Step 3

Replace the stale pinned-Tempo class comment with the actual song-global NodeLane adapter role. Preserve lack of controller/track identity and microseconds-to-BPM round-trip behavior.

## Acceptance predicate

TempoLane has no collapse/header/body layout state, while its BPM conversion, default lead-in, points, transaction and value-prompt semantics remain exactly unchanged.

## Controller verification

Controller: `deno task verify --filter automation-domain --verbose`, plus Tempo rows in editing/hover/raster.
