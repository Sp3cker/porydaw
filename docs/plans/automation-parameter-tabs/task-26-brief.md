# Task 26: Complete stroke activation and editing fixture cutover
...
Completed/reviewed tasks: 8, 9, 10, 12, 18, 28. Execute after task 28; the original task number is retained for stable references.
...
Migrate the previously unassigned AutomationPage::addEmptyLane caller in automationstroke.cpp: remove cosmetic empty-row seeding, retain all existing document point/event setup, and activate Pan through task 8's real fixture helper before exercising the plot. Retain original sweep/ramp tick/value/snapping/transaction/undo expectations. Do not create or replace MIDI points merely to make a supported tab exist.
...
The complete editing suite uses real supported-parameter activation, including stroke/sweep/ramp, with no caller left on the retired empty-row or fixture expansion/height APIs and unchanged musical outcomes.
