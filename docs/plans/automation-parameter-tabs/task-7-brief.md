# Task 7: Retire lane visibility actions without changing event deletion
...
Keep existing CanvasMenuAction numeric values until the declaration/caller retirement task. Remove Add/Show/Hide menu construction and dispatch. The former RemoveLane action remains the guarded event-deletion operation, presented as Delete automation events; it no longer promises to remove the parameter label.

## Implementation steps

### Step 1

Delete showAddLaneMenu and AddLaneBase/ShowLaneBase/HideLane dispatch paths after task 4 removes input callers. Remove corresponding rows from showLaneMenuFor. Keep the existing Copy/Paste/Clear/ValueRange and node-menu behavior. Do not alter the underlying stored range behavior or introduce new axis-scaling functionality as part of this change.

### Step 2

Clear continues to call `lane->replaceSpan(0, maxTick, {})` but must not call addEmptyLane. Guarded Delete events continues through openCcDeletePrompt and the existing accept-time document/revision/handle/row-id validation; after replacement, do not call removeEmptyLane or hideLane. An empty parameter needs no visibility operation. Do not substitute SongDocument::deleteLanePoints for replaceSpan: their transactional/XCMD semantics are outside this presentation change.

### Step 3

Update delete confirmation text to describe event deletion and the still-present parameter. Preserve stored-event counting (not synthetic projected nodes), confirmation cancellation, foreign-session ownership and exact undo expectations. In ccdeleteconfirmation.cpp change only assertions that the row disappears: assert the label remains selectable and its document-written points are absent after accept and restored after undo. Keep cancellation/stale-target assertions unchanged.

## Acceptance predicate

Parameter menus no longer add/hide/remove labels, while Copy/Paste/Clear/value prompts and guarded Delete-events transactions retain their existing document and undo semantics.

## Controller verification

Controller: `deno task build:app`; retain the ccDeletePrompt semantic scenarios for `deno task verify --filter automation-editing --verbose` after the common fixture migration. No event deletion is exercised on a user song.
