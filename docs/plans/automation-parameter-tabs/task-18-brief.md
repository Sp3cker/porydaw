# Task 18: Retire stacked-layout editing cases while retaining input routing
...
Completed/reviewed tasks: 8, 13, 17.

## Interface contract

Retain automationBandAndInputsExposed/layoutAlignsPlotGutterAndRollGrid where their real band alignment contract survives. Remove only lane-stack/resize/collapse assertions.

## Implementation steps

### Step 1

Remove slot declarations and bodies for rowStackAndGridResolution, rowResizeChangesOnlyTarget, laneHeights and tempoHeaderExpansionIsolated where they exclusively assert removed behavior. Replace the useful geometry portion with active plot equals automation viewport and labels remain gutter-local.

### Step 2

Replace scrollbarChromeTracksZeroRangeResize with the externally observable absence of an automation scrollbar, responsive labels and unchanged timeline split. Do not preserve zero-return scroll shims solely for this test.

### Step 3

Keep middle-pan horizontal behavior, wheel timeline zoom, Voice Changes/Velocity gesture isolation and all original document snapshots. Replace row-specific view-state mutations with outer automation section sizing; retain existing grid-resolution expected musical ticks.

## Acceptance predicate

Editing layout/routing coverage tests the full-height active plot and preserved cross-band isolation, not obsolete individual row sizes or Tempo collapse.
