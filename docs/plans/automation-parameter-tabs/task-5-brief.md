...
```
ContextMenu requires Qt 6.9; uniform grid cells require 6.6. All CI installs Qt 6.11.x. Windows CI pins an unreleased `aqtinstall` fix because Qt 6.11 changed the Windows package repository layout and released `aqtinstall` cannot parse it. Qt 6.11 is the explicit cross-platform project floor. Keep Qt Test checks-only. Verify the deployed QML imports include Controls Basic/Templates and QtQuick.Layouts; a successful C++ link alone is insufficient. Do not add another test framework.
