# Task 38: Remove unused lane-stack geometry constants
...
Completed/reviewed tasks: 18, 28, 32, 33, 34, 35, 37.

## Interface contract

Delete only fields/resolver expressions whose LSP references are zero after cutover. Keep every point hit radius, drag threshold, curve/node/hover stroke, value-axis inset and phantom geometry formula unchanged.

## Implementation steps

### Step 1

Refresh field references for row min/max/default/add-strip/header/resize geometry and remove only the now-dead presentation metrics and initializer entries. Do not infer that a generically named border/header field is unused without references.

### Step 2

Retain AutomationProjection valueY/pointer mapping, origin display math and all NodeLane interaction geometry bit-for-bit. If a surviving field is consumed by a node or Voice Changes path, leave it intact and record why.

### Step 3

Confirm the retired UI API/reference inventory is exhausted; EditorViewState storage/codec/remap references are intentionally retained, not missed deletions. Check across src/ui and affected checks; consumers of retained piano-roll vertical scrolling are explicitly excluded.

## Acceptance predicate

AutomationGeometry contains only geometry still used by the tabbed plot or existing node algorithms, with no dead per-row resize/header/add-strip metrics.

## Controller verification

Controller: final full verify and native smoke matrix; this is the final production interface cutover, not permission to broaden unrelated cleanup.
