# Task 10: Preserve shared multi-lane edits behind the single visible plot
...
Completed/reviewed tasks: 8, 9.

## Interface contract

Consumes activateParameter(row), existing mixedSelection/mixedSelectionSmf and selectionMatches helpers. Keeps real SongView shared key dispatch and existing selected-node batching.

## Implementation steps

### Step 1

In multiLaneSelectionDragPreservesTempoAndCcOrder, after setTimeSelection(mixedSelection(true)), activate Pan and then Tempo through real labels before starting the existing Tempo drag:
```cpp
...
QVERIFY(selectionMatches(tab().view().selectionModel().timeSelection(),
    kSelectionTick, kDestinationTick, true,
    {{kTrack, kPanController}, {kTrack, kLfoController}}));
```
Retain the exact existing same-tick Pan order `{10,20}`, LFO value `{96}`, Tempo microseconds, untouched Volume and undo/redo assertions. Unpainted selected lanes must still move.

### Step 2

For tests that used a vertical multi-row band drag only to seed a later group operation, seed the same explicit TimeSelection through the model and retain the real subsequent input and document assertions. In bandSelectionIsolatesTempoAndControlChangeRows itself, perform separate horizontal bands after activating Tempo and Pan; assert that each replaces scope with that parameter only. Do not simulate dragging through imaginary stacked rows.

### Step 3

Keep multiLaneSelectionDeleteAndEmptyDeleteNoop, group-drag rebuild cancellation, tracksSelectionGroupDragUndo, clipboard/range parity and untouched-lane sentinels. Selection-scope tab indicators should include covered lanes independent of the active tab. Do not change shared Copy/Paste/SelectAll targeting to make these tests easier.

## Acceptance predicate

Switching displayed parameters preserves explicit multi-lane command/drag scope, including Tempo and unpainted CC lanes, while a newly drawn band selects only the active parameter.

## Controller verification

Controller: after writers settle, run `deno task verify --filter automation-editing --verbose` once the editing-suite migration group is complete. Compilation is required at each settled task; run the suite at its completed surface checkpoint. Obsolete scenarios are migrated, not suppressed or declared passing. Also run the four selectionkey suites after their fixture migrations.
