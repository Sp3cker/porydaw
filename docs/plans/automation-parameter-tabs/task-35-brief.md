# Task 35: Remove automation page scroll and per-row sizing state
...
Completed/reviewed tasks: 30, 31, 32, 33, 34.

## Interface contract

Final viewport method:
```cpp
...
```
Retain shared horizontal projection/snap/zoom, pencil shortcut guard and setLaneRange. Task 4 already installed this viewport method; do not rewrite it again.

## Implementation steps

### Step 1

Remove automationContentHeight/verticalScroll/setVerticalScroll/scrollVertically/scrollStateChanged, m_contentHeight/m_scrollY/m_verticalWheelRemainder, laneHeightFor/scaleSharedHeight and add/remove empty declarations and definitions together. Delete their implementations and now-dead row-layout geometry fields.

### Step 2

Keep the synchronizeAutomationViewport implementation installed in task 4. Preserve necessary defaultPixelsPerBeat fallback if AutomationProjection still uses it; do not remove a musical zoom default because it was stored beside old row metrics.

### Step 3

Remove vertical offsets from AutomationProjection construction if present, leaving horizontal timeline math, snapped raw tick/value mapping and device-pixel alignment verbatim. Update page comments to one plot; no zero-return obsolete API remains.

## Acceptance predicate

AutomationPage exposes only the real plot viewport and shared timeline mapping, with no vertical scroll/content-height/empty-row/per-row-height state or facade.

## Controller verification

Controller: build app/checks after this contract deletion and run the full automation/selectionkey/host coverage.
