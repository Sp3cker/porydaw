# Task 27: Preserve label-local activation and shared keyboard fallback
...
Completed/reviewed tasks: 22.

## Interface contract

No production shared-key-routing edits. Reuse the real WindowTierTest fixture and active Quick window. Task 5 supplies native Controls Basic TabButton interaction plus only a Keys.AfterItem Enter/Return extension; test its integration with Porydaw, not Qt's internal button state machine.

## Implementation steps

### Step 1

Replace automation-scrollbar focus setup in the keyboard cases with a rendered parameter label. Keep remaining timeline/roll scrollbar keyboard cases where those are the intended subject. Delete automation scrollbar-only expectations instead of keeping hidden dummy items.
...
Continue that scenario with the existing shared Copy/Delete/SelectAll/Paste routes from the focused label, preserving original targets and popup/text precedence. Keep existing arrow-command outcomes; the selector must not add arrow navigation. Tab traversal can be smoke-tested during the native UI gate rather than receiving a new Qt-mechanics unit test. Keep declarations consistent with deleted scrollbar-only cases.

## Acceptance predicate

Focused parameter labels consume only their advertised activation keys while shared selection commands and text/popup routing retain the existing policy.

## Controller verification

Controller: `deno task verify --filter selectionkey --verbose` covers all four registered suites.
