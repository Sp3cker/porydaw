# Task 4: Route parameter input and expose the canvas to Quick
...
Completed/reviewed tasks: 3.

## Interface contract

Consumes task 2 canvas interface. Produces QML context object `automationCanvas`, available before TimelineCanvas.qml is loaded. Shared key routing and all NodeLane gesture implementations remain unchanged.

## Implementation steps

### Step 1

Expose the real canvas beside the existing context objects, before setSource:
```cpp
m_quickView->rootContext()->setContextProperty(QStringLiteral("automationCanvas"),
                                              automation.canvas());
```
Remove the constructor connection from DrawerChrome::scrollChanged to automation refresh once geometry/selection/document paths supply refresh; do not remove unrelated scrollbar discovery or key policy.

### Step 2

Remove automation gutter selection clearing, Tempo collapse/header double-click, Add-strip hit routing and row-resize branches from pointerPress/Move/Release/DoubleClick. Gutter events not handled by the forthcoming label controls return false; they must not clear the shared selection. Plot paths keep existing clearTimeSelectionIfOutsidePress semantics but resolve the active logical handle. Preserve right-drag band selection; both band endpoint handles now refer to the active row. Remove only the obsolete vertical-pan reads/writes; horizontal middle-pan math stays intact.

### Step 3

Remove gutter Ctrl-wheel row-height scaling and vertical automation scrolling; decline those gutter wheel events. Preserve the existing plot-wheel branches verbatim:
```cpp
...
Migrate AutomationPage::synchronizeAutomationViewport now, before removing any exported API: compare the stored QSize, update it on change and call the existing canvas->viewportResized(). Stop deriving a stacked content height or calling canvas->scrollStateChanged() from this live allocation path. In the still-existing setVerticalScroll implementation, use the surviving viewportResized notification instead of the soon-retired callback; retain its state update and signal until the unused scroll API is removed in task 35. This leaves no page reference to the callback deleted by task 32 and does not add a temporary wrapper/no-op. Task 2 already implements openParameterMenu; do not re-author it here.

## Acceptance predicate

Plot input targets the active row without changing gesture semantics, and label-area input cannot invoke old gutter selection clearing, row resizing or Tempo collapse.
