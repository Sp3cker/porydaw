# Task 34: Remove CC stack metrics and default-visible presentation constants
...
Completed/reviewed tasks: 1, 3, 7, 18, 30, 32, 33.

## Interface contract

Keep supportedControllers, laneLabel, row/adapter lookup, range helpers and classification. Remove CCLanes minimumHeight/stack-height methods and only the now-unused default-visible-controller presentation constants.

## Implementation steps

### Step 1

Delete CCLanes::minimumHeight and any row-height/layout helper with no remaining semantic caller. Keep row ids stable and rebuild guards intact.

### Step 2

Refresh LSP references to CoreTimeDefaults::kDefaultVisibleControllers/isDefaultVisibleController. Once only obsolete presentation code remains, remove these two symbols and their presentation comment. Do not modify kControllerDefaults, controller numbers, pitch-bend/default tempo values, supported-event classification or audio behavior.

### Step 3

Delete stale hide/add/stack comments and obsolete includes in these files only. Confirm active tab choice is not derived from which adapters contain stored points.

## Acceptance predicate

CCLanes owns the fixed supported adapter catalog without per-row stack height, while all controller values/defaults/classification and adapter operations remain unchanged.

## Controller verification

Controller: final app/checks build and automation-domain.
