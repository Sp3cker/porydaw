# Task 23: Preserve host seams and cancellation transactions
...
Completed/reviewed tasks: 6, 20, 22.

## Interface contract

Retains existing real host fixtures and all lifecycle data rows, including page-switch, drawer-hide, track/document/song/voice replacement, quick-ungrab/focus-loss/deactivate and escape.
...
Leave existing EditorViewState/remap sentinels and their assertions unchanged. The settings schema, stored laneHeight/laneHeights/emptyLanes/hiddenLanes and duplicate-destination rejection are not being refactored. Only adapt setup that actually controls the removed UI; do not rewrite identity/state tests merely to use laneRanges instead.

### Step 3

Keep horizontal automation pan and pointer-grab/follow-scroll cancellation assertions. Remove only vertical automation scroll expectations; keep the shared plot origin, viewport size and no-stale-document-write proof.

## Acceptance predicate

Host geometry and lifecycle checks pass with one active automation plot, while document/track replacement and gesture cancellation behavior remain unchanged.

## Controller verification

Controller: `deno task verify --filter host- --verbose`.
