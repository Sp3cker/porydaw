# Task 14: Adapt hover fixtures to parameter activation
...
Replace only vertical automation offset conversions with direct viewport coordinates; preserve effectiveDragContent/horizontal coordinate arithmetic and device-pixel rounding. Example final transform:
```cpp
return plot->mapToScene(contentPoint).toPoint();
```

### Step 3

Retain existing hover assertions on text/value, source tick, empty/implicit lead-in, node hit and lifecycle. Add an away/back parameter activation to the existing origin-hover case so a stale cache cannot display the previous parameter's held value.

## Acceptance predicate

Hover and phantom-origin readouts follow the active parameter after switching, while existing hover timing, value, clipping and cancellation outcomes stay unchanged.

## Controller verification

Controller: `deno task verify --filter automation-hover --verbose`.
