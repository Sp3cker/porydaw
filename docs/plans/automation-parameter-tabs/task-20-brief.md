# Task 20: Migrate shared-selection probe and core fixtures
...
- `src/checks/selectionkey/automationprobe.h`
- `src/checks/selectionkey/automationprobe.cpp`
- `src/checks/selectionkey/corefixture.cpp`

## Prerequisites

Completed/reviewed tasks: 5, 6, 10.

## Interface contract

Preserve AutomationProbe::locate identity behavior and CoreFixture creation. The probe must separate logical-lane lookup from explicit activation; no new focus memory or keyboard origin.
...
Remove only the vertical offset term from window coordinate mapping:
```cpp
return input.mapToScene(contentPoint).toPoint();
```
Preserve horizontal camera and device-pixel calculations. Retain the existing CC10/notes/time-range fixture data.

### Step 3

Configure the outer automation section rather than row heights. Keep focusBand and shared key origin unchanged so Copy/Delete/SelectAll/Paste continue to exercise SongView, not a new tab-local dispatcher.

## Acceptance predicate

Shared key tests locate and activate the intended automation parameter without scrolling, while their selection scopes and command targets remain unchanged.

## Controller verification

Controller: `deno task verify --filter selectionkey-core --verbose` once all selectionkey consumers migrate.
