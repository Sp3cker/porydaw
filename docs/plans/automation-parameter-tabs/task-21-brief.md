# Task 21: Preserve shared gesture arbitration after removing the automation thumb
...
- `src/checks/selectionkey/gesturethumbs.cpp`
- `src/checks/selectionkey/gesture.cpp`

## Prerequisites

Completed/reviewed tasks: 20.

## Interface contract

Uses the same TimelineQuickView gesture ownership and SongView command policy; no production keyboard changes.

## Implementation steps

### Step 1

Remove only test data rows that require the deleted automation scrollbar. Keep equivalent horizontal timeline and roll vertical thumb cases with their original command/cancellation expectations.
...
Drop vertical automation coordinate offsets. Keep Escape, Delete, Copy/Paste and modifier routing outcomes unchanged; explicit shared selections remain targets even when another parameter is displayed.

## Acceptance predicate

Existing shared key commands retain gesture cancellation/arbitration semantics, with automation-thumb-only cases removed and node/remaining-scrollbar gestures preserved.

## Controller verification

Controller: `deno task verify --filter selectionkey-gesture --verbose`.
