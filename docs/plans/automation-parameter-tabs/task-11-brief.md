# Task 11: Prove parameter switches invalidate stale forms and gestures
...
For the drag scenario use the existing armCcDrag/real pointer sequence to reach a visible provisional preview; invoke activateParameter(Tempo) while the gesture is active, then release the old pointer. Assert no transaction, no remaining pointer grab/follow-scroll pause, and cleared old transient/hover state. Finally reactivate CC and perform an ordinary successful drag to prove recovery.

### Step 3

Update point-menu fixture entry points to explicitly activate their intended parameter before fresh input. Do not change captured document/revision/row-id guards, undo semantics or the existing shared-popup ownership policies.

## Acceptance predicate

A parameter switch cannot commit a previously opened value prompt or active drag to the new parameter, and does not cancel a foreign popup or mutate either lane before a valid new edit.
