# Task 9: Preserve phantom source editing and normal node gestures
...
Completed/reviewed tasks: 8.

## Interface contract

Consumes task 8 activateParameter(row). Existing rowId(adapter), pointsOf/setPoints, FrozenDocumentState and gesture helpers remain authoritative. No production phantom/gesture algorithm edits are permitted in this task.

## Implementation steps

### Step 1

Before direct input in parameterized node scenarios, explicitly activate rowId(adapter). Replace simultaneous-lane paint assumptions with separate activation/observation, preserving every existing expected point/value/undo result. Preserve emptyTempoStorageComposesNoLeadIn, firstNonzeroTempoPointComposesImplicitLeadInCurve and explicitTickZeroTempoPointSuppressesLeadInCurve.

### Step 2

Extend scrolledOriginPhantomCommits after its horizontal-scroll setup, before pointer coordinates are captured:
```cpp
const FrozenDocumentState switched = frozenDocumentState();
const EditorAutomationRowId other = adapter == kTempo
...
```
kController is the file's existing CC10 fixture constant. Keep the original `sourceContent.x()==0`, hit radius, preview-direction and source-tick assertions. The expected vector remains `{firstTick,firstValue},{nodeTick,movedValue},{lateTick,lateValue}`; never expect insertion at the viewport edge.

### Step 3

After the phantom commit retain its revision check and add undo through the existing SongTab history path:
```cpp
QVERIFY(std::holds_alternative<DocumentHistoryApplied>(tab().history().requestUndo()));
QCOMPARE(tab().document().smf().write(), before.smf);
```
Keep nodeDragShiftAxisLocks, stationaryNodeInteractions, double-click guard, selectedRangeDragAndDelete, escape and rebuild-cancel scenarios with unchanged musical outcomes.

### Step 4

Adapt transient/hover paint probes to the active full-height plot only; use normalized existing inputPoint geometry rather than changed hard-coded value expectations. Do not delete a phantom, node-ring or preview assertion merely because the new visible plot needs an explicit activation.

## Acceptance predicate

Both Tempo and CC origin phantoms still paint, hover and drag their original source event after switching away and back, with the original tick/order and one undoable transaction preserved.

## Controller verification

Controller: after writers settle, run `deno task verify --filter automation-editing --verbose` once the editing-suite migration group is complete. Compilation is required at each settled task; run the suite at its completed surface checkpoint. Obsolete scenarios are migrated, not suppressed or declared passing. Final phantom pixels are also exercised by automation-raster/native rendering, not software-only captures.
