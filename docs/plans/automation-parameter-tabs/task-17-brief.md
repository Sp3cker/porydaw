# Task 17: Remove automation-scrollbar tests without weakening remaining scrollbars
...
Automation vertical scroll is removed; `timelineQuickView.verticalScroll*` remains piano-roll state and is out of scope.
...
Update canonical-band/layout/resize/event-list scenarios to expect no drawerAutomationScrollBar item and no reserved automation scrollbar strip. Retain existing assertions that the two remaining scrollbar tracks follow their own canonical bands.

### Step 3

Use a real parameter-label click after resizing to prove the newly reclaimed gutter/body geometry remains interactive; do not replace removed scrolling tests with tautological zero-value accessor tests.

## Acceptance predicate

The scrollbar suite requires no automation scrollbar and still proves all existing horizontal timeline and piano-roll vertical scrolling behavior.

## Controller verification

Controller: `deno task verify --filter scrollbar --verbose` after the chrome removal task.
