# Task 16: Preserve raster interaction and native playhead alignment
...
In RenderingPlayheadTest::automationHoverDecor use the active plot body and real parameter activation rather than addEmptyLane/pinnedTempoRect. Keep playhead-versus-hover placement and no-outside-pixels assertions. Do not touch piano-roll/velocity/voice rendering behavior.

### Step 3

Exercise horizontal scrolling past a node, switch parameter away/back, and observe the phantom at the plot origin before and during drag. Keep the original source-tick/value assertions and clear old hover/transient pixels on switch.

## Acceptance predicate

Real pointer edits, origin hover and playhead/grid alignment still agree with the active plot on the native backend, without drawing outside its canonical band.

## Controller verification

Controller: `deno task verify --filter automation-raster --verbose` and `deno task verify --filter rendering-playhead --verbose`. Native desktop input/rendering is required; coordinate with the user rather than repeatedly stress-running these cases.
