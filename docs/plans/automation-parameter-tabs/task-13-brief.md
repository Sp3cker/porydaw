# Task 13: Replace pinned Tempo presentation tests with tab presentation
...
Keeps real Quick scene/text/layer assertions and the existing presentation fixture. Remove the pinned/collapsed Tempo contract; do not rename it into a misleading shim.

## Implementation steps

### Step 1

Remove slots and bodies for collapsedTempoGeometryIsPinned, headerClickExpandsTempoToConfiguredHeight, verticalScrollPinsTempoOverCcContent, viewportResizeKeepsTempoPinned, headerClickRecollapsesTempoAndRecoversCanvasSpace and headerReexpansionRestoresConfiguredTempoHeight. Remove expanded/collapsed Tempo covering-CC clipping cases, since no second plot covers another.
...
Use existing distinct literal node values/ticks/colors to show the inactive parameter's nodes are absent from the active plot. Retain meaningful top/bottom borders, overflow clipping, semantic label identity and value-label paint tests. Test selection inclusion separately from active-tab fill so hidden selected lanes are discoverable.

### Step 4

Remove direct laneHeights/emptyLanes setup; set the outer automation section height and activate the intended row. Keep document-backed adapter construction, theme and track-color assertions intact.

## Acceptance predicate

Presentation tests prove all labels fit, only the active parameter is painted, Tempo occupies the shared plot, and selected inactive parameters retain visible scope indicators.

## Controller verification

Controller: `deno task verify --filter automation-presentation --verbose`. Repeat only the final native visual matrix with an enlarged font; do not assert arbitrary exact pixel sizes.
