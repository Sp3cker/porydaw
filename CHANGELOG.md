...
## Added
- Added JavaScript plugins: drop a folder with a `plugin.json` and `main.js` into the plugins folder (Settings → Plugins) and it loads at startup, hot-reloads on edit, can register rebindable commands (Settings → Keyboard Shortcuts) and read the song, selection, cursor, project and transport. View → Script Console shows plugin output and offers a REPL. See docs/scripting/API.md.
- Plugins can edit the song through `porydaw.edit` transactions: any number of note, automation, tempo, loop, time-signature, track and ripple edits become one Undo entry, and a script error rolls the whole transaction back. Bundled example `plugins/examples/note-tools` adds Legato, Insert chord, Humanize, Strum and Quantize commands to the piano roll.
- Plugins can now add entries to a **Plugins** menu and to the piano roll's note and time-selection context menus, open dialogs (alerts, prompts, forms, file pickers), read and write files in their folder or the project, keep per-song settings in the song's sidecar, draw overlays on the piano roll, read and edit the raw MIDI events of a chunk, move or duplicate a time range, open another song, and render the song to WAV. Bundled examples: `song-report`, `range-tools`, `scale-guide`.
