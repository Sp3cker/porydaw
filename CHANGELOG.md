...
- Insert Time is now selection-aware: with an active time selection it inserts a silent gap the length of the selection immediately (no prompt), honoring the selection's resolved scope. The selection stays over the new blank span and the edit cursor commits to the start seam; without a selection it still opens the bars/beats/fractions prompt for a whole-song insertion at the edit cursor, even while playing.
- The Edit menu and both time-selection context menus now offer Delete Time (Shift Left) (`edit.delete_time`, no default binding): it removes the active selection's whole span and shifts later scoped content left, clearing the selection and parking the edit cursor at the seam. There is no confirmation and one undo step restores the song; ordinary Delete, Backspace, and Cut remain contents-only and never shift time.
...
- Replace the always-prompt Insert blank time command with selection-aware Insert Time (`edit.insert_time`, `Ctrl+Shift+I`): an active selection inserts immediately while an active but unresolvable selection rejects silently rather than falling through to the whole-song prompt. `Remove contents (shift left)` was renamed Delete Time (Shift Left) in both context menus but keeps its guarded, silent no-op behavior.
- Keyboard shortcuts are now fixed platform defaults: the Keyboard settings page, remapping, and conflict/reset machinery are removed, and every menu, context menu, and key press invokes the same canonical action with its real platform-native binding. Window-scoped commands (transport, drawer toggles, Copy, Solo, Undo) fire anywhere in the window; editor commands are routed to the active editor after protected local input — text/IME entry, modal prompts, popup navigation, event-list row keys, and audition surfaces keep their keys.
- Right-clicking the timeline ruler now distinguishes selection from cursor intent: a press inside the active time selection preserves it and opens the selection menu without seeking; a press outside (or with no selection) commits the edit cursor to the clicked tick, seeks, and opens the cursor menu. The note grid's right-click handling is unchanged and never moves the cursor or seeks. Context menus now close themselves when the selection, scope, or edit cursor they were opened against changes, and menu enablement reflects the committed selection rather than an in-progress drag.
