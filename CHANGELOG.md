...
- Right-clicking an automation point now opens a small menu (Set value…, Delete) instead of deleting the point outright. Set value… opens the same type-in as double-click and edits exactly the clicked point in place — with same-tick duplicate points, the one under the cursor is targeted, not just the nearest in time. The targeted point shows a highlight ring while the menu is open, and right-clicking another point moves the menu there in one gesture, like the piano roll's note menu. Right-clicking a voice marker still deletes it directly.
