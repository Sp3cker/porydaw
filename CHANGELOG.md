...
## Added
- Added a velocity lane: a resizable pane between the piano roll and the automation lanes showing the selected track's notes as nodes on the timeline — a dot at each note's velocity with a stem across its length — beside a 1–127 value ruler. It is hidden until you turn it on with View → Velocity Lane or the V key, and the setting is remembered. Selected notes are ringed, with their velocities marked on the ruler; the lane pans and zooms with the roll. Dragging a node — or its stem — moves every selected note's velocity together, as one undoable edit applied when you let go; clicking selects (Ctrl adds or removes, empty space clears), and Escape abandons a drag in progress. Dragging from empty space paints across the selected notes the stroke crosses, Shift-dragging ramps them along a straight line, and clicking one of the ruler's printed values sets the whole selection to it. Hovering a node marks its value on the ruler. On a track playing through one of the GBA's four PSG channels the lane switches to that channel's real volume levels: the ruler becomes one row per level ("Vol 1…N"), the level boundaries are drawn across the lane, notes sit on the level they actually sound at, and edits move whole levels instead of velocity values that the hardware cannot tell apart — so a nudge is always an audible step. The header's Detents button turns that off for the track, and holding Ctrl (rebindable) while starting a drag writes exact velocities for that one gesture. Marquee selection is not in yet.
