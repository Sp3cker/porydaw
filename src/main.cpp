...
// scalecheck.cpp; scale catalog and diatonic destination resolver check.
...
// selectioncheck.cpp; public-interface EditorSelectionModel contract check.
...
int main(int argc, char *argv[])
{
    ...
    if (args.contains(QStringLiteral("--selectioncheck")))
        return runSelectionCheck();
    ...
    const int scaleCheck = args.indexOf(QStringLiteral("--scalecheck"));
    if (scaleCheck >= 0 && scaleCheck + 1 < args.size())
        return runScaleCheck(args[scaleCheck + 1]);
