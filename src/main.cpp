...
// noteidcheck.cpp; NoteId parse/serialize/projection boundary check.
...
int main(int argc, char *argv[])
{
    ...
    const int noteIdCheck = args.indexOf(QStringLiteral("--noteidcheck"));
    if (noteIdCheck >= 0 && noteIdCheck + 1 < args.size())
        return runNoteIdCheck(args[noteIdCheck + 1]);
