...
{
    // CCs whose PrintControllerOp branch emits game command bytes, audited
    // against the bundled converter. midi.cpp turns each SMF controller
    // (0xB0) into EventType::Controller with param1 = CC and param2 = value,
    // unfiltered, so the CC number alone picks the branch cited per group:
    // MOD/VOL/PAN/BENDR/LFOS/MODT/TUNE/LFODL and PRIO emit PrintOp/PrintByte
    // bytes; 0x0C/0x10 reach PrintMemAcc, which emits MEMACC bytes for the
    // implemented stored ops 0x00-0x05; 0x11 writes the asm loop label.
    // Everything else reaches only PrintWait: PORTAMENTO/PWMC/PWMS and the
    // MEMACC op/params have no emitting branch, XCMD plumbing (0x1D-0x1F)
    // emits nothing as bare CC bytes, and unmapped CCs hit the default.
    constexpr std::array<uint8_t, 13> kExportedCcs = {
        0x01, 0x07, 0x0A, 0x0C, 0x10, 0x11, 0x14, 0x15, 0x16, 0x18, 0x1A, 0x21, 0x27,
    };
    const bool supported = std::binary_search(kExportedCcs.begin(), kExportedCcs.end(), cc);
    return supported ? M4aExportSupport::Supported : M4aExportSupport::NotExported;
}
