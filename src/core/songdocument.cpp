...
bool cfgSemanticEqual(const SongCfg &a, const SongCfg &b) {
  return a.voicegroupArg == b.voicegroupArg &&
         a.masterVolume == b.masterVolume && a.reverb == b.reverb &&
         a.priority == b.priority && a.exactGate == b.exactGate &&
         a.extendedClocks == b.extendedClocks &&
         a.noCompression == b.noCompression;
}
...
// Repeated keyboard moves extend the command already at the top of the undo
// stack. Extending rewinds to the gesture's original notes before rebuilding,
// so a neighbor trimmed by an intermediate overlap comes back when the notes
// move past it. The document emits once, after the final state is in place.
...
public:
  ...
  static constexpr int CommandId = 0x4d76; // 'Mv'

  int id() const override { return m_mergeable ? CommandId : -1; }

  bool extend(const std::vector<DocNote> &notes, int64_t dTick, int dKey) {
    if (!m_mergeable || !movesMyOutputs(notes))
      return false;
    m_doc->revertOps(m_ops);
    m_dTick += dTick;
    m_dKey += dKey;
...
private:
  ...
  bool movesMyOutputs(const std::vector<DocNote> &next) const {
    ...
    for (const DocNote &n : next)
      theirs.push_back({n.engineTrack, n.tick, int(n.key), n.duration,
                        n.velocity, n.channel});
...
bool SongDocument::save(QString *error) {
  ...
  if (!cfgSemanticEqual(m_cfg, m_savedCfg) || !m_hadCfgLine) {
    ...
    if (!SongRegistry::writeSongFlags(QFileInfo(m_midPath).path(), m_label,
                                      flags, error))
      return false;
    m_savedCfg = m_cfg;
    m_hadCfgLine = true;
...
void SongDocument::rebuildTrackMap() {
  ...
  for (size_t t = 0; t < m_smf.tracks.size() && m_engineToSmf.size() < 16;
       t++) {
...
bool SongDocument::findNote(int engineTrack, uint64_t tick, uint8_t key,
                            DocNote *out) const {
...
std::vector<DocLanePoint> SongDocument::lanePoints(int engineTrack,
                                                   uint8_t cc) const {
...
bool SongDocument::findLoopMarkerEvent(bool endMarker, int *smfTrack,
                                       size_t *index) const {
...
SmfEvent SongDocument::makeChannelEvent(uint8_t typeNibble, uint8_t channel,
                                        uint64_t tick, uint8_t data0,
                                        uint8_t data1) const {
  SmfEvent ev;
  ev.tick = tick;
...
void SongDocument::appendNoteInsertOps(std::vector<EditOp> &ops, int smfTrack,
                                       uint8_t channel, uint64_t tick,
                                       uint8_t key, uint32_t duration,
  ...
  end.event = makeChannelEvent(0x9, channel,
                               tick + std::max<uint32_t>(1, duration), key, 0);
  ops.push_back(end);
}
...
void SongDocument::resolveNoteOverlaps(
    const std::vector<PlannedNote> &written,
...
void SongDocument::addNote(int engineTrack, uint64_t tick, uint8_t key,
                           uint32_t duration, uint8_t velocity) {
  const int smfTrack = smfTrackFor(engineTrack);
  if (smfTrack < 0)
    return;
  ...
  appendNoteInsertOps(ops, smfTrack, channelFor(engineTrack), tick, key,
                      duration, velocity);
  ops.insert(ops.end(), trims.begin(), trims.end());
  pushEdit(tr("add note"), std::move(ops));
}

void SongDocument::addNotes(int engineTrack,
                            const std::vector<NewNote> &notes) {
  const int smfTrack = smfTrackFor(engineTrack);
...
void SongDocument::moveNotes(const std::vector<DocNote> &notes, int64_t dTick,
                             int dKey, bool mergeable) {
  if (notes.empty() || (dTick == 0 && dKey == 0))
    return;
  const int index = m_undoStack.index();
  if (mergeable && index > 0 && index == m_undoStack.count() &&
      index != m_undoStack.cleanIndex()) {
    const QUndoCommand *command = m_undoStack.command(index - 1);
    if (command->id() == MoveNotesCommand::CommandId) {
      auto *move = const_cast<MoveNotesCommand *>(
          static_cast<const MoveNotesCommand *>(command));
      if (move->extend(notes, dTick, dKey))
        return;
    }
  }
...
SongDocument::buildMoveNotesOps(const std::vector<DocNote> &notes,
  ...
  for (const DocNote &note : notes) {
    ...
    written.push_back(
        {note.engineTrack, newKey, newTick, newTick + note.duration});
  }
  std::vector<EditOp> trims;
  ...
  for (const DocNote &note : notes) {
    ...
    if (note.unterminated()) {
      ...
      appendNoteInsertOps(ops, note.smfTrack, note.channel, newTick,
                          uint8_t(newKey), note.duration, note.velocity);
    }
  }
  ops.insert(ops.end(), trims.begin(), trims.end());
  return ops;
...
void SongDocument::resizeNotes(const std::vector<DocNote> &notes,
                               int64_t dDuration) {
  ...
  for (const DocNote &note : notes) {
    ...
    end.event = makeChannelEvent(0x9, note.channel, note.tick + newDuration,
                                 note.key, 0);
    ops.push_back(end);
  }
  ops.insert(ops.end(), trims.begin(), trims.end());
...
void SongDocument::resizeNotesLeft(const std::vector<DocNote> &notes,
                                   int64_t dTick) {
  ...
  for (const DocNote &note : notes) {
    ...
    const uint64_t newTick = uint64_t(std::clamp<int64_t>(
        int64_t(note.tick) + dTick, 0, int64_t(endTick) - 1));
  ...
  for (const DocNote &note : notes) {
    ...
    on.smfTrack = note.smfTrack;
    on.event =
        makeChannelEvent(0x9, note.channel, newTick, note.key, note.velocity);
    ops.push_back(on);
  }
  ops.insert(ops.end(), trims.begin(), trims.end());
  pushEdit(tr("resize %n note(s)", nullptr, int(notes.size())), std::move(ops));
...
void SongDocument::setNotesVelocity(const std::vector<DocNote> &notes,
                                    uint8_t velocity) {
  if (notes.empty())
    return;
  std::vector<EditOp> ops;
...
void SongDocument::nudgeNotesVelocity(const std::vector<DocNote> &notes,
                                      int delta) {
  ...
  for (const DocNote &note : notes) {
    ...
    op.smfTrack = note.smfTrack;
    op.index = note.onIndex;
    op.event = makeChannelEvent(
        0x9, note.channel, note.tick, note.key,
...
SmfEvent SongDocument::makeLaneEvent(uint8_t cc, uint8_t channel, uint64_t tick,
  ...
  if (cc == DOC_CC_VOICE)
    return makeChannelEvent(0xC, channel, tick,
                            uint8_t(std::clamp(value, 0, 127)), 0);
  return makeChannelEvent(0xB, channel, tick, cc,
                          uint8_t(std::clamp(value, 0, 127)));
}

void SongDocument::addLanePoint(int engineTrack, uint8_t cc, uint64_t tick,
                                int value) {
  ...
  pushEdit(cc == DOC_CC_VOICE ? tr("add voice change")
                              : tr("add automation point"),
           std::move(ops));
}

void SongDocument::writeLanePoints(int engineTrack, uint8_t cc,
...
void SongDocument::moveLanePoint(int engineTrack, uint8_t cc,
                                 const DocLanePoint &point, uint64_t newTick,
                                 int newValue) {
  const QString text =
      cc == DOC_CC_VOICE ? tr("change voice") : tr("edit automation point");
  std::vector<EditOp> ops;
  appendEventEditOps(
      ops, point.smfTrack, point.index,
...
void SongDocument::moveRange(const std::vector<DocNote> &notes,
  ...
  for (const DocNote &note : notes) {
    ...
    const uint64_t newTick =
        uint64_t(std::max<int64_t>(0, int64_t(note.tick) + dTick));
    written.push_back(
        {note.engineTrack, note.key, newTick, newTick + note.duration});
  }
...
void SongDocument::modifyRawEvent(int smfTrack, size_t index,
...
void SongDocument::setLoopTick(bool endMarker, int64_t tick) {
  ...
  pushEdit(endMarker ? tr("set loop end") : tr("set loop start"),
           std::move(ops));
}
...
void SongDocument::moveTimeSig(uint64_t fromTick, uint64_t toTick) {
  ...
  for (size_t t = 0; t < m_smf.tracks.size(); t++) {
    ...
    for (const DocTimeSig &sig : sigs) {
      ...
      if (sig.smfTrack != int(t) ||
          (sig.tick != fromTick && sig.tick != toTick))
        continue;
...
bool nameIsLoopMarker(const QString &name) { return smfTextIsMarker(name); }
...
QString SongDocument::trackName(int engineTrack) const {
  ...
  return nameIndex == SIZE_MAX ? QString()
                               : trackNameText(track.events[nameIndex]);
}
...
SongDocument::buildTimeline(double sampleRate) const {
  ...
  for (EditOp &op : ops) {
    ...
    case EditOp::InsertEvent: {
      ...
      auto it = std::upper_bound(
          evs.begin(), evs.end(), op.event.tick,
    ...
    case EditOp::InsertTrack:
      m_smf.tracks.insert(m_smf.tracks.begin() + long(op.smfTrack),
                          op.trackData);
      break;
