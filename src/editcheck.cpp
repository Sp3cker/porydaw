...
int documentContractFailures()
{
    ...
    const auto expectRemap = [&](const char *what, const QVector<int> &smfMap,
                                  const QVector<int> &engineMap, int newSmfCount,
                                  int newEngineCount) {
        expect(remaps.size() == 1 && remaps.front().smfTrackMap == smfMap &&
                   ...
                   order == std::vector<QString>{QStringLiteral("remap"),
                                                 QStringLiteral("changed")},
               what);
    };

    const auto notes = doc.notesForTrack(0);
    expect(notes.size() == 2 && notes[0].noteId.isAssigned() &&
               notes[1].noteId.isAssigned() && notes[0].noteId != notes[1].noteId,
           "duplicate note-ons did not receive distinct identities");
    if (ok) {
        DocNote byId, bySecondId;
        expect(doc.findNote(notes[0].noteId, &byId) &&
                   doc.findNote(notes[1].noteId, &bySecondId) &&
    ...
    if (ok) {
        ...
        expect(result && *result == before + 1 && doc.revision() == before + 1 &&
                   ...
                   secondNow.velocity == 127 && order == std::vector<QString>{QStringLiteral("changed")},
               "velocity batch did not last-write, clamp, and commit atomically");
        expect(doc.findNote(notes[0].noteId, &firstNow) &&
                   firstNow.noteId == notes[0].noteId && firstNow.velocity == 99,
        ...
        expect(doc.revision() == changedRevision + 1 && doc.findNote(notes[0].noteId, &firstNow) &&
                   ...
                   secondNow.velocity == 90 && order == std::vector<QString>{QStringLiteral("changed")},
    ...
    if (ok) {
        ...
        const auto staleResult =
            doc.setNotesVelocities(before - 1, {{current.noteId, 42}});
        ...
        const auto staleBatchResult = doc.setNotesVelocities(
            before - 1, {{current.noteId, 42}, {notes[1].noteId, 77}});
        expect(!staleBatchResult && doc.smf().write() == unchanged &&
                   doc.revision() == before && doc.undoStack()->count() == undoCount &&
                   order.empty(),
               "stale velocity batch was not rejected atomically");
        const NoteId invalidId;
        const auto invalidBatchResult = doc.setNotesVelocities(
            before, {{current.noteId, 42}, {invalidId, 77}});
        expect(!invalidBatchResult && doc.smf().write() == unchanged &&
                   doc.revision() == before && doc.undoStack()->count() == undoCount &&
                   order.empty(),
               "invalid velocity NoteId partially mutated the batch");
        DocNote secondCurrent;
        doc.findNote(notes[1].noteId, &secondCurrent);
        const auto noOpResult = doc.setNotesVelocities(
            before, {{current.noteId, current.velocity},
    ...
    if (ok) {
        ...
        clearSignals();
        doc.undoStack()->undo();
        expectRemap("track insertion undo remap was incomplete", {0, 1, 2, -1}, {0, 1, -1}, 3,
                    2);
        clearSignals();
        doc.undoStack()->redo();
        ...
        expectRemap("track deletion remap did not mark deleted owners", {0, 1, 2, -1},
                    {0, 1, -1}, 3, 2);
        clearSignals();
        doc.undoStack()->undo();
        ...
        clearSignals();
        doc.undoStack()->redo();
        expectRemap("track deletion redo remap was incomplete", {0, 1, 2, -1}, {0, 1, -1}, 3,
                    2);
    }
    if (ok) {
        clearSignals();
        ...
        clearSignals();
        doc.undoStack()->undo();
        expectRemap("track duplication undo remap was incomplete", {0, 1, 2, -1}, {0, 1, -1}, 3,
                    2);
        clearSignals();
        doc.undoStack()->redo();
    ...
    if (ok) {
        ...
        clearSignals();
        doc.undoStack()->undo();
        expectRemap("metadata-to-engine undo remap was incomplete", {0, 1, 2, 3},
                    {-1, 0, 1, 2}, 4, 3);
        clearSignals();
        doc.undoStack()->redo();
        expectRemap("metadata-to-engine redo remap was incomplete", {0, 1, 2, 3}, {1, 2, 3}, 4,
                    4);
        ...
        expectRemap("engine-to-metadata remap was incomplete", {0, 1, 2, 3}, {-1, 0, 1, 2}, 4,
                    3);
        clearSignals();
        doc.undoStack()->undo();
        expectRemap("engine-to-metadata undo remap was incomplete", {0, 1, 2, 3}, {1, 2, 3}, 4,
                    4);
        clearSignals();
        doc.undoStack()->redo();
        expectRemap("engine-to-metadata redo remap was incomplete", {0, 1, 2, 3}, {-1, 0, 1, 2},
                    4, 3);
    }
    if (ok) {
        SmfFile collision;
        ...
        QObject::connect(&collisionDoc, &SongDocument::tracksRemapped,
                         [&collisionRemaps](TrackRemap remap) {
                             collisionRemaps.push_back(std::move(remap));
                         });
        ...
        expect(collisionDoc.channelFor(0) == 1 && source.size() == 1 &&
                   source.front().tick == 0 && source.front().key == 60 &&
                   source.front().duration == 24 && source.front().velocity == 100 &&
                   source.front().noteId.isAssigned() && foreignNote.noteId.isAssigned() &&
                   foreignNote.noteId != source.front().noteId &&
        ...
        if (copiedSmfTrack >= 0) {
            ...
            copiedEvents =
                copiedTrack.endTick == sourceTrack.endTick && copiedTrack.events.size() == 3 &&
        ...
        if (copy >= 0 && !copied.empty() && !source.empty()) {
            ...
            expect(collisionDoc.findNote(copiedId, &redone) && redone.tick == source.front().tick &&
                       redone.key == source.front().key && redone.duration == source.front().duration &&
    ...
    if (ok) {
        ...
        expect(activeGlobalsAreOriginal(), "duplicate-global fixture did not load canonical globals");
        ...
        if (copiedOnlyOwned) {
            ...
            copiedOnlyOwned =
                copiedTrack.endTick == globalSource.endTick && copiedTrack.events.size() == 3 &&
    ...
    if (ok) {
        ...
        const bool collisionsLoaded =
            collisions.writeFile(collisionsPath, &error) && collisionsDoc.load(collisionsInfo, &error);
        ...
        const bool collisionsReady =
            collisionsResolved && a.unterminated() && b.unterminated() &&
            a.velocity == b.velocity && a.noteId.isAssigned() && b.noteId.isAssigned() &&
            a.noteId != b.noteId;
        ...
        if (collisionsReady) {
            ...
            expect(collisionsDoc.undoStack()->count() == countBefore + 1 &&
                       collisionsDoc.revision() == beforeFirstMove + 1 &&
                       hasCollisionState(61, 61),
    ...
    if (ok) {
        ...
        const bool movesLoaded = moves.writeFile(movesPath, &error) && movesDoc.load(movesInfo, &error);
        ...
        QObject::connect(&movesDoc, &SongDocument::documentChanged, [&moveOrder] {
            moveOrder.push_back(QStringLiteral("changed"));
        });
        ...
        if (ok) {
            ...
            expect(movesDoc.undoStack()->count() == countBefore && !movesDoc.undoStack()->canUndo() &&
            ...
            expect(movesDoc.smf().write() == reversalBaseline &&
                       movesDoc.revision() == afterInverse && moveRemaps.empty() && moveOrder.empty(),
        ...
        if (ok) {
            ...
            expect(movesDoc.findNote(0, 0, 70, &moved) &&
                       movesDoc.findNote(0, 2, 70, &survivor) && survivor.duration == 2,
        ...
        if (ok) {
            ...
            expect(movesDoc.findNote(0, 0, 71, &moved) &&
                       movesDoc.findNote(0, 0, 70, &survivor) && survivor.duration == 4,
        ...
        if (ok) {
            ...
            expect(movesDoc.findNote(0, 0, 69, &moved) &&
                       movesDoc.findNote(0, 0, 70, &survivor) && survivor.duration == 4,
...
int runEditCheck(const QString &projectRoot)
{
    ...
    // instead of stacking another duplicate. Undo restores the duplicates.
    {
        ...
        QObject::connect(&doc, &SongDocument::documentChanged, [&changedSignals] {
            changedSignals++;
        });
        ...
        if (ok) {
            ...
            changedSignals = 0;
            doc.moveLanePoint(0, 7, pt, pt.tick, pt.value);
            const auto after = ccPointsAt(7, 0);
            if (after.size() != 1 || after[0].value != 80 ||
            ...
            if (ok && (restored.size() != 2 || restored[0].value != 100 ||
                       restored[1].value != 80 || doc.smf().write() != before ||
                       doc.revision() != canonicalRevision + 1 ||
                       doc.undoStack()->count() != beforeUndoCount + 1 ||
        ...
        if (ok) {
            ...
            if (!doc.findLanePoint(0, DOC_CC_TEMPO, 48, &pt) ||
                ...
                const int beforeUndoIndex = doc.undoStack()->index();
                changedSignals = 0;
                doc.moveLanePoint(0, DOC_CC_TEMPO, pt, pt.tick, pt.value);
                if (doc.smf().write() != before || doc.revision() != beforeRevision ||
                    doc.undoStack()->count() != beforeUndoCount ||
                    doc.undoStack()->index() != beforeUndoIndex || changedSignals != 0) {
