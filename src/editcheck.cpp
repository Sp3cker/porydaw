...
int runEditCheck(const QString &projectRoot) {
  ...
  for (const SongInfo &song : project.songs()) {
    ...
    if (track >= 0) {
      ...
      if (ok) {
        ...
        edit.addNotes.push_back(
            {track, {{base + step * 40, 65, step * 2, 90}}});
        ...
        if (doc.findNote(track, base + step * 30, 60, &n) ||
            ...
            !doc.findLanePoint(track, 7, base + step * 40, &p) ||
            p.value != 70 ||
      ...
      if (ok) {
        ...
        if (ok) {
          ...
          if (documentChanges != 2) {
            fail("merged move emitted more than one document change per "
                 "keypress");
            ok = false;
          }
        ...
        QObject::disconnect(documentChanged);
      }
      ...
      if (ok) {
        ...
        DocLanePoint pt;
        if (!doc.findLanePoint(track, 7, base + step * 2, &pt) ||
            pt.value != 100) {
          ...
          if (!doc.findLanePoint(track, 7, base + step * 5, &pt) ||
            ...
            if (doc.findLanePoint(track, DOC_CC_TEMPO, base + step * 4,
                                  &tempoPt))
    ...
    if (ok && doc.canAddTrack()) {
      ...
      if (newTrack < 0) {
        ...
        if (ok && (seed.empty() || seed.front().tick != 0 ||
                   seed.front().value != 7)) {
    ...
    if (ok) {
      ...
      DocTimeSig sig;
      if (ok &&
          (!findSig(base, &sig) || sig.numerator != 3 || sig.denomPow2 != 3)) {
    ...
    doc.setLoopTick(false,
                    loopStart == UINT64_MAX ? 0 : int64_t(loopStart + step));
  ...
  // names. Synthetic file — decomp projects are format 1 in practice.
  {
    ...
    // silent data loss).
    tr.events.push_back(
        meta(0, 0x51, QByteArray("\x07\xA1\x20", 3))); // 120 BPM
    ...
    if (ok) {
      const auto note = [&doc](int track) {
        ...
        return notes.size() == 1 && notes[0].duration == 24 ? int(notes[0].key)
                                                            : -1;
      };
    ...
    if (ok && (doc.trackName(1) != QStringLiteral("Lead") ||
    ...
    if (ok) {
      ...
      if (ok && !hasMeta(chunks[4], 0x03, "Ambient")) {
        fail0(
            "prefixed name on a silent channel was lost (no name-only chunk)");
        ok = false;
      }
      if (ok) {
