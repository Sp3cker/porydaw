...
// Shared production-shell session for the selectionkey window-tier checks
// (window.cpp, localinput.cpp): one shown MainWindow with a scratch project
// and song opened through WorkspaceUi's non-dialog seams, plus the readiness
// waits and Quick lookups both files share. Rig-agnostic assertion/delivery
// primitives live in checks/selectionkey/primitives.h, which this header
// includes; the sibling core/gesture checks drive standalone rigs and share
// only that header. The check runner redirects QSettings to a per-run
// directory, so session state stays deterministic and isolated.
...
// genuine no-prompt branch, and report a stage-labeled failure when the tab
// still counts as dirty afterwards.
inline void makeSelectedTabClean(WorkspaceUi &workspace, SongDocument &document, const char *stage,
                                 int &failures)
{
    while (document.undoStack()->index() > 0)
        document.undoStack()->undo();
    settle();
    check(failures, !workspace.selectedSongDirty(),
          stage && *stage != '\0'
              ? stage
              : "the selected tab stayed dirty after undoing its document edits");
}
