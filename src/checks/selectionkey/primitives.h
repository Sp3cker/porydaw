...
// Shared primitives for the selectionkey checks: failure accounting, real
// QTest delivery, live keymap resolution, isolated standalone-rig assembly,
// document note lookups, focus diagnostics, and keymap-override RAII.
...
// Shared failure accounting. scope keeps each suite's stderr prefix without
// per-suite copies of the same body.
inline bool check(int &failures, bool condition, const char *message,
                  const char *scope = "selectionkeycheck")
{
    if (condition)
        return true;
...
inline void fail(int &failures, const QString &message, const char *scope = "selectionkeycheck")
{
    std::fprintf(stderr, "%s: FAIL: %s\n", scope, qPrintable(message));
    ++failures;
}
