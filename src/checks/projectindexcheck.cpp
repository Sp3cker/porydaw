...
int runProjectIndexCheck(const QString &scratchProject, const QString &scratchDir)
{
    ...
    // Test 5: the application cache is enabled by default and the opt-out
    // environment variable disables only that default location.
    constexpr auto disableCacheVariable = "PORYDAW_DISABLE_INDEX_CACHE";
    const bool disableCacheWasSet = qEnvironmentVariableIsSet(disableCacheVariable);
    const QByteArray previousDisableCache = qgetenv(disableCacheVariable);
    qunsetenv(disableCacheVariable);
    if (ProjectIndex::defaultStoreDir(scratchProject).isEmpty())
        fail("default cache directory was not selected");
    qputenv(disableCacheVariable, QByteArrayLiteral("1"));
    if (!ProjectIndex::defaultStoreDir(scratchProject).isEmpty())
        fail("PORYDAW_DISABLE_INDEX_CACHE did not disable the default cache");
    if (disableCacheWasSet)
        qputenv(disableCacheVariable, previousDisableCache);
    else
        qunsetenv(disableCacheVariable);

    // Test 6: Comma-bearing flag roundtrip and dynamic midPath derivation.
    ...
    // Test 7: unregistered-song sidecar content is refreshed on a true cache hit.
    // Sidecar names participate in the fingerprint, but their contents do not.
    ...
    if (!SongRegistry::saveRegistrationMeta(scratchProject, probeLabel,
                                            QStringLiteral("MUS_INITIAL_CONSTANT"),
                                            QStringLiteral("MUSIC_PLAYER_INITIAL"))) {
        fail("cannot write initial sidecar metadata");
    }
    const QByteArray sidecarFinger = projectFingerprint(scratchProject);
    ...
    } else if (!SongRegistry::saveRegistrationMeta(scratchProject, probeLabel,
                                                   QStringLiteral("MUS_CUSTOM_CONSTANT"),
                                                   QStringLiteral("MUSIC_PLAYER_CUSTOM"))) {
        fail("cannot update sidecar metadata");
    } else {
        const QByteArray updatedSidecarFinger = projectFingerprint(scratchProject);
        QVector<SongInfo> cachedSongs;
        QVector<MusicPlayer> cachedPlayers;
        if (updatedSidecarFinger != sidecarFinger) {
            fail("sidecar content unexpectedly changed the project fingerprint");
        } else if (!ProjectIndex::load(sidecarCacheDir, scratchProject, updatedSidecarFinger,
                                       &cachedSongs, &cachedPlayers)) {
            fail("sidecar content change did not preserve the cache hit");
        } else {
