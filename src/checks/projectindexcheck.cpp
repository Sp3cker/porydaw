...
int runProjectIndexCheck(const QString &scratchProject, const QString &scratchDir)
{
    ...
    QDir().mkpath(listingDir + QStringLiteral("/directory.mid"));
    ...
    if (!ProjectIndex::save(commaCacheDir, scratchProject, commaFinger, commaSongs, commaPlayers)) {
        ...
        if (!ProjectIndex::load(commaCacheDir, scratchProject, commaFinger, &loadedCommaSongs,
            ...
            const QString expectedMid =
                QDir(scratchProject)
                    .absoluteFilePath(QStringLiteral("sound/songs/midi/mus_comma.mid"));
    ...
    if (!SongRegistry::saveRegistrationMeta(scratchProject, probeLabel,
        ...
        fail("cannot create initial sidecar metadata");
    } else {
        ...
        if (!sidecarWarmup.open(scratchProject, &error)) {
            ...
            if (!writeBytes(probeSidecar, QByteArrayLiteral("{}\n"))) {
                ...
                DecompProject sidecarReset;
                sidecarReset.setIndexCache(sidecarCacheDir);
                if (!sidecarReset.open(scratchProject, &error)) {
                    fail("sidecar reset open failed");
                } else {
