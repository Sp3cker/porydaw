...
func voiceSubtitleFollowsProgramPosition(
    ...
    report.expectEqual(expected: 1, actual: program(at: 0), cppID: id, what: "initial numeric program")
    let unaffected = h.rows[0]
    let rebuilds = h.rowRebuildCount
    fx.session.editCursor = 479
    h.refreshFromDocument()
    report.expectEqual(expected: initial, actual: h.rows[1].subtitle, cppID: id,
                       what: "cursor before change still shows first program")
    fx.session.editCursor = 480
    h.refreshFromDocument()
    report.expectEqual(expected: 5, actual: program(at: fx.session.editCursor), cppID: id,
    ...
    h.refreshFromDocument()
    report.expectEqual(expected: initial, actual: h.rows[1].subtitle, cppID: id, what: "stopped cursor restores first program")
    h.refreshPlayhead(tick: 480, playing: true)
    report.expectEqual(expected: 5, actual: program(at: 480), cppID: id,
                       what: "playing position resolves the changed numeric program")
    report.expectEqual(expected: changed, actual: h.rows[1].subtitle, cppID: id, what: "playing follows playhead over edit cursor")
    ...
    report.expectEqual(expected: 1, actual: program(at: fx.session.editCursor), cppID: id,
                       what: "stopped position resolves the original numeric program")
    report.expectEqual(expected: initial, actual: h.rows[1].subtitle, cppID: id, what: "stop immediately returns to edit cursor")
    report.expectEqual(expected: rebuilds, actual: h.rowRebuildCount, cppID: id, what: "subtitle transitions never rebuild rows")
...
func coreHeaderVoiceUndoRegression(_ report: CheckReport, suite: DocumentSession,
    ...
    report.expectEqual(expected: 127, actual: fx.document.lanePoints(track: 0, lane: .voice).first?.value,
    ...
    report.expectEqual(expected: before, actual: fx.document.state, cppID: id, what: "one undo restores the entire document")
    report.expectEqual(expected: subtitle, actual: fx.headers.rows[0].subtitle, cppID: id,
    ...
    report.expectEqual(expected: changed, actual: fx.headers.rows[0].subtitle, cppID: id,
