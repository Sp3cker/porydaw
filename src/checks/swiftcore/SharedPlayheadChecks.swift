...
private let publicationID = "swiftcore/SharedPlayhead::publicationAndRepeatedNoOp"
private let reprojectionID = "swiftcore/SharedPlayhead::cameraReprojection"
...
@MainActor
private func checkPublicationAndReprojection(_ report: CheckReport, session: DocumentSession,
                                            presenter: SharedPlayheadPresenter) {
    _ = session.mutateCamera { _ = $0.setHScroll(0) }
    let sample = session.timeline.sample(for: 48)
    report.expect(presenter.observe(sample: sample, transport: 0), cppID: publicationID,
    ...
    report.expect(near(presenter.tick, 48, tolerance: 0.5) && presenter.timelineAttached
                      && presenter.playing == false,
                  cppID: publicationID,
                  message: "the published tick is the authoritative sample's timeline position")
    report.expect(near(presenter.contentX, session.camera.contentX(tick: presenter.tick)),
                  cppID: publicationID,
    ...
    presenter.refreshProjection()
    report.expect(presenter.presentationCount == count, cppID: publicationID,
    ...
    report.expect(near(presenter.tick, tick), cppID: reprojectionID,
                  message: "a camera publication keeps the authoritative tick")
    report.expect(near(presenter.contentX, session.camera.contentX(tick: tick)), cppID: reprojectionID,
                  message: "a camera publication reprojects through the new camera")
    report.expect(presenter.presentationCount == count + 1, cppID: reprojectionID,
                  message: "one camera publication presents exactly once")
    _ = session.mutateCamera { _ = $0.setHScroll(1_000_000) }
    report.expect(!presenter.visible && presenter.contentX < 0, cppID: reprojectionID,
                  message: "a camera scrolled past the position hides the segment")
    _ = session.mutateCamera { _ = $0.setHScroll(0) }
    report.expect(presenter.visible && near(presenter.tick, tick)
                      && near(presenter.contentX, session.camera.contentX(tick: tick)),
                  cppID: reprojectionID,
