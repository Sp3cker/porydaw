...
private let wheelID = "swiftcore/EditorGridCamera::wheelPolicy"
...
@MainActor
private final class IntegrationCounters {
    var camera = 0
    var playback = 0
    var document = 0
    var cursor = 0
...
func runEditorGridCameraChecks(_ report: CheckReport, session: DocumentSession) {
    ...
    let counters = IntegrationCounters()
...
private func checkViewport(
    ...
    report.expect(
        near(first.viewportWidth, 640) && near(first.rollHeight, 320),
        cppID: viewportID, message: "viewport dimensions are pushed into the session camera")
    report.expect(
        near(first.minHScroll, expectedMin)
            && near(first.maxHScroll,
                    Double(session.timeline.lengthTicks) * first.pixelsPerTick)
            && near(first.maxVScroll, expectedMaxV),
    ...
    report.expect(
        near(grid.beatWidth, first.pixelsPerBeat) && near(grid.rowHeight, first.keyHeight),
    ...
    report.expect(
        tallViewport.maxVScroll < shortHeight.maxVScroll
            && near(tallViewport.scrollY, tallViewport.maxVScroll),
    ...
    report.expect(
        near(doubled.pixelsPerBeat, 69) && near(doubled.keyHeight, 26),
        cppID: viewportID, message: "double-font push applies the rounded font-relative defaults")
    report.expect(
        near(restored.pixelsPerBeat, 35) && near(restored.keyHeight, 13),
    ...
    report.expect(
        near(fontMinimum.pixelsPerBeat, 9) && near(fontMinimum.keyHeight, 9),
        cppID: viewportID, message: "pushed font defines time and key-height minimums")
    report.expect(
        near(fontMaximum.pixelsPerBeat, 1_387) && near(fontMaximum.keyHeight, 69),
    ...
    report.expect(
        farID != nil && expandedLength > originalLength
            ...
            && near(shrunk.scrollX, shrunk.maxHScroll),
...
private func checkWheel(
    ...
    for phase in [QtScrollPhase.noScroll, .begin, .update, .end] {
        ...
        report.expect(
            after.pixelsPerBeat > before.pixelsPerBeat
                && near(session.camera.tickAtContentX(anchorX), anchorTick, tolerance: 1e-7)
                && counters.camera == publications + 1,
            cppID: wheelID, message: "phase \(phase.rawValue) performs one anchored time zoom publication")
    }
    ...
    report.expect(
        ...
        cppID: wheelID, message: "momentum suppresses time zoom without publishing")
    ...
    grid.handleWheel(
        ...
        overGutter: false, anchorX: anchorX, anchorY: 100)
    report.expect(
        near(angleZoom, 35 * pow(1.0015, 120), tolerance: 1e-7)
            && near(session.camera.snapshot.pixelsPerBeat,
                    angleZoom * pow(1.0015, 120), tolerance: 1e-7),
        cppID: wheelID, message: "pixel deltas receive five-times angle weighting")
    report.expect(
        counters.camera == 2,
        cppID: wheelID, message: "effective angle and pixel zooms each publish once")
    ...
    grid.handleWheel(
        ...
        overGutter: false, anchorX: anchorX, anchorY: 100)
    report.expect(
        near(session.camera.snapshot.scrollX, 90) && counters.camera == 1,
        cppID: wheelID, message: "Shift wheel pans by negative selected delta exactly once")
    ...
    grid.handleWheel(
        ...
        overGutter: false, anchorX: anchorX, anchorY: 100)
    report.expect(
        near(minimumPan.scrollX, minimumPan.minHScroll)
            && minimumPublications == 1 && counters.camera == minimumPublications,
        cppID: wheelID, message: "Shift pan clamps at minimum and repeated clamped pan is silent")
    ...
    grid.handleWheel(
        ...
        overGutter: false, anchorX: anchorX, anchorY: 100)
    report.expect(
        near(maximumPan.scrollX, maximumPan.maxHScroll)
            && maximumPublications == 1 && counters.camera == maximumPublications,
        cppID: wheelID, message: "Shift pan clamps at maximum and repeated clamped pan is silent")
    ...
    grid.handleWheel(
        ...
        overGutter: false, anchorX: anchorX, anchorY: 100)
    report.expect(
        near(session.camera.snapshot.scrollX, 112) && counters.camera == 1,
        cppID: wheelID, message: "horizontal-only momentum still pans exactly once")
    ...
    report.expect(
        pitchAfter.keyHeight > pitchBefore.keyHeight
            && near((pitchAfter.scrollY + 100) / pitchAfter.keyHeight, pitchRow, tolerance: 1e-7)
            && counters.camera == 1,
        cppID: wheelID, message: "Ctrl wheel performs one row-anchored pitch zoom publication")
    ...
    report.expect(
        ...
        cppID: wheelID, message: "momentum suppresses pitch zoom without publishing")
    ...
    report.expect(
        near(session.camera.snapshot.scrollY, gutterBefore + 6) && counters.camera == 1,
        cppID: wheelID, message: "gutter wheel scrolls by negative half-delta exactly once")
}
...
private func checkProjection(
    ...
    let projected = knownNote.flatMap { note -> Bool? in
        ...
        return near(rect.x, session.camera.displayX(
            tick: Double(note.tick), origin: 0, dpr: grid.devicePixelRatio))
            && near(rect.y, expectedY + 1 / grid.devicePixelRatio)
    } ?? false
    report.expect(
        projected,
    ...
    report.expect(
        previewRect.map {
            near($0.x, session.camera.displayX(
                tick: Double(expectedTick), origin: 0, dpr: grid.devicePixelRatio))
                && near($0.y, expectedPreviewY + 1 / grid.devicePixelRatio)
        } ?? false,
...
private func checkIsolation(
    ...
    report.expect(
        counters.camera == before + 1
            && near(tickReveal.scrollX, expectedTickReveal),
    ...
    report.expect(
        counters.camera == before + 1 && near(startReveal.scrollX, 0),
    ...
    report.expect(
        counters.camera == before + 1
            && near(keyReveal.scrollY, keyReveal.maxVScroll),
        cppID: isolationID, message: "ensureKeyVisible aligns the bottom pitch at the viewport edge")
    report.expect(
        near(grid.cameraScrollX, session.camera.snapshot.scrollX)
            && near(grid.cameraScrollY, session.camera.snapshot.scrollY),
