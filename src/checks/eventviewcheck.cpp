...
// --eventviewcheck <projectRoot>: MIDI event list UI check. The pass on the
// first playable song: the event list swaps in for the roll, mirrors the
...
int runUiPass(const SongInfo &song, const QString &screenshotPath)
{
    ...
    if (anchorEngine >= 0) {
        ...
        QObject::connect(
            &doc, &SongDocument::tracksRemapped, &notificationObserver,
        ...
        const auto expectAnchor = [&](int expectedChunk, const char *what) {
            ...
            if (chunkCombo->currentIndex() < 0 ||
                chunkCombo->currentData().toInt() != expectedChunk ||
        ...
        if (doc.canAddTrack()) {
            ...
            if (addedEngine < 0) {
                ...
                expectNotifications(true,
                                    "track insertion did not notify remap before documentChanged");
                ...
                expectNotifications(true,
                                    "track deletion did not notify remap before documentChanged");
        ...
        if (doc.canAddTrack()) {
            ...
            if (copyEngine < 0) {
                ...
                expectNotifications(
                    true, "track duplication did not notify remap before documentChanged");
                ...
                expectAnchor(anchorChunk,
                             "undoing track duplication changed the source chunk anchor");
                notifications.clear();
                doc.undoStack()->redo();
                expectNotifications(true, "redoing track duplication did not notify in order");
                expectAnchor(anchorChunk,
                             "redoing track duplication changed the source chunk anchor");
                notifications.clear();
                doc.undoStack()->undo();
                expectNotifications(true, "restoring duplicated track did not notify in order");
                expectAnchor(anchorChunk,
                             "restoring duplicated track changed the source chunk anchor");
            }
        }
    }
    ...
    if (!events) {
        ...
        }
        {
            ...
            if (iA < 0 || iB != iA + 1 || iN != iA + 2) {
                ...
                checks::events::sendKey(*table, QEvent::KeyPress, Qt::Key_Up, Qt::AltModifier,
                                        QString(), false, 1);
                const auto &evs = doc.smf().tracks[chunk2];
        ...
        }
        {
            ...
            if (iA < 0 || iB < 0) {
                ...
                checks::events::sendKey(*table, QEvent::KeyPress, Qt::Key_Delete, Qt::NoModifier,
                                        QString(), false, 1);
                const auto &evs = doc.smf().tracks[chunk2];
                ...
                if (iSolo < 0) {
                    ...
                    checks::events::sendKey(*table, QEvent::KeyPress, Qt::Key_Delete,
