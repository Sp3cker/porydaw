...
func drawerVelocityValueAxisLadder(_ report: CheckReport) {
    ...
    report.expectEqual(expected: VelocityAxisModel.Mode.continuous.rawValue, actual: continuous.mode.rawValue,
    ...
    report.expectEqual(expected: 2, actual: continuous.markers.count, cppID: drawerVelocityAxisID,
                       what: "one marker per distinct displayed value")
    report.expectEqual(expected: 64, actual: continuous.markers[0].velocity, cppID: drawerVelocityAxisID,
                       what: "the lowest displayed value names the first marker")
    report.expectEqual(expected: 100, actual: continuous.markers[1].velocity, cppID: drawerVelocityAxisID,
    ...
    report.expectEqual(expected: 127, actual: continuous.yToVelocity(continuous.top), cppID: drawerVelocityAxisID,
                       what: "the ruler's top is velocity 127")
    report.expectEqual(expected: 1, actual: continuous.yToVelocity(continuous.bottom), cppID: drawerVelocityAxisID,
                       what: "the ruler's bottom is velocity 1")
    report.expectEqual(expected: 127, actual: continuous.yToVelocity(-500), cppID: drawerVelocityAxisID,
    ...
    report.expectEqual(expected: 64, actual: continuous.rulerVelocityAt(y: labelY, labelHeight: 14), cppID: drawerVelocityAxisID,
                       what: "a ruler press inside a label row takes its value")
    let betweenRows = (continuous.labels[0].y + continuous.labels[1].y) / 2
    report.expectEqual(expected: -1, actual: continuous.rulerVelocityAt(y: betweenRows, labelHeight: 2),
    ...
    report.expectEqual(expected: "Velocity", actual: continuous.accessibleDescription, cppID: drawerVelocityAxisID,
...
func drawerVelocityPsgIntrinsicRows(_ report: CheckReport) {
    ...
    let square = VelocityAxisModel(map: VelocityMap(voiceKind: .square1), geometry: geometry,
                                   activeValues: [76])
    report.expectEqual(expected: VelocityAxisModel.Mode.intrinsic.rawValue, actual: square.mode.rawValue, cppID: drawerVelocityPsgID,
                       what: "a PSG voice uses the intrinsic level ruler")
    report.expectEqual(expected: 16, actual: square.graduations.count, cppID: drawerVelocityPsgID,
                       what: "Square 1 publishes sixteen volume levels")
    let wave = VelocityAxisModel(map: VelocityMap(voiceKind: .wave), geometry: geometry)
    report.expectEqual(expected: 5, actual: wave.graduations.count, cppID: drawerVelocityPsgID,
    ...
    report.expectEqual(expected: 16, actual: noise.graduations.count, cppID: drawerVelocityPsgID,
                       what: "Noise publishes sixteen volume levels")
    report.expectEqual(expected: "Vol 10", actual: square.graduations[9].text, cppID: drawerVelocityPsgID,
                       what: "a graduation names its own level")
    report.expectEqual(expected: 76, actual: square.graduations[9].velocity, cppID: drawerVelocityPsgID,
    ...
    report.expectEqual(expected: 9, actual: square.yToLevel(square.levelToY(9)), cppID: drawerVelocityPsgID,
    ...
    report.expectEqual(expected: 76, actual: square.rulerVelocityAt(y: square.levelToY(9), labelHeight: 14),
