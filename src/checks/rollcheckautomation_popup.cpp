                                   ...
                                   int rowsHeight, int &failures)
{
    ...
    const auto menuAction = [&](const QPointF &position, const QString &trigger,
        ...
        QTimer::singleShot(0, [&, trigger] {
            ...
            if (QApplication::activePopupWidget() == menu) {
                ...
                if (actionRect.isValid()) {
                    sendMouse(menu, QEvent::MouseButtonPress, actionRect.center(), Qt::LeftButton,
                              Qt::LeftButton);
                    sendMouse(menu, QEvent::MouseButtonRelease, actionRect.center(), Qt::LeftButton,
                              Qt::NoButton);
                }
            }
            if (QApplication::activePopupWidget() == menu)
        ...
        sendMouse(page.canvas(), QEvent::MouseButtonPress, position, Qt::RightButton,
                  Qt::RightButton);
        sendMouse(page.canvas(), QEvent::MouseButtonRelease, position, Qt::RightButton,
                  Qt::NoButton);
    ...
    if (!tempoWasExpanded) {
        sendMouse(page.canvas(), QEvent::MouseButtonPress, tempoHeader, Qt::LeftButton,
                  Qt::LeftButton);
        sendMouse(page.canvas(), QEvent::MouseButtonRelease, tempoHeader, Qt::LeftButton,
                  Qt::NoButton);
        QCoreApplication::processEvents();
    }
