...
internal func runSongModelSuite(_ report: CheckReport) {
    ...
    report.expectEqual(raw, parsed.rawFlags, cppID: parseID,
                       what: "raw flags retain spelling, order, and duplicates")
    report.expectEqual("_final", parsed.voicegroupArgument, cppID: parseID,
                       what: "last voicegroup flag wins")
    report.expectEqual(80, parsed.masterVolume, cppID: parseID,
                       what: "case-insensitive volume overrides invalid volume")
    report.expectEqual(0, parsed.reverb, cppID: parseID,
                       what: "empty reverb argument clamps to zero")
    report.expectEqual(0, parsed.priority, cppID: parseID,
    ...
    report.expectEqual(0, SongFlags.fromRaw(["-Vabc"]).masterVolume, cppID: parseID,
    ...
    report.expectEqual("", SongFlags.fromRaw(["-V080"]).voicegroupArgument,
    ...
    report.expectEqual(stable, SongFlags.merge(SongFlags.fromRaw(stable)), cppID: mergeID,
                       what: "known flags round-trip in original order")
    let unknown = ["-Lfoo", "-E", "-Zother", "-R50", "-V080"]
    report.expectEqual(unknown, SongFlags.merge(SongFlags.fromRaw(unknown)), cppID: mergeID,
                       what: "unknown flags retain their positions on merge")
    report.expectEqual(["-V080"], SongFlags.merge(SongFlags.fromRaw(["-v080"])),
    ...
    report.expectEqual(["-V005", "-Lfoo"], SongFlags.merge(changed), cppID: mergeID,
                       what: "nil reverb, empty voicegroup and absent values remove flags; volume is padded")
    report.expectEqual(["-V005"], SongFlags.merge(SongConfig(voicegroupArgument: "", masterVolume: 5)),
