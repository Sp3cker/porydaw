...
void AutomationDomainTest::nodeDragAndPhantomOutcomes()
{
    ...
    {
        // Lower edge preserved: a leftward drag past the earliest sibling's
        // zero headroom clamps the common delta, so the group keeps its
        // spacing instead of collapsing onto tick zero.
        NodeDragGesture gesture;
        const DocLanePoint original0{kTrack, 7, 24, 60};
        const DocLanePoint original1{kTrack, 7, 48, 80};
        gesture.points = {laneNode(original0, {24, 60}), laneNode(original1, {48, 80})};
        gesture.grabbedPoint = 1;
        gesture.drag.press({100.0, 100.0}, false);
        gesture.drag.dragSlop.markExceeded({105.0, 100.0});
        gesture.update({PointDragUpdate::Phase::Dragging, {}, AxisLock::None}, {0, 80});
        QVERIFY(hasPoint(gesture.points[0].current, 0, 60));
        QVERIFY(hasPoint(gesture.points[1].current, 24, 80));
        const NodeDragFinish finish = gesture.finish();
        QCOMPARE(finish.release, PointDragRelease::Move);
        QVERIFY(finish.changed);
