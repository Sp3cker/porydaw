...
func drawerAutomationParameterCatalogAndMetadata(_ report: CheckReport) {
    ...
    report.expectEqual(expected: AutomationCatalog.count, actual: catalog.count, cppID: drawerAutomationCatalogID,
                       what: "the catalog carries every supported parameter plus Tempo")
    report.expectEqual(expected: [TimeDefaults.ccVolume, TimeDefaults.ccPan, TimeDefaults.ccModulation,
         ...
         TimeDefaults.ccFineTune, TimeDefaults.ccLFODelay], actual: AutomationCatalog.controllers, cppID: drawerAutomationCatalogID,
        what: "the supported parameters keep the production selector order")
    report.expectEqual(expected: AutomationParameter.tempo, actual: catalog.last, cppID: drawerAutomationCatalogID,
                       what: "Tempo closes the selector order")
    report.expectEqual(expected: 6, actual: AutomationCatalog.index(of: .controlChange(
    ...
    report.expectEqual(expected: Optional(0), actual: AutomationParameter.controlChange(track: 0,
    ...
    report.expectEqual(expected: Optional(.pitchBend), actual: AutomationParameter.pitchBend(track: 0).lane,
    ...
    report.expectEqual(expected: "Volume", actual: AutomationCatalog.tabLabel(
    ...
    report.expectEqual(expected: "Volume (VOL)", actual: AutomationCatalog.title(
        .controlChange(track: 0, controller: TimeDefaults.ccVolume)), cppID: drawerAutomationCatalogID,
                       what: "a classified parameter titles itself with its mnemonic")
    report.expectEqual(expected: "Echo volume (xIECV)", actual: AutomationCatalog.title(
    ...
    report.expectEqual(expected: "Pitch bend (BEND)", actual: AutomationCatalog.title(.pitchBend(track: 0)),
                       cppID: drawerAutomationCatalogID, what: "Pitch bend keeps its dedicated title")
    report.expectEqual(expected: "Tempo (BPM)", actual: AutomationCatalog.title(.tempo), cppID: drawerAutomationCatalogID,
    ...
    report.expectEqual(expected: 0, actual: volume.minimum, cppID: drawerAutomationCatalogID, what: "Volume's range starts at 0")
    report.expectEqual(expected: 127, actual: volume.maximum, cppID: drawerAutomationCatalogID, what: "Volume's range ends at 127")
    ...
    report.expectEqual(expected: "127", actual: volume.valueText(127), cppID: drawerAutomationCatalogID,
    ...
    report.expectEqual(expected: "c_v+0", actual: pan.valueText(64), cppID: drawerAutomationCatalogID,
                       what: "Pan formats its neutral as a centered c_v value")
    report.expectEqual(expected: "c_v-4", actual: pan.valueText(60), cppID: drawerAutomationCatalogID,
                       what: "Pan's formatting stays centered on 64")
    let bend = AutomationParameterMetadata(parameter: .pitchBend(track: 0))
    report.expectEqual(expected: -8192, actual: bend.minimum, cppID: drawerAutomationCatalogID, what: "Bend's range is signed")
    report.expectEqual(expected: 8191, actual: bend.maximum, cppID: drawerAutomationCatalogID, what: "Bend's range ends at 8191")
    report.expectEqual(expected: 0, actual: bend.neutral, cppID: drawerAutomationCatalogID, what: "Bend's neutral is zero")
    report.expectEqual(expected: "+8191", actual: bend.valueText(8191), cppID: drawerAutomationCatalogID,
                       what: "Bend formats its positive extreme")
    report.expectEqual(expected: "-8192", actual: bend.valueText(-8192), cppID: drawerAutomationCatalogID,
                       what: "Bend formats its negative extreme")
    let tempo = AutomationParameterMetadata(parameter: .tempo)
    report.expectEqual(expected: 20, actual: tempo.minimum, cppID: drawerAutomationCatalogID, what: "Tempo's range starts at 20 BPM")
    report.expectEqual(expected: 255, actual: tempo.maximum, cppID: drawerAutomationCatalogID, what: "Tempo's range ends at 255 BPM")
    ...
    report.expectEqual(expected: "150", actual: tempo.valueText(150), cppID: drawerAutomationCatalogID,
    ...
    report.expectEqual(expected: 2, actual: modType.maximum, cppID: drawerAutomationCatalogID,
                       what: "LFO type is bounded to its three modes")
    report.expectEqual(expected: 0, actual: AutomationCatalog.defaultRange(TimeDefaults.ccModulation), cppID: drawerAutomationCatalogID,
                       what: "Modulation's default value window starts at 0")
    report.expectEqual(expected: 127, actual: AutomationCatalog.defaultRange(TimeDefaults.ccPan), cppID: drawerAutomationCatalogID,
                       what: "every other zoomable parameter opens on the full window")
    report.expectEqual(expected: 16, actual: AutomationCatalog.autoRange(maximum: 9), cppID: drawerAutomationCatalogID,
                       what: "auto range fits the smallest window to the data")
    report.expectEqual(expected: 32, actual: AutomationCatalog.autoRange(maximum: 20), cppID: drawerAutomationCatalogID,
                       what: "auto range steps up with the data")
    report.expectEqual(expected: 64, actual: AutomationCatalog.autoRange(maximum: 60), cppID: drawerAutomationCatalogID,
                       what: "auto range keeps a wider window for wider data")
    report.expectEqual(expected: 127, actual: AutomationCatalog.autoRange(maximum: 127), cppID: drawerAutomationCatalogID,
                       what: "auto range keeps the full window for full data")

    let prompt = pan.prompt(storedValue: 64)
    report.expectEqual(expected: "Pan (PAN)", actual: prompt.title, cppID: drawerAutomationCatalogID,
                       what: "the value prompt titles the lane")
    report.expectEqual(expected: "c_v value (0 = center):", actual: prompt.label, cppID: drawerAutomationCatalogID,
                       what: "a centered parameter prompts in its displayed domain")
    report.expectEqual(expected: 0, actual: prompt.initialValue, cppID: drawerAutomationCatalogID,
                       what: "the displayed value offsets the stored value by the midpoint")
    report.expectEqual(expected: -64, actual: prompt.minimum, cppID: drawerAutomationCatalogID,
                       what: "the displayed domain is the stored domain less the offset")
    report.expectEqual(expected: 64, actual: pan.storedValue(prompted: 0), cppID: drawerAutomationCatalogID,
                       what: "the prompt's displayed value maps back to the stored value")
    report.expectEqual(expected: 127, actual: pan.storedValue(prompted: 200), cppID: drawerAutomationCatalogID,
                       what: "a stored value clamps into the parameter's domain")
    let bendPrompt = bend.prompt(storedValue: 0)
    report.expectEqual(expected: "Bend (0 = none):", actual: bendPrompt.label, cppID: drawerAutomationCatalogID,
                       what: "Bend's zero-centered prompt keeps its production label")
    let tempoPrompt = tempo.prompt(storedValue: 120)
    report.expectEqual(expected: "Set tempo", actual: tempoPrompt.title, cppID: drawerAutomationCatalogID,
                       what: "Tempo's prompt keeps its production title")
    report.expectEqual(expected: "BPM:", actual: tempoPrompt.label, cppID: drawerAutomationCatalogID,
                       what: "Tempo's prompt labels its unit")
    report.expectEqual(expected: 0, actual: tempoPrompt.storedOffset, cppID: drawerAutomationCatalogID,
...
func drawerAutomationLaneProjection(_ report: CheckReport, suite: DocumentSession,
    ...
    report.expectEqual(expected: 0, actual: modulation.points.count, cppID: drawerAutomationProjectionID,
                       what: "a lane the document never wrote projects no point")
    report.expectEqual(expected: 0, actual: modulation.segments.count, cppID: drawerAutomationProjectionID,
                       what: "an empty lane projects no curve segment")
    report.expectEqual(expected: 0, actual: modulation.leadIn?.value ?? -1, cppID: drawerAutomationProjectionID,
    ...
    report.expectEqual(expected: ["0:64", "24:90", "120:0"], actual: fixture.laneValues(pan.points.map {
    ...
    report.expectEqual(expected: ["24:30", "24:90", "120:0"], actual: pan.sources.map { "\($0.tick):\($0.value)" }, cppID: drawerAutomationProjectionID,
    ...
    report.expectEqual(expected: 90, actual: pan.heldValue(at: 30) ?? -1, cppID: drawerAutomationProjectionID,
                       what: "a tick after a point holds that point's value")
    report.expectEqual(expected: 90, actual: pan.heldValue(at: 24) ?? -1, cppID: drawerAutomationProjectionID,
                       what: "a tick on a point holds its own value")
    report.expectEqual(expected: 64, actual: pan.heldValue(at: 20) ?? -1, cppID: drawerAutomationProjectionID,
    ...
    report.expectEqual(expected: 90, actual: AutomationLaneReplacement.held(occurrences, at: 24, inclusive: true),
                       cppID: drawerAutomationProjectionID,
    ...
    report.expectEqual(expected: 90, actual: AutomationLaneReplacement.held(occurrences, at: 120, inclusive: false),
                       cppID: drawerAutomationProjectionID,
    ...
    report.expectEqual(expected: ["0:127", "96:64"], actual: fixture.laneValues(volume.points.map {
        AutomationLanePoint(tick: $0.tick, value: $0.value)
    ...
    report.expectEqual(expected: 2, actual: volume.eventCount, cppID: drawerAutomationProjectionID,
    ...
    report.expectEqual(expected: [Tick?.some(96), nil], actual: volume.segments.map(\.tickEnd), cppID: drawerAutomationProjectionID,
                       what: "the last step runs open to the song's end")
    report.expectEqual(expected: [127, 64], actual: volume.segments.map(\.fromValue), cppID: drawerAutomationProjectionID,
    ...
    report.expectEqual(expected: 127, actual: volume.heldValue(at: 95) ?? -1, cppID: drawerAutomationProjectionID,
                       what: "the step's value holds until the next point")
    report.expectEqual(expected: 64, actual: volume.heldValue(at: 96) ?? -1, cppID: drawerAutomationProjectionID,
    ...
    report.expectEqual(expected: 1, actual: projectedVolume.points.count, cppID: drawerAutomationProjectionID,
    ...
    report.expectEqual(expected: 0, actual: projectedVolume.eventCount, cppID: drawerAutomationProjectionID,
    ...
    report.expectEqual(expected: Tick(0), actual: scrolledProjection.originPhantom?.point.tick ?? 99,
                       cppID: drawerAutomationProjectionID,
    ...
    report.expectEqual(expected: 50, actual: AutomationInterpolation.ramp.value(
    ...
    report.expectEqual(expected: 0, actual: AutomationInterpolation.step.value(
        at: 5, from: AutomationLanePoint(tick: 0, value: 0),
        to: AutomationLanePoint(tick: 10, value: 100)), cppID: drawerAutomationProjectionID,
                       what: "the step interpolation holds its first point's value")
    report.expectEqual(expected: AutomationInterpolation.step, actual: rampMetadata.interpolation, cppID: drawerAutomationProjectionID,
    ...
    report.expectEqual(expected: ["0:120"], actual: fixture.laneValues(tempo.points.map {
        AutomationLanePoint(tick: $0.tick, value: $0.value)
    }), cppID: drawerAutomationProjectionID, what: "a tempo point projects at its tick as BPM")
    report.expectEqual(expected: fixture.y(.tempo, 120), actual: tempo.points.first?.y ?? -1, cppID: drawerAutomationProjectionID,
                       what: "Tempo uses the shared value axis")

    // Camera edges: every x comes from the shared camera, and a zoom rescales it.
    report.expectEqual(expected: fixture.session.camera.contentX(tick: 96), actual: volume.points[1].x, cppID: drawerAutomationProjectionID,
    ...
    report.expectEqual(expected: fixture.session.camera.contentX(tick: 0), actual: atZeroScroll, cppID: drawerAutomationProjectionID,
    ...
    report.expectEqual(expected: Tick(0), actual: preRoll.originPhantom?.point.tick ?? 99, cppID: drawerAutomationProjectionID,
                       what: "the off-plot point becomes the lane's origin phantom")
    _ = fixture.session.mutateCamera { $0.setHScroll(-1000) }
    report.expectEqual(expected: fixture.session.camera.minHScroll, actual: fixture.session.camera.snapshot.scrollX,
                       cppID: drawerAutomationProjectionID,
...
func drawerAutomationPointIdentityAndStaleness(_ report: CheckReport, suite: DocumentSession,
    ...
    let pan = fixture.projection(fixture.panLane)
    report.expectEqual(expected: 3, actual: pan.sources.count, cppID: drawerAutomationIdentityID,
                       what: "each occurrence exposes its own identity")
    report.expectEqual(expected: fixture.lanePoints(fixture.panLane).map(\.eventIndex), actual: pan.sources.map(\.identity.occurrence), cppID: drawerAutomationIdentityID,
    ...
    report.expectEqual(expected: ["24:30", "24:90", "48:20"], actual: rewritten.sources.map { "\($0.tick):\($0.value)" }, cppID: drawerAutomationIdentityID,
    ...
    report.expectEqual(expected: 1, actual: live?.writes.count ?? 0, cppID: drawerAutomationIdentityID,
    ...
    report.expectEqual(expected: ["0:120", "48:150"], actual: tempoFixture.laneValues(tempo.points.map {
        AutomationLanePoint(tick: $0.tick, value: $0.value)
...
func drawerAutomationDeleteTransactions(_ report: CheckReport, suite: DocumentSession,
    ...
    report.expectEqual(expected: ["120:40"], actual: fixture.values(fixture.panLane), cppID: drawerAutomationDeleteID,
                       what: "every occurrence at the deleted tick goes")
    report.expectEqual(expected: before.revision + 1, actual: fixture.document.revision, cppID: drawerAutomationDeleteID,
    ...
    report.expectEqual(expected: revision, actual: fixture.document.revision, cppID: drawerAutomationDeleteID,
    ...
    report.expectEqual(expected: projectedRevision, actual: projectionFixture.document.revision, cppID: drawerAutomationDeleteID,
    ...
    report.expectEqual(expected: ["120:40"], actual: selection.values(selection.panLane), cppID: drawerAutomationDeleteID,
                       what: "the covered lane keeps only its outside point")
    report.expectEqual(expected: ["140:20"], actual: selection.values(selection.volumeLane), cppID: drawerAutomationDeleteID,
                       what: "the second covered lane keeps only its outside point")
    report.expectEqual(expected: ["0:120", "150:150"], actual: selection.tempoValues, cppID: drawerAutomationDeleteID,
                       what: "Tempo stays untouched while the selection excludes it")
    report.expectEqual(expected: selectionBefore.revision + 1, actual: selection.document.revision, cppID: drawerAutomationDeleteID,
    ...
    report.expectEqual(expected: ["24:30", "120:40"], actual: selection.values(selection.panLane), cppID: drawerAutomationDeleteID,
                       what: "one undo restores the first lane")
    report.expectEqual(expected: ["48:100", "140:20"], actual: selection.values(selection.volumeLane), cppID: drawerAutomationDeleteID,
