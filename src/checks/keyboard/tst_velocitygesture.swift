...
func runVelocityGestureParityChecks(_ report: CheckReport) {
    ...
    report.expectEqual(Optional(UInt8(40)), lifecycle?.frozenNote(liveFirst)?.velocity, cppID: lifecycleID, what: "A015 first frozen target holds its begin value")
    report.expectEqual(Optional(UInt8(120)), lifecycle?.frozenNote(liveSecond)?.velocity, cppID: lifecycleID, what: "A016 second frozen target holds its begin value")
    ...
    report.expectEqual(UInt8(120), atomicDocument.note(atomicSecond)?.velocity, cppID: atomicityID, what: "A022 rejected commit keeps the second value")
    ...
    report.expectEqual(UInt8(120), atomicDocument.note(atomicSecond)?.velocity, cppID: atomicityID, what: "A026 duplicated commit keeps the second value")
    ...
    report.expectEqual(Optional(UInt8(100)), completion!.preview[finishingFirst], cppID: completionID, what: "A029 first preview takes the absolute value")
    ...
    report.expectEqual(Optional(UInt8(30)), completion!.preview[finishingFirst], cppID: completionID, what: "A032 first preview measures the delta from its origin")
    report.expectEqual(Optional(UInt8(110)), completion!.preview[finishingSecond], cppID: completionID, what: "A033 second preview measures the delta from its origin")
    ...
    report.expectEqual(finishingRevision, completion!.revision, cppID: completionID, what: "A035 completion carries the begin revision")
    report.expectEqual(2, payload.count, cppID: completionID, what: "A036 completion carries both targets")
    report.expectEqual(finishingFirst, payload.first?.noteID ?? NoteID(), cppID: completionID, what: "A037 completion orders the first target by note")
    report.expectEqual(30, payload.first?.velocity ?? -1, cppID: completionID, what: "A038 completion carries the first delta value")
    report.expectEqual(finishingSecond, payload.last?.noteID ?? NoteID(), cppID: completionID, what: "A039 completion orders the second target by note")
    report.expectEqual(110, payload.last?.velocity ?? -1, cppID: completionID, what: "A040 completion carries the second delta value")
    ...
    report.expectEqual(UInt8(1), clampedDocument.note(clampedFirst)?.velocity, cppID: clampedID, what: "A046 committed floor value reads one")
    ...
    report.expectEqual(UInt8(127), clampedDocument.note(clampedFirst)?.velocity, cppID: clampedID, what: "A048 committed ceiling value reads 127")
    ...
    report.expectEqual(Optional(UInt8(1)), cancellation!.preview[clampedFirst], cppID: clampedID, what: "A050 clamped delta floors the first preview")
    report.expectEqual(Optional(UInt8(70)), cancellation!.preview[clampedSecond], cppID: clampedID, what: "A051 clamped delta moves the second preview from its origin")
