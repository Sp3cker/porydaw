...
private func eventViewsChannelAndDataConversions(_ report: CheckReport) {
    ...
    report.expectEqual(expected: before + 1, actual: document.history.undoIndex, cppID: id,
                       what: "channel edit pushes one undo step")
    report.expectEqual(expected: UInt8(0x04), actual: document.rawChunks[0].events[noteIndex].status & 0x0F,
                       cppID: id, what: "one-based channel value converts to the status nibble")
    report.expectEqual(expected: Optional(noteID), actual: document.rawChunks[0].events[noteIndex].noteID,
    ...
    report.expectEqual(expected: before + 3, actual: document.history.undoIndex, cppID: id,
    ...
    report.expectEqual(expected: UInt8(100), actual: data0, cppID: id,
                       what: "first data byte stores the edited value")
    report.expectEqual(expected: UInt8(33), actual: data1, cppID: id,
    ...
    report.expectEqual(expected: before + 4, actual: document.history.undoIndex, cppID: id,
                       what: "blob edit pushes one undo step")
    report.expectEqual(expected: Optional(Array("room".utf8)), actual: document.rawChunks[0].events[blobIndex].blob,
                       cppID: id, what: "blob edit stores the quoted payload bytes")
    while document.history.undoIndex > before {
        _ = document.history.undoDocument()
    }
    report.expectEqual(expected: UInt8(0x00), actual: document.rawChunks[0].events[noteIndex].status & 0x0F,
    ...
    report.expectEqual(expected: UInt8(60), actual: restored0, cppID: id,
                       what: "undo restores the original first data byte")
    report.expectEqual(expected: UInt8(90), actual: restored1, cppID: id,
                       what: "undo restores the original second data byte")
    report.expectEqual(expected: Optional(noteID), actual: document.rawChunks[0].events[noteIndex].noteID,
                       cppID: id, what: "undo restores the note identity")
    report.expectEqual(expected: Optional(Array("marker".utf8)),
                       actual: document.rawChunks[0].events[blobIndex].blob,
...
private func eventViewsRawTempoAtomic(_ report: CheckReport) {
    ...
    report.expectEqual(expected: 1, actual: publications, cppID: id,
                       what: "meta-to-tempo conversion publishes once")
    report.expectEqual(expected: initialIndex + 1, actual: document.history.undoIndex, cppID: id,
                       what: "meta-to-tempo conversion pushes one undo step")
    report.expectEqual(expected: initialCount + 1, actual: document.history.undoCount, cppID: id,
    ...
    report.expectEqual(expected: tickZeroMeta,
                       actual: document.rawChunks[0].events.filter { $0.tick == 0 && $0.isMeta },
    ...
    report.expectEqual(expected: 1, actual: publications, cppID: id, what: "conversion undo publishes once")
    report.expectEqual(expected: initialIndex, actual: document.history.undoIndex, cppID: id,
                       what: "conversion undo restores the index")
    report.expectEqual(expected: initialCount + 1, actual: document.history.undoCount, cppID: id,
                       what: "conversion undo retains the entry")
    report.expectEqual(expected: Optional(Array("eventviews conversion".utf8)),
                       actual: document.rawChunks[0].events.first(where: {
                           $0.tick == conversionTick && $0.isMeta
                       }).flatMap(\.blob),
    ...
    report.expectEqual(expected: 1, actual: publications, cppID: id, what: "conversion redo publishes once")
    report.expectEqual(expected: initialIndex + 1, actual: document.history.undoIndex, cppID: id,
    ...
    report.expectEqual(expected: initialIndex + 2, actual: document.history.undoIndex, cppID: id,
                       what: "tempo-to-meta conversion pushes a second undo step")
    report.expectEqual(expected: initialCount + 2, actual: document.history.undoCount, cppID: id,
    ...
    report.expectEqual(expected: 1, actual: publications, cppID: id, what: "reverse-conversion undo publishes once")
    report.expectEqual(expected: initialIndex + 1, actual: document.history.undoIndex, cppID: id,
    ...
    report.expectEqual(expected: 1, actual: publications, cppID: id, what: "reverse-conversion redo publishes once")
    report.expectEqual(expected: initialIndex + 2, actual: document.history.undoIndex, cppID: id,
...
private func eventViewsSameTickReorder(_ report: CheckReport) {
    ...
    report.expectEqual(expected: first + 1, actual: document.rawChunks[0].events.firstIndex(of: ccB), cppID: id,
                       what: "second controller follows first")
    report.expectEqual(expected: first + 2, actual: document.rawChunks[0].events.firstIndex(of: noteOn),
    ...
    report.expectEqual(expected: first, actual: document.rawChunks[0].events.firstIndex(of: ccB), cppID: id,
                       what: "legal in-run swap moves the second controller first")
    report.expectEqual(expected: second, actual: document.rawChunks[0].events.firstIndex(of: ccA), cppID: id,
                       what: "legal in-run swap moves the first controller second")
    report.expectEqual(expected: swapBase + 1, actual: document.history.undoIndex, cppID: id,
    ...
    report.expectEqual(expected: orderAfterDrop, actual: document.history.undoIndex, cppID: id,
                       what: "past-run destination pushes no undo step")
    document.moveRawEvent(chunk: 0, index: note, to: 0)
    report.expectEqual(expected: note, actual: document.rawChunks[0].events.firstIndex(of: noteOn), cppID: id,
    ...
    report.expectEqual(expected: offIndex + 1, actual: document.rawChunks[0].events.firstIndex(of: pinnedOn),
    ...
    report.expectEqual(expected: offIndex, actual: document.rawChunks[0].events.firstIndex(of: pinnedOff),
                       cppID: id, what: "pinned note-end stays ahead of its note-on")
    report.expectEqual(expected: onIndex, actual: document.rawChunks[0].events.firstIndex(of: pinnedOn), cppID: id,
                       what: "pinned note-on stays after its note-end")
    report.expectEqual(expected: orderBeforePin, actual: document.history.undoIndex, cppID: id,
                       what: "pinned reorder pushes no undo step")
    let undoBeforeMenuMove = document.history.undoIndex
    document.moveRawEvent(chunk: 0, index: first, to: second)
    report.expectEqual(expected: first, actual: document.rawChunks[0].events.firstIndex(of: ccA), cppID: id,
                       what: "row move down restores the first controller")
    report.expectEqual(expected: second, actual: document.rawChunks[0].events.firstIndex(of: ccB), cppID: id,
                       what: "row move down advances the second controller")
    report.expectEqual(expected: note, actual: document.rawChunks[0].events.firstIndex(of: noteOn), cppID: id,
                       what: "row move down keeps the note in place")
    report.expectEqual(expected: undoBeforeMenuMove + 1, actual: document.history.undoIndex, cppID: id,
    ...
    report.expectEqual(expected: undoBeforeKeyMove + 1, actual: document.history.undoIndex, cppID: id,
    ...
    report.expectEqual(expected: orderBeforeBoundaryMove, actual: document.history.undoIndex, cppID: id,
