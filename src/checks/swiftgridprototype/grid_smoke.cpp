...
bool separatorNearRow(int row, const PixelAtRow &pixelAtRow)
{
    static const QColor separatorColor(QStringLiteral("#BCB4AF"));
    return pixelAtRow(row - 1) == separatorColor || pixelAtRow(row) == separatorColor ||
           pixelAtRow(row + 1) == separatorColor;
}
...
void verifyChromeRaster(Scene &scene)
{
    ...
    require(separatorNearRow(sepRow,
                             [&](int row) {
                                 return windowPixel(
                                     image, QPointF(rollOrigin.x() + keyboardWidth / 2,
    ...
    require(separatorNearRow(gridSepRow,
                             [&](int row) {
                                 return pixelAt(image,
                                                {barX + 24, double(row) / image.devicePixelRatio() -
                                                                surfaceOrigin.y()});
                             }),
...
void exercise(Scene scene)
{
    ...
    verifyDocumentFeed(scene.window, scene.viewport, scene.surface, scene.model);
