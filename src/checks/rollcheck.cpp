...
// modifier velocity edit switches to its note instead of accumulating it.
// Ctrl+arrows transpose (Shift: octave) and nudge the selection along the
// same absolute grid — both the roll's note selection and a multi-track
// time selection — and the view follows
                 ...
                 const QString &screenshotPath)
{
    ...
    // Camera/grid state and typed drawer cosmetics are detached snapshots.
    {
        ...
        const double maxHScroll =
            std::max(0.0, double(timeline->lengthTicks) * expected.pxPerBeat / tpb + 100.0 -
        ...
        if (!remapDoc.load(*info, &remapError)) {
            ...
            const auto hasTimeSelectionLanes =
                [&](const std::vector<std::pair<int, uint8_t>> &expected) {
                    const songview::TimeSelection &selection =
            ...
            const auto hasMovedTrackState = [&] {
                return remapView.selectionModel().primaryTrack() == 0 &&
                       remapView.selectionModel().resolvedTrackScope(
                           usedTrackMask(remapView.timeline())) == 0x3 &&
            ...
            if (inserted < 0) {
                ...
                if (remapView.trackMuted(inserted) || remapView.trackSoloed(inserted) ||
                    (remapView.selectionModel().resolvedTrackScope(
                         usedTrackMask(remapView.timeline())) &
                     (1u << inserted)) ||
                    !hasMovedTrackState() ||
            ...
            if (duplicate < 0) {
                ...
                if (remapView.selectionModel().primaryTrack() != fallback ||
                    remapView.selectionModel().resolvedTrackScope(
                        usedTrackMask(remapView.timeline())) != (1u << fallback) ||
                    remapView.trackMuted(0) || !remapView.trackMuted(1) ||
                    remapView.trackSoloed(1) || !remapView.trackSoloed(0) ||
                    !hasRemappedCosmetics(remapView.editorViewState(), 1, 0)) {
                ...
                if (remapView.selectionModel().primaryTrack() != duplicate - 1 ||
                    remapView.selectionModel().resolvedTrackScope(
                        usedTrackMask(remapView.timeline())) != (1u << (duplicate - 1)) ||
                    remapView.trackMuted(0) || !remapView.trackMuted(1) ||
                    remapView.trackSoloed(1) || !remapView.trackSoloed(0) ||
                    !hasRemappedCosmetics(remapView.editorViewState(), 1, 0)) {
            ...
            const uint32_t metadataHeaderMask =
                remapView.selectionModel().resolvedTrackScope(usedTrackMask(remapView.timeline()));
            ...
            const auto metadataStateUnchanged = [&] {
                return remapView.selectionModel().primaryTrack() == metadataSelected &&
                       remapView.selectionModel().resolvedTrackScope(
                           usedTrackMask(remapView.timeline())) == metadataHeaderMask &&
            ...
            if (!rawDir.isValid() || !rawSmf.writeFile(rawInfo.midPath, &rawError)) {
                ...
                if (!rawDoc.load(rawInfo, &rawError)) {
                    ...
                    if (rawView.selectionModel().primaryTrack() != 1 ||
                        rawView.selectionModel().resolvedTrackScope(
                            usedTrackMask(rawView.timeline())) != 0x2 ||
                    ...
                    if (rawView.selectionModel().primaryTrack() != 0 ||
                        rawView.selectionModel().resolvedTrackScope(
                            usedTrackMask(rawView.timeline())) != 0x1 ||
    ...
    // through the integer-native scrollbar projection.
    {
        ...
        sendWheel(roll, QPointF(pianoKeyboardWidth - 1.0, anchor.y()), 0, 1, Qt::NoModifier);
        ...
        sendMouse(roll, QEvent::MouseMove, QPointF(pianoKeyboardWidth + 40.0, boundary - 0.25),
    ...
    // the widget clears the mark.
    {
        ...
        sendMouse(roll, QEvent::MouseMove, QPoint(pianoKeyboardWidth + 40, y), Qt::NoButton,
                  Qt::NoButton);
    ...
    auto findFreeCell = [&](int firstProbe = 8, bool checkAllTracks = false) -> Cell {
        ...
        for (int key = 115; key >= 24; key--) {
            ...
            if (top < 0 || bottom > roll->height())
                continue;
            for (int probe = firstProbe; probe < roll->width() - pianoKeyboardWidth - 40;
                 probe += 24) {
                ...
                const int x1 = pianoKeyboardWidth + view.contentX(double(tick + dur));
                const int xs =
    ...
    failures += runPitchBendCheck(doc, view, roll, track, noteA, a.center, songLabel);
    ...
    const auto checkVelocityCancellation = [&](const char *fixtureFailure, const char *stageFailure,
        ...
        sendMouse(roll, QEvent::MouseButtonPress, cancelHandle, Qt::LeftButton, Qt::LeftButton);
        sendMouse(roll, QEvent::MouseMove, cancelHandle - QPoint(0, 8), Qt::NoButton,
                  Qt::LeftButton);
        QCoreApplication::processEvents();
    ...
    checkVelocityCancellation(
        ...
        "SongView cancellation must reset piano-roll local drag state without mutation", [&] {
            ...
            sendMouse(roll, QEvent::MouseMove, cancelHandle - QPoint(0, 8), Qt::NoButton,
                      Qt::LeftButton);
            sendMouse(roll, QEvent::MouseButtonRelease, cancelHandle - QPoint(0, 8), Qt::LeftButton,
                      Qt::NoButton);
            QCoreApplication::processEvents();
        });
    ...
    // stationary Ctrl+edge click just joins, editing nothing.
    {
        ...
        const qreal cellPx = view.displayX(double(d.tick + 3 * d.dur), pianoKeyboardWidth, dpr) -
                             view.displayX(double(d.tick + 2 * d.dur), pianoKeyboardWidth, dpr);
    ...
    const QPoint edge2(pianoKeyboardWidth + view.contentX(double(d.tick + 2 * d.dur)), rowY);
    const QPoint overshoot(pianoKeyboardWidth + view.contentX(double(d.tick) - 0.5 * double(d.dur)),
                           rowY);
    ...
    // same border its wide neighbors have instead of shedding it.
    {
        ...
        const int narrowLeftX = pianoKeyboardWidth + view.contentX(double(d.tick));
        const int narrowRightX = pianoKeyboardWidth + view.contentX(double(d.tick + snapCell));
    ...
    {
        const songview::TimeSelection duplicateSource = view.selectionModel().timeSelection();
        ...
        const songview::TimeSelection firstSelection = view.selectionModel().timeSelection();
        ...
        const songview::TimeSelection secondSelection = view.selectionModel().timeSelection();
    ...
    if (doc.engineTrackCount() >= 2) {
        ...
        if (otherKey >= 128) {
            ...
            if (!doc.findNote(otherTrack, insertStart, otherPitch, &otherBefore)) {
                ...
                const songview::TimeSelection insertedSelection =
    ...
    // same track stay fixed, proving the resolver does not widen lane scope.
    {
        ...
        if (!doc.findLanePoint(track, laneCc, lanePointTick, &laneBefore) ||
            ...
            const songview::TimeSelection insertedSelection = view.selectionModel().timeSelection();
    ...
    sendMouse(roll, QEvent::MouseMove, QPoint(pianoKeyboardWidth + 60, roll->height() / 3),
              Qt::NoButton, Qt::NoButton);
    ...
    // Twenty-one commands: draw, set, draw, nudge, draw, the double-click
    ...
    if (undos != 21 + (reordered ? (dragRenamed ? 2 : 1) : 0))
    ...
    // Fold occupancy and editing follow the Primary Track, and the earlier
