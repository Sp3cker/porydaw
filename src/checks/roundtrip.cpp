...
int runRoundTrip(const QString &projectRoot, const QString &mid2agbArg)
{
    ...
    for (const SongInfo &song : project.songs()) {
        ...
        // SmfFile::read coerces format 0 to format 1 (the same parse layer
        // SongDocument::load goes through), so a real save writes the
        // converted file — the .s diff below therefore also proves the
        // conversion is invisible to mid2agb.
