...
bool VelocityGestureModel::update(const std::vector<NoteVelocity> &updates)
{
    ...
    for (const NoteVelocity &update : updates) {
        ...
        if (targetIndex == m_session->targets.size() || m_session->updateMarks[targetIndex])
            return false;
        m_session->updateMarks[targetIndex] = 1;
    }
...
size_t VelocityGestureModel::targetIndex(NoteId noteId) const
{
    ...
    return target == m_session->targets.end() || target->noteId != noteId
               ...
               : static_cast<size_t>(target - m_session->targets.begin());
}
...
std::optional<uint8_t> VelocityGestureModel::previewVelocity(NoteId noteId) const
{
    ...
    const auto targetIndex = this->targetIndex(noteId);
    if (targetIndex == m_session->targets.size())
        return std::nullopt;
    return uint8_t(m_session->targets[targetIndex].velocity);
}
