...
Sf2ZonePicker::Sf2ZonePicker(const Sf2File &file, QWidget *parent) : QDialog(parent), m_file(file)
{
    ...
    m_search->setPlaceholderText(tr("Search samples, instruments, presets…"));
    ...
    m_tree->setHeaderLabels(
        {tr("Sample"), tr("Key"), tr("Rate"), tr("Frames"), tr("Loop"), tr("Notes")});
    ...
    connect(m_tree, &QTreeWidget::currentItemChanged, this, [this] { updateSelection(); });
...
void Sf2ZonePicker::rebuild()
{
    ...
    for (int i = 0; i < int(m_file.zones.size()); i++) {
        ...
        if (!needle.isEmpty() && !zone.name.contains(needle, Qt::CaseInsensitive) &&
            !zone.instrument.contains(needle, Qt::CaseInsensitive) &&
            !zone.preset.contains(needle, Qt::CaseInsensitive))
            continue;
        QString label = zone.instrument.isEmpty() ? tr("(no instrument)") : zone.instrument;
...
void Sf2ZonePicker::updateSelection()
{
    ...
    const QVariant zone = current ? current->data(0, Qt::UserRole) : QVariant();
