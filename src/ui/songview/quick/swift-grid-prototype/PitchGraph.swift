...
public final class PitchGraph {
    ...
    @QtIgnored
    var points: [Int: Int] {
        Dictionary(uniqueKeysWithValues: vertices.map { (Int($0.tick), Int($0.value)) })
    }
    ...
    private func refresh() {
        ...
        if vertices.count < count {
            vertices.append(
                contentsOf: repeatElement(SGCurveVertex(), count: count - vertices.count))
        } else if vertices.count > count {
            vertices.removeLast(vertices.count - count)
        }
    ...
    private func redraw() {
        ...
        for (index, vertex) in vertices.enumerated() {
            ...
            if let next, next.tick - vertex.tick == 1 {
