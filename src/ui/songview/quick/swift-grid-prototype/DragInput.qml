...
Item {
    ...
    implicitWidth: Math.max(fontMetrics.advanceWidth(String(minimumValue)), fontMetrics.advanceWidth(String(maximumValue))) + 2 * (appearance.horizontalPadding + appearance.borderWidth)
    ...
    QtObject {
        ...
        function scrubTo(distance) {
            ...
            if (!dragging) {
                if (Math.abs(distance) <= control.appearance.dragThreshold)
                    return;
                dragging = true;
                lastDragDistance = distance > 0 ? control.appearance.dragThreshold : -control.appearance.dragThreshold;
            }
            const rate = (scrubDrag.centroid.modifiers & Qt.ShiftModifier) ? shiftStepsPerPixel : normalStepsPerPixel;
        ...
        function finishEditing() {
            ...
            const acceptable = trimmed !== "" && !Number.isNaN(parsed) && parsed >= control.minimumValue && parsed <= control.maximumValue;
    ...
    FontMetrics {
        ...
        font: Qt.font(control.appearance.font)
    }
    ...
    TextInput {
        ...
        HoverHandler {
            ...
            onHoveredChanged: control.hintChanged(hovered ? qsTr("Drag vertically: adjust · Shift-drag: fine adjustment · Ctrl-wheel: 10 steps") : "")
        }
        ...
        WheelHandler {
            ...
            onWheel: event => {
                const useHorizontal = Qt.platform.os === "osx" && (event.modifiers & Qt.ShiftModifier);
