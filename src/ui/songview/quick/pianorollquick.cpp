    ...
    , m_songView(&songView)
{
    ...
    static constexpr std::array<std::pair<TimelineQuickLayer, const char *>, 11> layers = {{
...
{
    if (!m_ruler || !m_roll || !m_otherEvents || !m_songView)
        return;

    const auto bandRectInSongView = [songView = m_songView.data()](const QWidget &band) {
        return QRect{band.mapTo(songView, QPoint{}), band.size()};
    };
    ...
    const QRect hostRect = rulerBandRect.united(rollBandRect).united(otherEventsBandRect);
    ...
    if (isVisible() != (rulerVisible || rollVisible || otherEventsVisible))
        setVisible(rulerVisible || rollVisible || otherEventsVisible);
