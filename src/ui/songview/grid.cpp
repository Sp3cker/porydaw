...
{
    return m_timeAxis.segmentAt(tick);
}
...
{
    return gridTicksIn(gridSegAt(tick), pxPerTick());
}
...
{
    return gridTicksIn(gridSegAt(tick), pxPerTick(), /*snap=*/true);
}
...
uint64_t SongView::snapTick(double tick, bool fine) const
{
    ...
    const uint64_t g = gridTicksIn(seg, pxPerTick(), /*snap=*/true);
