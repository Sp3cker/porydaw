...
int clampAutomationRowHeight(int height)
{
    ...
    return std::clamp(height, geometry.automationRowMinimumHeight,
                      geometry.automationRowMaximumHeight);
}
...
bool load(const QString &projectRoot, const QString &songLabel, Snapshot *snapshot)
{
    ...
    const QJsonObject editor = root.value(QLatin1String(kEditorKey)).toObject();
    const auto laneValue = [&editor, &view](const char *key) {
        const QLatin1String name(key);
        return editor.contains(name) ? editor.value(name) : view.value(name);
    };
    if (const auto value =
            decodeInteger(laneValue(kLaneHeightKey), 0, std::numeric_limits<int>::max())) {
        loaded.editor.laneHeight = *value == 0 ? 0 : clampAutomationRowHeight(*value);
    }
    decodeRowHeights(laneValue(kLaneHeightsKey), &loaded);
    decodeRowRanges(laneValue(kLaneRangesKey), &loaded);
    decodeEmptyLanes(laneValue(kEmptyLanesKey), &loaded);
    decodeHiddenLanes(laneValue(kHiddenLanesKey), &loaded.editor);
