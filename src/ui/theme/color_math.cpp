...
LinearRgb toLinearRgb(const Oklab &lab)
{
    ...
    return {4.0767416621 * lCubed - 3.3077115913 * mCubed + 0.2309699292 * sCubed,
...
{
    return QColor::fromRgb(gammaChannel(rgb.red), gammaChannel(rgb.green), gammaChannel(rgb.blue),
                           alpha);
}
...
{
    return {gammaChannel(clamp(rgb.red, 0.0, 1.0)), gammaChannel(clamp(rgb.green, 0.0, 1.0)),
...
{
    return finite(value) && value >= -kGamutEpsilon && value <= 1.0 + kGamutEpsilon;
}
...
bool sampleSrgb(const Oklab &color, SrgbSample &sample)
{
    ...
    if (!inUnitInterval(rgb.red) || !inUnitInterval(rgb.green) || !inUnitInterval(rgb.blue))
        return false;
...
bool sampleSrgb(const Oklch &color, SrgbSample &sample)
{
    if (!finite(color.lightness) || !finite(color.chroma) || !finite(color.hue) ||
        color.lightness < 0.0 || color.lightness > 1.0 || color.chroma < 0.0 || color.hue < 0.0 ||
        color.hue >= 360.0)
        return false;
    const auto radians = color.hue * kPi / 180.0;
    return sampleSrgb(
        Oklab{color.lightness, color.chroma * std::cos(radians), color.chroma * std::sin(radians)},
        sample);
}
...
{
    return 0.2126 * linearChannel(sample.red) + 0.7152 * linearChannel(sample.green) +
...
{
    return contrastRatioFromLuminance(relativeLuminance(first), relativeLuminance(second));
}
