    ...
    , m_draft{}
{
    ...
    auto *accentCaption =
        new QLabel(tr("Accent colors focus, selection, and edit emphasis."), this);
    ...
    for (int y = 0; y < maskPattern.height(); y += 4) {
        for (int x = 0; x < maskPattern.width(); x += 4) {
            ...
            maskPatternPainter.fillRect(x, y, 4, 4,
                                        dark ? QColor(64, 64, 64) : QColor(232, 232, 232));
        }
    }
    maskSample->setPixmap(maskPattern);
    auto *pickerLabel =
        new QLabel(tr("Patterned colors cannot be selected. They are outside sRGB or have "
    ...
    const auto groupTop = QFontMetrics(m_customEditorGroup->font()).lineSpacing() +
                          ::layout::space(::layout::Space::One);
    customLayout->setContentsMargins(groupMargin, groupTop, groupMargin, groupMargin);
...
QColor ThemeDialog::parseField(const QLineEdit *field) const
{
    ...
    if (!parsed.isValid() || parsed.name(QColor::HexRgb).compare(text, Qt::CaseInsensitive) != 0)
        return {};
    return parsed;
}
...
void ThemeDialog::updatePicker()
{
    ...
    const bool selectedSet = targetPrimary ? m_draft.primary.isValid() : m_draft.accent.isValid();
    const bool otherSet = targetPrimary ? m_draft.accent.isValid() : m_draft.primary.isValid();
    const QColor selected =
        selectedSet ? (targetPrimary ? m_draft.primary : m_draft.accent) : QColor(128, 128, 128);
    const QColor other = otherSet ? (targetPrimary ? m_draft.accent : m_draft.primary) : QColor();
...
void ThemeDialog::primaryHexEdited()
{
    if (m_ignoreFieldSignals)
        return;
    readDraftFromFields();
    if (m_draft.primary.isValid() && m_primaryHexEdit->text() != canonicalHex(m_draft.primary))
...
void ThemeDialog::accentHexEdited()
{
    if (m_ignoreFieldSignals)
        return;
    readDraftFromFields();
    if (m_draft.accent.isValid() && m_accentHexEdit->text() != canonicalHex(m_draft.accent))
