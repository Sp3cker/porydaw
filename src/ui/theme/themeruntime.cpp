...
void applyPaletteGroup(QPalette &palette, QPalette::ColorGroup group, const Theme &theme)
{
    const auto text = group == QPalette::Disabled ? Role::disabled_text : Role::window_text;
    const auto buttonText = group == QPalette::Disabled ? Role::disabled_text : Role::button_text;
...
{
    return QStringLiteral("QMainWindow,QDialog,QWizard,QWizardPage,QDockWidget,QScrollArea,"
                          "QStackedWidget{"
...
{
    return QStringLiteral("QPushButton,QToolButton{background-color:%1;color:%2;"
                          "border-color:%3;}"
...
{
    return QStringLiteral("QToolButton#trackSoloButton:pressed{background-color:%1;"
...
{
    return QStringLiteral("QLineEdit,QTextEdit,QPlainTextEdit{background-color:%1;"
...
void paintGlyph(QPainter &painter, Glyph glyph, const QColor &color, int width, int height)
{
    ...
    case Glyph::SpinDown: {
        ...
        painter.drawPolygon(
            glyph == Glyph::SpinUp
                ? QPolygonF{{0.0, qreal(height)}, {qreal(width), qreal(height)}, {peakX, 0.0}}
                : QPolygonF{{0.0, 0.0}, {qreal(width), 0.0}, {peakX, qreal(height)}});
        return;
    }
...
QString glyphImagePath(Glyph glyph, const QColor &color, int devicePixelRatio)
{
    ...
    const auto path =
        QStringLiteral("%1/%2-%3%4.png")
            .arg(directory, glyphName(glyph), color.name(QColor::HexRgb).mid(1),
                 devicePixelRatio > 1 ? QStringLiteral("@%1x").arg(devicePixelRatio) : QString());
...
{
    const auto check = glyphImage(Glyph::CheckMark, theme.color(Role::indicator_check_mark));
    if (check.isEmpty())
        return {};
...
QString spinBoxStyleSheet(const Theme &theme)
{
    return spinArrowStyleSheet(theme) +
           QStringLiteral("QAbstractSpinBox{background-color:%1;color:%2;"
                          "border-color:%3;}"
...
{
    return QStringLiteral("QComboBox{background-color:%1;color:%2;border-color:%3;}"
...
{
    return QStringLiteral("QFrame#vgSamplePickerPopup{background-color:%1;"
                          "border-color:%2;}")
        .arg(colorName(theme, Role::window_background))
        .arg(colorName(theme, Role::menu_outline));
}
...
{
    return QStringLiteral("QCheckBox,QRadioButton{background-color:transparent;color:%1;"
...
{
    return QStringLiteral("QMenuBar{background-color:%1;color:%2;border-color:%3;}"
...
{
    return QStringLiteral("QMenu{background-color:%1;color:%2;border-color:%3;}"
...
{
    return QStringLiteral("QToolTip{background-color:%1;color:%2;border-color:%3;}")
...
{
    return QStringLiteral("QAbstractScrollArea{background-color:%1;color:%2;"
                          "border-color:%3;}"
...
{
    return QStringLiteral("QHeaderView{background-color:%1;color:%2;border-color:%3;}"
...
{
    return QStringLiteral("QScrollBar{background-color:%1;border-color:%2;}"
...
{
    return QStringLiteral("QSplitter{background-color:%1;}"
