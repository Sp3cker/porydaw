...
namespace {
constexpr int kAuditionKey =
    60; // middle C; drumkits play that key's percussion
...
bool macroIsDrumkit(VgMacro m) { return m == VgMacro::KeysplitAll; }
...
VoicegroupBrowser::VoicegroupBrowser(QWidget *parent) : QWidget(parent) {
  ...
  connect(m_vgCombo->lineEdit(), &QLineEdit::editingFinished, this,
          commitVgArg);
  ...
  connect(m_tree, &QTreeWidget::itemPressed, this,
          &VoicegroupBrowser::pressedVoice);
  ...
  const auto addRow = [form](const QString &text, QWidget *field,
                             QLabel **labelOut) {
  ...
  connect(m_samplePicker, &SamplePickerButton::symbolPicked, this,
          [this] { commitEdit(); });
  ...
  buttonLayout->setContentsMargins(0, ::layout::space(::layout::Space::Half), 0,
                                   0);
  ...
  for (QWidget *w : std::initializer_list<QWidget *>{
           m_tree, m_vgCombo, m_typeCombo, m_symbolCombo, m_dutyCombo,
           m_periodCombo, m_synthWaveCombo, m_synthDutySpin, m_synthStepSpin,
           m_synthDepthSpin, m_synthPhaseSpin, m_sweepTimeSpin, m_sweepDirCombo,
           m_sweepShiftSpin, m_attackSpin, m_decaySpin, m_sustainSpin,
           m_releaseSpin}) {
  ...
  for (QSpinBox *spin :
       {m_sweepTimeSpin, m_sweepShiftSpin, m_synthDutySpin, m_synthStepSpin,
        m_synthDepthSpin, m_synthPhaseSpin, m_attackSpin, m_decaySpin,
...
void VoicegroupBrowser::setVoicegroup(const LoadedVoiceGroup *vg) {
  ...
  for (int i = 0; i < VOICEGROUP_SIZE; i++) {
    ...
    const QString type = toneIsSynth(voice) ? tr("Synth (Golden Sun)")
...
void VoicegroupBrowser::setSource(
    VoicegroupSource *source, const QStringList &sampleSymbols,
  ...
  for (auto it = pendingSynths.constBegin(); it != pendingSynths.constEnd();
       ++it)
  ...
  for (const QString &symbol : sampleSymbols) {
    (symbol.contains(QStringLiteral("Phoneme")) ? m_phonemes : m_plainSamples)
        .append(symbol);
  }
...
void VoicegroupBrowser::setSampleInfoProvider(
    std::function<SamplePickInfo(const QString &)> fn) {
...
void VoicegroupBrowser::setEditorRowsVisible(VgMacro macro, bool synth,
                                             bool visible) {
...
void VoicegroupBrowser::populateEditor() {
  ...
  const int shownData =
      synth ? kSynthTypeData : int(typeComboMacro(voice->macro));
  m_typeCombo->clear();
  for (VgMacro macro : kSelectableMacros) {
    m_typeCombo->addItem(vgMacroDisplayName(macro), int(macro));
  ...
  if (vgMacroHasSymbol(voice->macro)) {
    ...
    const bool sampleVoice =
        !synth && !wave && !drumkit && macroIsDsFamily(voice->macro);
  ...
  m_adsrRow->setToolTip(
      cgb ? tr("Attack/decay/release 0-7, sustain 0-15.")
...
void VoicegroupBrowser::commitEdit() {
  ...
  if (v.macro != VgMacro::Keysplit && !macroIsDrumkit(v.macro)) {
    if (crossedFamily) {
      ...
      const VgAdsr adopted =
          remembered.contains(newFamily)
    ...
    v.sweep = (cur->sweep & 0x80) | (m_sweepTimeSpin->value() << 4) |
              (m_sweepDirCombo->currentIndex() << 3) |
              m_sweepShiftSpin->value();
...
void VoicegroupBrowser::updateRow(int slot) {
  ...
  const QString name =
      vgMacroHasSymbol(voice->macro) ? voice->symbol : QString();
  item->setText(0, QStringLiteral("%1  %2")
