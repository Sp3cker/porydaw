...
{
    return m == VgMacro::Keysplit || (vgMacroHasSymbol(m) && !macroIsWave(m) && !macroIsDrumkit(m));
}
...
QString formatDiagnostic(const porydaw::VoicegroupProject::Diagnostic &diagnostic)
{
    QString path = diagnostic.sourcePath.isEmpty() ? diagnostic.assetPath : diagnostic.sourcePath;
    if (diagnostic.range && diagnostic.range->startLine > 0)
        path += (path.isEmpty() ? QString() : QStringLiteral(":")) +
                QString::number(qulonglong(diagnostic.range->startLine));
    if (path.isEmpty())
        return diagnostic.message;
    return QStringLiteral("%1: %2").arg(path, diagnostic.message);
}
...
VoicegroupBrowser::VoicegroupBrowser(QWidget *parent) : QWidget(parent)
{
    ...
    m_vgCombo->setToolTip(tr("The song's voicegroup: the prefix plus this name form the "
    ...
    const auto commitVgArg = [this] {
        if (m_updating || !m_vgCombo->isEnabled())
            return;
        const QString arg =
            SongRegistry::voicegroupArgFromDisplay(m_vgCombo->currentText().trimmed(), m_vgChoices);
        if (arg == m_vgArg)
            return;
        m_vgArg = arg;
    ...
    m_tree->setStyleSheet(QStringLiteral("QTreeWidget::item{padding-left:%1px;}"
    ...
    // Sample-list voices swap the combo for the searchable picker: the
    // project's sample list is huge and prefix-heavy, so it gets search,
    // loop badges, and audition-on-highlight instead of a mile-long menu.
    m_samplePicker = new SamplePickerButton(m_symbolRow);
    ...
    m_newSampleButton->setToolTip(tr("New sample: import a sample and assign it to this voice."));
    ...
    m_sweepTimeSpin->setToolTip(tr("Speed: 128 Hz clocks between pitch steps (1 = fastest, 7 = "
                                   "slowest, Off = no sweep)."));
    m_sweepDirCombo = new QComboBox(m_sweepRow);
    m_sweepDirCombo->addItems({tr("Rise"), tr("Fall")});
    m_sweepDirCombo->setToolTip(tr("Direction. A rising sweep silences the note when it overflows "
                                   "the highest pitch."));
    ...
    m_dutyCombo->addItems({QStringLiteral("12.5%"), QStringLiteral("25%"), QStringLiteral("50%"),
                           QStringLiteral("75%")});
    ...
    for (QSpinBox **spin :
         {&m_synthDutySpin, &m_synthStepSpin, &m_synthDepthSpin, &m_synthPhaseSpin}) {
    ...
    m_synthDutySpin->setToolTip(tr("Base duty cycle: the pulse width the wave centers on "
                                   "(128 = 50% square)."));
    m_synthStepSpin->setToolTip(tr("Duty LFO step per frame: how fast the pulse width wobbles "
                                   "(0 = static)."));
    m_synthDepthSpin->setToolTip(tr("Modulation amount: how far the pulse width swings around the "
                                    "base duty."));
    ...
    m_reloadButton = new QPushButton(tr("Reload Voicegroup"), buttons);
    m_reloadButton->setObjectName(QStringLiteral("vgReloadButton"));
    m_reloadButton->setToolTip(tr("Reload the voicegroup source from disk."));
    ...
    connect(m_reloadButton, &QPushButton::clicked, this,
            &VoicegroupBrowser::reloadVoicegroupRequested);
    connect(m_newButton, &QPushButton::clicked, this, &VoicegroupBrowser::newVoicegroupRequested);
    ...
    // The picker button too — its popup's search box takes focus in its own
    // popup window instead, where plain Space types rather than toggling
    // playback.
    m_samplePicker->setFocusPolicy(Qt::NoFocus);
    for (QWidget *w : std::initializer_list<QWidget *>{
             m_tree, m_vgCombo, m_typeCombo, m_symbolCombo, m_dutyCombo, m_periodCombo,
             m_synthWaveCombo, m_synthDutySpin, m_synthStepSpin, m_synthDepthSpin, m_synthPhaseSpin,
             m_sweepTimeSpin, m_sweepDirCombo, m_sweepShiftSpin, m_attackSpin, m_decaySpin,
             m_sustainSpin, m_releaseSpin}) {
...
void VoicegroupBrowser::setVoicegroup(const LoadedVoiceGroup *vg)
{
    ...
    for (int i = 0; i < VOICEGROUP_SIZE; i++) {
        ...
        QString name = QString::fromUtf8(vg->voiceSampleNames[i]).trimmed();
        const QString type =
...
void VoicegroupBrowser::setSource(VoicegroupSource *source, const QStringList &sampleSymbols,
                                  const QStringList &waveSymbols,
                                  const QList<QPair<QString, QString>> &keysplits,
                                  const QStringList &drumkits, const VgAdsrDefaults &adsrDefaults,
                                  ...
                                  std::function<QString(const VgSynthDesc &)> mintSynth)
{
    if (source != m_source)
        m_adsrHistory.clear();
    m_source = source;
    m_reloadButton->setEnabled(m_vg != nullptr && m_source != nullptr);
...
void VoicegroupBrowser::setDiagnostics(
    const QVector<porydaw::VoicegroupProject::Diagnostic> &diagnostics)
{
...
void VoicegroupBrowser::selectSlot(int slot)
{
    ...
    item->setFlags(flags);
    // QTreeWidget ignores disabled items in its selection path. Explicit
    // navigation still needs to expose their diagnostic in the detail panel.
    populateEditor();
}
...
void VoicegroupBrowser::setEditorRowsVisible(VgMacro macro, bool synth, bool visible)
{
    ...
    showRow(m_dutyLabel, m_dutyCombo, visible && (macroIsSquare1(macro) || macroIsSquare2(macro)));
...
void VoicegroupBrowser::setEditorEnabled(bool enabled)
{
    for (QWidget *field :
         {static_cast<QWidget *>(m_typeCombo), static_cast<QWidget *>(m_symbolCombo),
          static_cast<QWidget *>(m_samplePicker), static_cast<QWidget *>(m_newSampleButton),
          static_cast<QWidget *>(m_editSampleButton), static_cast<QWidget *>(m_sweepTimeSpin),
          static_cast<QWidget *>(m_sweepDirCombo), static_cast<QWidget *>(m_sweepShiftSpin),
          static_cast<QWidget *>(m_dutyCombo), static_cast<QWidget *>(m_periodCombo),
          static_cast<QWidget *>(m_synthWaveCombo), static_cast<QWidget *>(m_synthDutySpin),
          static_cast<QWidget *>(m_synthStepSpin), static_cast<QWidget *>(m_synthDepthSpin),
          static_cast<QWidget *>(m_synthPhaseSpin), static_cast<QWidget *>(m_attackSpin),
          static_cast<QWidget *>(m_decaySpin), static_cast<QWidget *>(m_sustainSpin),
          static_cast<QWidget *>(m_releaseSpin)})
        field->setEnabled(enabled);
}

const porydaw::VoicegroupProject::Diagnostic *VoicegroupBrowser::diagnosticForSlot(int slot) const
{
    if (slot < 0)
        return nullptr;
    for (const auto &diagnostic : m_diagnostics) {
        if (diagnostic.slot && *diagnostic.slot == size_t(slot))
            return &diagnostic;
    }
    return nullptr;
}

bool VoicegroupBrowser::synthDescFor(const VgVoice &voice, int slot, VgSynthDesc *desc) const
{
...
void VoicegroupBrowser::populateEditor()
{
    ...
    const auto *slotDiagnostic = diagnosticForSlot(slot);
    const auto draft =
        (m_source && slot >= 0)
            ? m_source->voiceDraft(
    ...
    if (!draft) {
        ...
        QString notice;
        if (slotDiagnostic)
            notice = formatDiagnostic(*slotDiagnostic);
        else if (!m_vg)
            notice.clear();
        else if (!m_source)
    ...
    if (slotDiagnostic) {
        m_notice->setText(formatDiagnostic(*slotDiagnostic));
    ...
    if (vgMacroHasSymbol(voice->macro)) {
        ...
        if (pickerList) {
            ...
            m_symbolCombo->clear();
            if (synth) {
                // The current symbol may be a zero-size .bin descriptor with
                // no set_synth entry; setCurrentText still shows it (editable
                // combo).
                for (const auto &def : m_synths.defs)
    ...
    setEditorEnabled(slotDiagnostic == nullptr);
    m_updating = false;
}
        ...
        if (symbol.isEmpty()) {
            if (curSynth && desc == curDesc) {
                ...
                if (symbol.isEmpty()) {            // then pending ones
                    for (auto it = m_synthBySymbol.constBegin(); it != m_synthBySymbol.constEnd();
                         ++it) {
                        if (it.value() == desc) {
                            symbol = it.key();
                            break;
...
void VoicegroupBrowser::updateRow(int slot)
{
    ...
    if (!item)
        return;
    const auto kind = m_source ? m_source->kindAt(slot) : VgLineKind::Other;
    const auto *diagnostic = diagnosticForSlot(slot);
    const bool blocked = diagnostic || kind == VgLineKind::Broken;
    if (blocked) {
        item->setFlags(item->flags() & ~Qt::ItemIsEnabled);
        item->setToolTip(
            0, diagnostic ? formatDiagnostic(*diagnostic)
                          : tr("This voice line couldn't be parsed; the last valid audio remains "
                               "loaded."));
    } else {
        item->setFlags(item->flags() | Qt::ItemIsEnabled);
    ...
    const QString marker = tr("[Error] ");
    if (item->text(0).startsWith(marker))
        item->setText(0, item->text(0).mid(marker.size()));
    if (kind == VgLineKind::None) {
        item->setText(
            0, QStringLiteral("%1  %2").arg(slot, 3, 10, QLatin1Char('0')).arg(tr("[Blank]")));
        item->setText(1, QString());
        item->setText(2, QString());
    } else if (const VgVoice *voice = m_source ? m_source->voiceAt(slot) : nullptr) {
        VgSynthDesc desc;
    ...
    if (blocked)
        item->setText(0, marker + item->text(0));
}
