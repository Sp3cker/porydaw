...
// Continuous mode is the fallback when the map has no PSG levels. It maps the
// editable note-velocity domain 1–127 and sets values below or above the domain
// to an endpoint. Intrinsic mode maps one compatible PSG voice's levels at
// equal vertical intervals; raw velocity values do not set distances between levels.
class VelocityAxis {
public:
    enum class Mode {
        Continuous,
    ...
    static constexpr bool nodesFocusable()
    {
        return false;
    }

    static constexpr bool graduationLabelsFocusable()
    {
        return false;
    }
...
private:
    ...
    void buildIntrinsicGraduations(const uint8_t *activeValues, std::size_t activeValueCount);
