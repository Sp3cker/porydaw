...
class PlayheadOverlay::Platform final
{
  public:
    ...
    bool setLayout(const QSize &overlaySize, const QRegion &visibleSurfaceRegion,
                   const QRect &triangleClip, const QRect &playheadGeometry, qreal devicePixelRatio,
  ...
  private:
    ...
    bool paintBody(IDCompositionSurface *surface, bool playing)
    {
        return paintSurface(surface, [&](ID2D1RenderTarget &target) -> HRESULT {
            ...
            const float left = float(playing ? kPlayheadGlowRadius - 1 : kPlayheadGlowRadius) * dpr;
    ...
    bool createCachedSurfaces()
    {
        ...
        m_bodySurfaceWidth = qCeil(qreal(2 * (kPlayheadGlowRadius + kSurfacePadding)) * dpr);
        ...
        m_triangleSurfaceHeight = qCeil(qreal(kPlayheadTriangleHeight + 2 * kSurfacePadding) * dpr);
        m_triangleCenter = qreal(kPlayheadTriangleHalfWidth + kSurfacePadding) * dpr;
    ...
    bool setVisualLayout()
    {
        ...
        for (VisualClip &visual : m_bodyClips) {
            ...
            visual.contentVisual->SetOffsetY(float(m_playheadTop * dpr - visual.rect.y()));
        }
        ...
        if (m_triangleClip.contentVisual) {
            ...
            m_triangleClip.contentVisual->SetOffsetY(float(
                m_playheadTop * dpr - qreal(kSurfacePadding) * dpr - m_triangleClip.rect.y()));
        }
        return true;
    }
...
{
    Q_ASSERT(m_platform);
    if (!m_platform->setLayout(size(), m_visibleSurfaceRegion, m_triangleClip, m_playheadGeometry,
