...
public:
  bool initialize() {
    ...
    result = m_d3dDevice->QueryInterface(
        __uuidof(IDXGIDevice), reinterpret_cast<void **>(m_dxgiDevice.put()));
    if (FAILED(result))
      return fail(result);
...
public:
  ...
  bool setLayout(const QSize &overlaySize,
    ...
    const bool surfacesChanged =
        !m_surfacesValid || m_devicePixelRatio != dpr ||
        m_playheadHeight != playheadGeometry.height();
    const bool clipsChanged = !m_visualTreeValid ||
  ...
  bool setPosition(qreal finalX, bool visible, bool playing,
...
private:
  ...
  bool ensureHostWindow() {
    ...
    m_hostWindow = CreateWindowExW(extendedStyle, kHostWindowClass, L"",
                                   WS_CHILD, 0, 0, 1, 1, ownerWindow, nullptr,
  ...
  bool paintBody(IDCompositionSurface *surface, bool playing) {
    return paintSurface(surface, [&](ID2D1RenderTarget &target) -> HRESULT {
      ...
      const float left =
          float(playing ? kPlayheadGlowRadius - 1 : kPlayheadGlowRadius) * dpr;
      const float right = float(playing ? 0 : kPlayheadGlowRadius) * dpr;
      const qreal peak = playing ? kPlayheadPeakPlaying : kPlayheadPeakPaused;
      ...
      if (left > 0.0f) {
        ...
        result = target.CreateLinearGradientBrush(gradient, gradientStops.get(),
                                                  brush.put());
        if (FAILED(result))
          return result;
        target.FillRectangle(D2D1::RectF(center - left, 0.0f, center, height),
                             brush.get());
      }
      ...
      result =
          target.CreateSolidColorBrush(d2dColor(m_color.alphaF()), core.put());
      if (FAILED(result))
        return result;
  ...
  bool paintTriangle(IDCompositionSurface *surface, bool pointsUp) {
    return paintSurface(surface, [&](ID2D1RenderTarget &target) -> HRESULT {
      ...
      result =
          target.CreateSolidColorBrush(d2dColor(m_color.alphaF()), brush.put());
      if (FAILED(result))
        return result;
  ...
  bool createCachedSurfaces() {
    ...
    m_triangleSurfaceWidth =
        qCeil(qreal(2 * (kPlayheadTriangleHalfWidth + kSurfacePadding)) * dpr);
    ...
    if (!createSurface(UINT(m_bodySurfaceWidth), UINT(m_bodySurfaceHeight),
                       m_bodyPaused) ||
        !createSurface(UINT(m_bodySurfaceWidth), UINT(m_bodySurfaceHeight),
                       m_bodyPlaying) ||
  ...
  bool rebuildVisualTree() {
    ...
    if (!triangleRect.isEmpty() &&
        !createVisualClip(triangleRect, m_triangleClip)) {
      return false;
    }
    return true;
  }
  ...
  bool setVisualContent() {
    ...
    if (m_triangleClip.contentVisual) {
      const HRESULT result = m_triangleClip.contentVisual->SetContent(
          m_visible
              ? (m_trianglePointsUp ? m_triangleUp.get() : m_triangleDown.get())
              : nullptr);
      if (FAILED(result))
        return fail(result);
    }
  ...
  bool setVisualLayout() {
    ...
    if (m_triangleClip.contentVisual) {
      ...
      m_triangleClip.contentVisual->SetOffsetY(
          float(m_playheadTop * dpr - qreal(kSurfacePadding) * dpr -
...
void PlayheadOverlay::setPlatformLayout() {
  Q_ASSERT(m_platform);
  if (!m_platform->setLayout(
          size(), m_visibleSurfaceRegion, m_triangleClip, m_playheadGeometry,
...
bool PlayheadOverlay::setPlatformPosition() {
  Q_ASSERT(m_platform);
  const bool applied = m_platform->setPosition(
      finalX(), m_visible, m_playing, m_trianglePointsUp);
