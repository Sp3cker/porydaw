    ...
    const auto findFreeCell = [&](int firstProbe, bool checkAllTracks,
        ...
        for (int key = 115; key >= 24; --key) {
            ...
            for (int probe = firstProbe;
                 probe < roll->width() - songview::kKeyboardW - 40;
                 probe += 24) {
                const uint64_t tick =
                    view.snapTickDown(view.tickAtContentX(probe));
                const uint64_t dur = view.gridTicksAt(tick);
                const int x0 = songview::kKeyboardW
                    + view.contentX(double(tick));
                const int x1 = songview::kKeyboardW
                    + view.contentX(double(tick + dur));
                ...
                if (x0 < songview::kKeyboardW || x1 - x0 < 12
                    ...
                    || occupied(tick, dur, key, checkAllTracks)) {
                    continue;
                }
...
// pass so the earlier generic gesture checks retain their original setup.
{
    ...
    if (!slotCases.empty()) {
        ...
        for (const SlotCase &slot : slotCases) {
            ...
            doc.addNote(track, cell.tick, uint8_t(cell.key),
                        uint32_t(cell.dur), 64);
            DocNote placed;
    ...
    auto dragVelocity = [&](const NoteCase &note, int startVelocity,
        ...
        const QPoint handle(
            note.cell.center.x(),
            qRound(songview::velBarRect(
                       rowRect, startVelocity, rows.dpr())
                       .center()
                       .y()));
        const QPoint target = handle - QPoint(0, delta);
        sendMouse(roll, QEvent::MouseButtonPress, handle, Qt::LeftButton,
                  Qt::LeftButton);
        sendMouse(roll, QEvent::MouseMove, target, Qt::NoButton,
                  Qt::LeftButton);
    ...
    if (!notes.empty()) {
        const auto initialSquare = std::find_if(
            notes.begin(), notes.end(), [](const NoteCase &note) {
        ...
        if (initialSquare != notes.end()) {
            ...
            if (!initialVelocityArea) {
                ...
                QApplication::sendEvent(initialVelocityArea,
                                        &leaveVelocityArea);
                const songview::VelocityAxis continuousAxis(
                    double(initialVelocityArea->height()), std::nullopt);
                const QPoint initialHandle(
                    qRound(view.displayX(
                        double(initialSquare->cell.tick), songview::kGutterW,
                        initialVelocityArea->devicePixelRatioF())),
                    qRound(continuousAxis.velocityToY(64)));
                sendMouse(initialVelocityArea, QEvent::MouseMove,
                          initialHandle, Qt::NoButton, Qt::NoButton);
                ...
                view.setSelection(
                    {{uint32_t(initialSquare->cell.tick),
                      uint8_t(initialSquare->cell.key)}});
                (void)initialVelocityArea->grab();
                QCoreApplication::processEvents();
                const auto initialMaximumLevel = view.noteVelocityLevel(
                    track, initialSquare->cell.tick,
        ...
        for (const NoteCase &note : notes)
            ids.push_back(
                {uint32_t(note.cell.tick), uint8_t(note.cell.key)});
        ...
        for (const NoteCase &note : notes) {
            DocNote committed;
            if (!doc.findNote(track, note.cell.tick,
                              uint8_t(note.cell.key), &committed)) {
                fail("mixed velocity drag lost a selected note");
                continue;
            }
            if (note.family == DetentFamily::Wave
        ...
        if (wave != notes.end()) {
            ...
            if (!doc.findNote(track, wave->cell.tick,
                              uint8_t(wave->cell.key), &committed)
        ...
        if (!velocityArea || velocityArea->height() <= 0) {
            ...
            auto checkReducedDetents =
                [&](const NoteCase &note,
                    ...
                    const auto info = view.velocityDetentsForNotes(
                        track,
                        {{uint32_t(note.cell.tick),
                          uint8_t(note.cell.key)}});
                    ...
                    for (std::size_t level = 0;
                         level < info->levels.size(); ++level) {
                        const VelocityDetentLevel &detent =
                            info->levels[level];
            ...
            const auto waveAtCursor = std::find_if(
                notes.begin(), notes.end(), [](const NoteCase &note) {
                    return note.family == DetentFamily::Wave;
                });
            if (waveAtCursor != notes.end()) {
                view.setSelection(
                    {{uint32_t(waveAtCursor->cell.tick),
            ...
            if (square != notes.end()) {
                view.setSelection(
                    {{uint32_t(square->cell.tick),
                      uint8_t(square->cell.key)}});
                (void)velocityArea->grab();
                QCoreApplication::processEvents();
                const std::size_t reachableCount =
                    checkReducedDetents(*square, 16);
                if (reachableCount == 0 ||
                    !velocityArea->accessibleDescription().contains(
                        QStringLiteral("%1 levels").arg(reachableCount))) {
            ...
            for (int slot = 1; slot < VOICEGROUP_SIZE; ++slot) {
                ...
                if (cgbType == VOICE_PROGRAMMABLE_WAVE &&
                    tone.wavePointer) {
                    preProgramSlot = slot;
                    break;
                }
                if (cgbType == VOICE_SQUARE_1 ||
                    cgbType == VOICE_SQUARE_2 ||
                    cgbType == VOICE_NOISE) {
                    preProgramSlot = slot;
                    break;
            ...
            if (preProgramSlot >= 0) {
                ...
                auto preProgramTimeline =
                    MidiTimeline::build(preProgramSmf, 48000.0);
                if (!preProgramTimeline) {
                    fail("could not build the pre-program PSG fixture");
                } else {
                    SongView preProgramView;
                    preProgramView.setSong(preProgramTimeline.get(),
                                          nullptr);
                    ...
                    const auto beforeFirstProgram =
                        preProgramView.noteVelocityLevel(
                            0, noteTick, noteKey, 64);
                    const auto atFirstProgram =
                        preProgramView.noteVelocityLevel(
                            0, firstVoiceTick, noteKey, 64);
                    const auto detents =
                        preProgramView.velocityDetentsForNotes(
                            0, {{uint32_t(noteTick), noteKey}});
                    const auto maximumBeforeFirstProgram =
                        preProgramView.noteVelocityLevel(
                            0, noteTick, noteKey, 127);
                    if (preProgramView.programAt(0, noteTick) !=
                        preProgramSlot) {
                        fail("a pre-program PSG note resolved program "
                             "slot zero instead of firstProgram");
                    }
                    if (!beforeFirstProgram ||
                        beforeFirstProgram != atFirstProgram ||
                        !maximumBeforeFirstProgram ||
                        !detents ||
                        detents->levels.size() !=
                ...
                std::printf(
                    "rollcheck: SKIP %s pre-program PSG resolution "
            ...
            for (const SlotCase &slot : slotCases) {
                ...
                const auto anchor = std::find_if(
                    notes.begin(), notes.end(), [&](const NoteCase &note) {
                        return note.family == slot.family;
                    });
                if (anchor == notes.end())
                    continue;
                ...
                doc.addLanePoint(track, DOC_CC_VOICE, partner.tick,
                                 slot.slot);
                ...
                const auto detents =
                    view.velocityDetentsForNotes(track, pair);
                if (!detents ||
                    int(detents->levels.size()) != expectedLevelCount ||
                    !velocityArea->accessibleDescription().contains(
                        QStringLiteral("%1 levels")
                ...
                const auto startLevel = view.noteVelocityLevel(
                    track, anchor->cell.tick, uint8_t(anchor->cell.key),
                    64);
                if (!startLevel || *startLevel >= detents->levels.size()) {
                    fail("PSG velocity handle had no hardware level");
                    continue;
                }
                const uint8_t targetLevel =
                    *startLevel + 1 < detents->levels.size()
                        ? uint8_t(*startLevel + 1)
                        : uint8_t(*startLevel - 1);
                const int anchorX = qRound(view.displayX(
                    double(anchor->cell.tick), songview::kGutterW, dpr));
                const QPoint handle(
                    anchorX,
                ...
                sendMouse(velocityArea, QEvent::MouseMove, target,
                ...
                for (const SongView::NoteId &id : pair) {
                    DocNote committed;
                    const auto expected = view.velocityForLevel(
                        track, id.tick, id.key, targetLevel);
                    const bool found =
                        doc.findNote(track, id.tick, id.key, &committed);
                    const auto committedLevel =
                        found ? view.noteVelocityLevel(
                                    track, id.tick, id.key,
                                    committed.velocity)
                              : std::nullopt;
                    if (!found || !expected ||
                ...
                if (slot.family == DetentFamily::Cgb16) {
                    ...
                    for (const SongView::NoteId &id : pair) {
                        DocNote original;
                        if (!doc.findNote(track, id.tick, id.key,
                                          &original)) {
                    ...
                    if (haveOriginals) {
                        ...
                        for (const SongView::NoteId &id : pair) {
                            ...
                            const auto expected = view.velocityForLevel(
                                track, id.tick, id.key, targetLevel);
                            const bool found = doc.findNote(
                                track, id.tick, id.key, &committed);
                            const auto committedLevel =
                                found ? view.noteVelocityLevel(
                                            track, id.tick, id.key,
                                            committed.velocity)
                        ...
                        for (std::size_t i = 0; i < pair.size(); ++i) {
                            DocNote restored;
                            if (!doc.findNote(track, pair[i].tick,
                                              pair[i].key, &restored) ||
                                restored.velocity !=
                ...
                if (sweepStart.x() < songview::kGutterW) {
                    ...
                    sendMouse(velocityArea, QEvent::MouseButtonPress,
                              sweepStart, Qt::LeftButton, Qt::LeftButton);
                    sendMouse(velocityArea, QEvent::MouseMove, sweepEnd,
                              Qt::NoButton, Qt::LeftButton);
                    sendMouse(velocityArea, QEvent::MouseButtonRelease,
                              sweepEnd, Qt::LeftButton, Qt::NoButton);
                    DocNote committed;
                    const auto expected = view.velocityForLevel(
                        track, anchor->cell.tick,
            ...
            if (pcm != notes.end()) {
                const SongView::NoteId pcmId{
                    uint32_t(pcm->cell.tick), uint8_t(pcm->cell.key)};
                view.setSelection({pcmId});
                (void)velocityArea->grab();
                QCoreApplication::processEvents();
                const int pcmX = qRound(view.displayX(
                    double(pcm->cell.tick), songview::kGutterW, dpr));
                const QPoint handle(
                    pcmX, qRound(songview::velocityToY(
                              64, velocityArea->height())));
                const QPoint target(
                    pcmX, qRound(songview::velocityToY(
                              81, velocityArea->height())));
                const int linearTarget = songview::yToVelocity(
                    target.y(), velocityArea->height());
                ...
                if (doc.undoStack()->index() != pcmIndex + 1 ||
                    !doc.findNote(track, pcm->cell.tick,
                                  uint8_t(pcm->cell.key), &committed) ||
                    committed.velocity != linearTarget ||
                    view.noteVelocityLevel(
                        track, pcm->cell.tick, uint8_t(pcm->cell.key),
                        committed.velocity)) {
    ...
    click(roll, context.restoreLatchCenter); // restore the pre-check velocity latch (78)
}
