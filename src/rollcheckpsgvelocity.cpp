...
int runRollCheckPsgVelocity(const RollCheckPsgVelocityContext &context) {
  ...
  int benchmarkPassed = 0;
  int benchmarkTotal = 0;
  const auto benchmarkCase = [&](const char *name, bool passed) {
    ++benchmarkTotal;
    if (passed)
      ++benchmarkPassed;
    std::printf("PSG_VELOCITY_CASE %s=%d\n", name, passed ? 1 : 0);
  };
        ...
        if (!initialVelocityArea) {
          ...
          if (!initialMaximumLevel ||
              !initialVelocityArea->accessibleDescription().contains(
                  QStringLiteral("%1 volume levels")
      ...
      if (!velocityArea || velocityArea->height() <= 0) {
        ...
        for (const SlotCase &slot : slotCases) {
          ...
          if (!detents || int(detents->levels.size()) != expectedLevelCount ||
              !velocityArea->accessibleDescription().contains(
                  QStringLiteral("%1 volume levels").arg(expectedLevelCount))) {
          ...
          view.clearSelection();
          const QPoint bandStart = handle - QPoint(8, 8);
          const QPoint bandEnd = handle + QPoint(8, 8);
          sendMouse(velocityArea, QEvent::MouseButtonPress, bandStart,
                    Qt::RightButton, Qt::RightButton);
          QContextMenuEvent contextMenuEvent(
              QContextMenuEvent::Mouse, bandStart,
              velocityArea->mapToGlobal(bandStart));
          QCoreApplication::sendEvent(velocityArea, &contextMenuEvent);
          sendMouse(velocityArea, QEvent::MouseMove, bandEnd, Qt::NoButton,
                    Qt::RightButton);
          const auto highlightedNode = std::find_if(
              view.model().notes.begin(), view.model().notes.end(),
              [&](const ViewNote &note) {
                return note.track == track &&
                       note.startTick == pair.front().tick &&
                       note.key == pair.front().key;
              });
          if (!view.selection().empty())
            fail("velocity band preview committed selection before release");
          if (highlightedNode == view.model().notes.end() ||
              !view.isSelectionHighlighted(*highlightedNode)) {
            fail("velocity band entrant was not highlighted before release");
          }
          sendMouse(velocityArea, QEvent::MouseButtonRelease, bandEnd,
                    Qt::RightButton, Qt::NoButton);
          if (std::find(view.selection().begin(), view.selection().end(),
                        pair.front()) == view.selection().end()) {
            fail("native context-menu event cancelled velocity band selection");
          }
          view.setSelection(pair);
          ...
          for (const SongView::NoteId &id : pair) {
            ...
            if (doc.undoStack()->index() != relativeIndex ||
                !doc.findNote(track, id.tick, id.key, &live) ||
                live.velocity != 64) {
              fail("velocity node drag modified the document before release");
            }
          }
          sendMouse(velocityArea, QEvent::MouseButtonRelease, target,
                    Qt::LeftButton, Qt::NoButton);
          ...
          if (doc.undoStack()->index() > relativeIndex)
            doc.undoStack()->undo();
          std::vector<uint8_t> cancelOrigins;
          cancelOrigins.reserve(pair.size());
          for (const SongView::NoteId &id : pair) {
            DocNote original;
            if (doc.findNote(track, id.tick, id.key, &original))
              cancelOrigins.push_back(original.velocity);
          }
          sendMouse(velocityArea, QEvent::MouseButtonPress, handle,
                    Qt::LeftButton, Qt::LeftButton);
          sendMouse(velocityArea, QEvent::MouseMove, target, Qt::NoButton,
                    Qt::LeftButton);
          QKeyEvent cancelEvent(QEvent::KeyPress, Qt::Key_Escape,
                                Qt::NoModifier);
          QCoreApplication::sendEvent(velocityArea, &cancelEvent);
          if (velocityArea->gestureActive() ||
              doc.undoStack()->index() != relativeIndex) {
            fail("cancelled velocity node drag retained its undo command");
          }
          for (std::size_t i = 0; i < cancelOrigins.size(); ++i) {
            DocNote restored;
            if (!doc.findNote(track, pair[i].tick, pair[i].key, &restored) ||
                restored.velocity != cancelOrigins[i]) {
              fail("cancelled velocity node drag did not restore its "
                   "starting velocity");
            }
          }
          view.setSelection(pair);
        ...
        const auto researchSquare =
            std::find_if(notes.begin(), notes.end(), [](const NoteCase &note) {
              return note.family == DetentFamily::Cgb16;
            });
        const auto researchWave =
            std::find_if(notes.begin(), notes.end(), [](const NoteCase &note) {
              return note.family == DetentFamily::Wave;
            });
        const auto researchPcm =
            std::find_if(notes.begin(), notes.end(), [](const NoteCase &note) {
              return note.family == DetentFamily::Pcm;
            });
        const auto labelsFollowSelection = [&](const NoteCase &selected,
                                               const NoteCase &cursor) {
          const SongView::NoteId id{uint32_t(selected.cell.tick),
                                    uint8_t(selected.cell.key)};
          view.setPlayheadSample(0, false);
          view.setEditCursorTick(cursor.cell.tick);
          ...
          return detents &&
                 velocityArea->accessibleDescription().contains(
                     QStringLiteral("%1 volume levels")
                         .arg(int(detents->levels.size())));
        };

        bool squareLabelsFollowSelection = false;
        if (researchSquare != notes.end()) {
          const auto other =
              researchPcm != notes.end() ? researchPcm : researchWave;
          if (other != notes.end())
            squareLabelsFollowSelection =
                labelsFollowSelection(*researchSquare, *other);
        }
        benchmarkCase("square_selection_labels",
                      squareLabelsFollowSelection);

        bool waveLabelsFollowSelection = false;
        if (researchWave != notes.end()) {
          const auto other =
              researchPcm != notes.end() ? researchPcm : researchSquare;
          if (other != notes.end())
            waveLabelsFollowSelection =
                labelsFollowSelection(*researchWave, *other);
        }
        benchmarkCase("wave_selection_labels", waveLabelsFollowSelection);
        ...
        if (researchSquare != notes.end()) {
          ...
          DocNote squareNote;
          const int originSetupIndex = doc.undoStack()->index();
          if (doc.findNote(track, id.tick, id.key, &squareNote)) {
            doc.setNotesVelocity({squareNote}, 95);
            view.setPlayheadSample(0, false);
            view.setEditCursorTick(id.tick);
            view.setSelection({id});
            (void)velocityArea->grab();
            QCoreApplication::processEvents();

            const auto detents = view.velocityAxisDetents(track, {id});
            const auto originLevel =
                view.noteVelocityLevel(track, id.tick, id.key, 95);
            if (detents && originLevel && detents->levels.size() > 1) {
              const songview::VelocityAxis axis(velocityArea->height(), detents);
              const int adjacentLevel =
                  *originLevel + 1 < detents->levels.size()
                      ? int(*originLevel) + 1
                      : int(*originLevel) - 1;
              const int noteX = qRound(view.displayX(
                  double(id.tick), songview::kGutterW,
                  velocityArea->devicePixelRatioF()));
              const QPoint originPoint(noteX,
              ...
              sendMouse(velocityArea, QEvent::MouseButtonPress, originPoint,
                        Qt::LeftButton, Qt::LeftButton);
              sendMouse(velocityArea, QEvent::MouseMove, adjacentPoint,
                        Qt::NoButton, Qt::LeftButton);
              sendMouse(velocityArea, QEvent::MouseMove, originPoint,
                        Qt::NoButton, Qt::LeftButton);
              sendMouse(velocityArea, QEvent::MouseButtonRelease, originPoint,
                        Qt::LeftButton, Qt::NoButton);
              DocNote committed;
              originReturnPreservesStoredValue =
                  doc.findNote(track, id.tick, id.key, &committed) &&
                  committed.velocity == 95;
              while (doc.undoStack()->index() > originSetupIndex + 1)
                doc.undoStack()->undo();
              ...
              sendMouse(velocityArea, QEvent::MouseMove, adjacentPoint,
                        Qt::NoButton, Qt::LeftButton);
              sendMouse(velocityArea, QEvent::MouseButtonRelease, adjacentPoint,
                        Qt::LeftButton, Qt::NoButton);
              DocNote changed;
              const bool changedLevel =
                  doc.findNote(track, id.tick, id.key, &changed) &&
                  changed.velocity != 95;
              if (doc.undoStack()->index() > originSetupIndex + 1)
                doc.undoStack()->undo();
              DocNote restored;
              undoRestoresStoredValue =
                  changedLevel &&
                  doc.findNote(track, id.tick, id.key, &restored) &&
                  restored.velocity == 95;

              QMenu *noteMenu = nullptr;
              for (QMenu *candidate : roll->findChildren<QMenu *>()) {
                const QList<QAction *> actions = candidate->actions();
                if (actions.size() >= 2 && actions[1]->isSeparator()) {
                  noteMenu = candidate;
                  break;
                }
              }
              if (noteMenu && !noteMenu->actions().empty()) {
                QTimer::singleShot(0, [] {
                  auto *dialog =
                ...
                noteMenu->actions().front()->trigger();
                DocNote exact;
                exactInputRestoresStoredValue =
                    doc.findNote(track, id.tick, id.key, &exact) &&
                    exact.velocity == 95;
              }
            }
          }
        ...
        benchmarkCase("typed_velocity_stays_exact",
                      exactInputRestoresStoredValue);
      }
  ...
  std::printf("PSG_VELOCITY_BENCH passed=%d total=%d\n", benchmarkPassed,
              benchmarkTotal);
  return failures;
}
