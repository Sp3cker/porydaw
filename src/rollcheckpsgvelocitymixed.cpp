...
Cell findFreeCell(SongDocument &document, SongView &view, QWidget &roll,
    ...
    for (int key = 115; key >= 24; --key) {
        ...
        for (int probe = 8;
            ...
            const bool occupied =
                std::any_of(notes.begin(), notes.end(),
                            [&](const DocNote &note) {
                                if (int(note.key) != key)
                                    return false;
                                ...
                                return note.tick < tick + 2 * duration &&
                                    end > tick;
                            });
            DocLanePoint point;
            if (occupied ||
...
int runRollCheckPsgVelocityMixed(
    ...
    const auto runFixture = [&] {
        ...
        doc.addNote(track, first.tick, uint8_t(first.key),
                    uint32_t(first.duration), 64);
        ...
        const SongView::NoteId firstId{uint32_t(first.tick),
                                       uint8_t(first.key)};
        const SongView::NoteId secondId{uint32_t(second.tick),
                                        uint8_t(second.key)};
        const auto firstDetents = view.velocityDetentsForNotes(track, {firstId});
        const auto secondDetents =
            view.velocityDetentsForNotes(track, {secondId});
        ...
        if (!firstDetents || !secondDetents ||
            ...
            fail("mixed-context PSG sweep fixture did not produce incompatible detent models");
        } else {
            for (std::size_t level = 0; level < firstDetents->levels.size();
                ...
                const uint8_t continuousVelocity = view.canonicalNoteVelocity(
                    track, secondId.tick, secondId.key,
                    capturedAxis.yToVelocity(y));
                const auto rebuiltFirst = view.velocityForLevel(
                    track, firstId.tick, firstId.key,
                    uint8_t(rebuiltAxis.yToLevel(y)));
                const uint8_t rebuiltSecond = view.canonicalNoteVelocity(
                    track, secondId.tick, secondId.key,
                    rebuiltAxis.yToVelocity(y));
                const bool resizeDiscriminates =
                    firstVelocity && rebuiltFirst &&
                ...
                if (firstVelocity && secondCategorical && *firstVelocity != 64 &&
                    *secondCategorical != 64 &&
                    continuousVelocity != *secondCategorical &&
                    resizeDiscriminates) {
        ...
        const qreal dpr = roll->devicePixelRatioF();
        const int firstX = qRound(view.displayX(double(first.tick),
                                                songview::kGutterW, dpr));
        const int secondX = qRound(view.displayX(double(second.tick),
                                                 songview::kGutterW, dpr));
        ...
        const auto runSweep = [&](Path path) {
            ...
            sendMouse(velocityArea, QEvent::MouseButtonPress, start,
                      Qt::LeftButton, Qt::LeftButton);
            if (velocityArea->accessibleDescription() !=
            ...
            if (path == Path::Resized) {
                ...
                if (velocityArea->height() != resizedHeight)
                    fail("mixed-context PSG resize fixture did not resize the velocity area");
                if (velocityArea->accessibleDescription() !=
                    categoricalDescription) {
                    fail("mixed-context PSG resize changed its latched accessibility axis");
                }
            } else if (path == Path::Segmented) {
                sendMouse(velocityArea, QEvent::MouseMove, midpoint,
            ...
            if (velocityArea->accessibleDescription() !=
                categoricalDescription) {
                fail("mixed-context PSG sweep changed its latched axis after crossing the incompatible note");
            }
            sendMouse(velocityArea, QEvent::MouseButtonRelease, end,
                      Qt::LeftButton, Qt::NoButton);

            DocNote firstCommitted;
            DocNote secondCommitted;
            const bool foundFirst = doc.findNote(
                track, firstId.tick, firstId.key, &firstCommitted);
            const bool foundSecond = doc.findNote(
                track, secondId.tick, secondId.key, &secondCommitted);
            ...
            if (!foundFirst || firstCommitted.velocity != *expectedFirst) {
                fail("mixed-context PSG sweep missed the first note's targeted hardware level");
            }
            if (!foundSecond || secondCommitted.velocity != *expectedSecond) {
                fail("mixed-context PSG sweep did not use continuous mapping for the incompatible note");
            }

            if (foundFirst && foundSecond) {
                const auto committed = std::make_pair(firstCommitted.velocity,
                                                      secondCommitted.velocity);
                if (firstRun && committed != *firstRun) {
                    fail(path == Path::Resized
                             ? "mixed-context PSG sweep depended on a resize during the gesture"
                             : "mixed-context PSG sweep depended on move-event segmentation");
                }
                if (!firstRun)
                    firstRun = committed;
            }
            ...
            if (!doc.findNote(track, firstId.tick, firstId.key,
                              &firstRestored) ||
                !doc.findNote(track, secondId.tick, secondId.key,
                              &secondRestored) ||
