...
int runFontCheck(int expectedBaseFontPx)
{
    ...
    check(baseFontPx.has_value(), "Typography did not capture the base font pixel size");
    if (!baseFontPx)
        return 1;
    ...
    constexpr auto expectedSpaces = std::array{
        ExpectedSpace{layout::Space::Zero, 0.0},   ExpectedSpace{layout::Space::Half, 0.125},
        ExpectedSpace{layout::Space::One, 0.25},   ExpectedSpace{layout::Space::Two, 0.5},
        ExpectedSpace{layout::Space::Three, 0.75}, ExpectedSpace{layout::Space::Four, 1.0},
        ExpectedSpace{layout::Space::Six, 1.5},    ExpectedSpace{layout::Space::Eight, 2.0},
    };
    const auto checkSpaces = [&] {
        for (const auto &expected : expectedSpaces) {
            const auto value =
                expected.multiplier == 0.0 ? 0 : qMax(1, qRound(*baseFontPx * expected.multiplier));
    ...
    checkSpaces();
    check(layout::initialize(*application, *baseFontPx), "Layout initialization is not idempotent");
    check(!layout::initialize(*application, 0), "Layout accepted an invalid base font pixel size");
    ...
    check(body.hintingPreference() == QFont::PreferNoHinting,
          "Body does not prefer unhinted glyph outlines");
    check(body.styleStrategy() & QFont::PreferAntialias,
          "Body does not prefer antialiased glyph outlines");
    ...
    check(mono.styleStrategy() & QFont::PreferAntialias,
          "Body Mono does not prefer antialiased glyph outlines");
    ...
    check(caption.styleStrategy() & QFont::PreferAntialias,
          "Caption does not prefer antialiased glyph outlines");
    ...
    for (auto height = 0; height <= captionHeight + 4; ++height) {
        ...
        check(size <= captionInfo.pixelSize() && metrics.ascent() + metrics.descent() <= height,
    ...
    check(displayedBounds.translated(typography::glyphCenteringOffset(body, selected, text))
    ...
    check(typography::bodyFont().has_value(), "Typography did not capture the installed Body font");
    ...
    check(QFontInfo(QApplication::font()).family() == QStringLiteral("Atkinson Hyperlegible Next"),
    ...
    check(!platformFamily.isEmpty(), "Typography did not capture the platform font family");
    ...
    // The View-menu toggle lands the swap through applyUseSystemFont, without
    // a theme apply — repolishing the stylesheet is unsafe while playback is
    // painting on Windows — by re-installing Body and re-asserting
    // inheritance on polished widgets, whose fonts QStyleSheetStyle
    // otherwise keeps frozen.
