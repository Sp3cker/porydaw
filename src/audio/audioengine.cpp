...
void AudioEngine::process(float *interleavedOut, uint32_t frameCount)
{
    ...
    while (done < frameCount) {
        ...
        const uint32_t rampFrames = std::min(n, m_outputGainRampRemaining);
        for (uint32_t i = 0; i < rampFrames; ++i) {
            m_appliedOutputGain += m_outputGainStep;
            interleavedOut[(done + i) * 2] = (m_bufL[i] + m_pvL[i]) * m_appliedOutputGain;
            interleavedOut[(done + i) * 2 + 1] = (m_bufR[i] + m_pvR[i]) * m_appliedOutputGain;
        }
        m_outputGainRampRemaining -= rampFrames;
        if (m_outputGainRampRemaining == 0)
            m_appliedOutputGain = m_outputGainTarget;
        for (uint32_t i = rampFrames; i < n; ++i) {
            interleavedOut[(done + i) * 2] = (m_bufL[i] + m_pvL[i]) * m_appliedOutputGain;
            interleavedOut[(done + i) * 2 + 1] = (m_bufR[i] + m_pvR[i]) * m_appliedOutputGain;
        }
        done += n;
