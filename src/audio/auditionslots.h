...
class AuditionSlots
{
  public:
    ...
    bool publishNote(const QByteArray &s8, uint32_t freq, uint32_t loopStart, bool looped,
                     uint8_t midiKey, const Adsr &adsr, uint8_t toneKey = 60);
    ...
    bool publishWave(const QByteArray &wave16, uint8_t midiKey, const Adsr &adsr);
