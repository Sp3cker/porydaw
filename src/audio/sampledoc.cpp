...
SampleDocument::SampleDocument(ImportedSample source) : m_source(std::move(source))
{
...
SampleEditParams SampleDocument::defaultParams(const ImportedSample &source)
{
    ...
    p.fineTuneCents = qBound(0.0, source.fracSemitone * 100.0, 99.9999999999);
    ...
    p.targetRate = source.gbaReady ? source.sampleRate : qMin(source.sampleRate, kGbaDefaultRate);
    if (source.gbaReady) {
...
ProcessedSample SampleDocument::render() const
{
    ...
    std::vector<float> buf(m_source.buffer.begin() + cropStart, m_source.buffer.begin() + cropEnd);
    ...
    if (loopOn) {
        ...
        if (loopEndC <= loopStartC) {
            loopOn = false;
            out.warnings += QStringLiteral("loop markers fall outside the crop — loop disabled.");
        }
    }
    ...
    const bool dcOn =
        p.dcRemove == SampleEditParams::On || (p.dcRemove == SampleEditParams::Auto && loopOn);
    if (dcOn) {
        double mean = 0.0;
    ...
    if (identity) {
        ...
        const qint64 lout = qMax<qint64>(1, qint64(std::llround(double(lin) * r0)));
    ...
    if (mode == SampleEditParams::NormalizeAuto)
        mode = loopOn ? SampleEditParams::NormalizeLooped : SampleEditParams::NormalizeOneShot;
    double gain = 1.0;
    ...
    if (p.crossfadeOn && loopOn) {
        ...
        F = std::min(F, qint64(std::llround(std::min(double(loopLen) / 4.0, 0.050 * outputRate))));
        ...
        if (F >= 4) {
            ...
            const double ncc = (aa > 0.0 && bb > 0.0) ? ab / std::sqrt(aa * bb) : 0.0;
            ...
            for (qint64 i = 0; i < F; i++) {
                ...
                const double w =
                    linear ? 1.0 - t : std::cos(kPi * t / 2.0) * std::cos(kPi * t / 2.0);
                const qint64 idx = E - F + 1 + i;
                // One loop length back: idx − L = S − F + i.
                const qint64 src = S - F + i;
                grid[size_t(idx)] =
                    float(w * double(grid[size_t(idx)]) + (1.0 - w) * double(grid[size_t(src)]));
            }
        } else {
            out.warnings +=
                QStringLiteral("loop start too close to the sample start for a crossfade "
                               "bake — skipped.");
        }
    }
    ...
    out.effectiveRate = outputRate * std::pow(2.0, (60.0 - exactKey) / 12.0);
    out.freq = p.exactPitchOverride != 0 ? p.exactPitchOverride
