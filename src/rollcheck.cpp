...
void sendWheel(QWidget *w, QPointF pos, int angleDeltaY, int pixelDeltaY = 0,
  ...
  QWheelEvent ev(pos, QPointF(w->mapToGlobal(pos.toPoint())),
                 QPoint(pixelDeltaX, pixelDeltaY), QPoint(0, angleDeltaY),
...
int runRollCheck(const QString &projectRoot, const QString &songLabel,
  ...
  std::unique_ptr<LoadedVoiceGroup, decltype(&voicegroup_free)> voicegroup(
      voicegroup_load(rootUtf8.constData(), voicegroupName.constData(),
                      nullptr),
      &voicegroup_free);
  if (!voicegroup) {
  ...
  // through the integer-native scrollbar projection.
  {
    ...
    sendWheel(roll, QPointF(songview::kKeyboardW - 1.0, anchor.y()), 0, 1,
              Qt::NoModifier);
    ...
    const qreal boundary =
        std::round((boundaryRow * view.keyHeight() - view.scrollY()) * dpr) /
        dpr;
  ...
  // the integer scrollbar is only a projection of the fractional camera.
  {
    ...
    if (std::abs(view.viewState().scrollPx - (horizontalScroll - 8.0)) >
            1e-12 ||
  ...
  // the widget clears the mark.
  {
    ...
    sendMouse(roll, QEvent::MouseMove, QPoint(4, rows.centerY(expected - 1)),
              Qt::NoButton, Qt::NoButton);
  ...
  // document note to start at that displayed cell rather than a neighbor.
  {
    ...
    for (int key = 115; key >= 24 && probe.key < 0; --key) {
      ...
      for (int guard = 0; guard < 1000; ++guard) {
        ...
        if (leftX >= origin + 4.0 && rightX <= rightLimit &&
            rightX - leftX >= 4.0 && !occupied(tick, dur, key)) {
          ...
          if (view.snapTickDown(view.tickAtContentX(center.x() - origin)) ==
              tick) {
            probe.tick = tick;
    ...
    if (probe.key < 0) {
      ...
      sendMouse(roll, QEvent::MouseButtonDblClick, probe.center, Qt::LeftButton,
                Qt::LeftButton);
      sendMouse(roll, QEvent::MouseButtonRelease, probe.center, Qt::LeftButton,
                Qt::NoButton);
      ...
      if (doc.undoStack()->index() != undoIndex ||
          doc.smf().write() != beforeProbe || view.document() != &doc ||
          !view.timeline())
  ...
  QImage rollBeforeDrawing(roll->size(), QImage::Format_ARGB32_Premultiplied);
  ...
  view.setEditCursorTick(overlayTick);
  QImage rollAfterDrawing(roll->size(), QImage::Format_ARGB32_Premultiplied);
  ...
  for (int y = noteInteractionRect.top(); y <= noteInteractionRect.bottom();
       ++y) {
  ...
  for (int x = noteInteractionRect.left(); x <= noteInteractionRect.right();
       ++x) {
    paintEscapedInteractionRect |=
  ...
  const QRectF twoPixelBarNoteRect(noteFrame.left(), noteFrame.top(),
                                   noteFrame.width(), 20 * rows.pixel());
  ...
  doc.addNote(track, noteA.tick + noteA.duration, noteA.key, noteA.duration,
              100);
  ...
  // bottom edge with the camera centered at a fractional scale.
  {
    ...
    fractionalView.scrollY =
        std::max(0.0, (127.5 - double(noteA.key)) * fractionalView.keyHeight -
                          roll->height() / 2.0);
    ...
    for (int ringPixel = 0; ringPixel < ringPixels; ++ringPixel) {
      if (!isSelectionRingColor(
          ...
          !isSelectionRingColor(selectedNoteImage.pixel(
              centerPixelX, bottomPixel - 1 - ringPixel))) {
    ...
    for (int borderPixel = 0; borderPixel < borderPixels; ++borderPixel) {
      ...
      if (!isBlackBorder(selectedNoteImage.pixel(
              centerPixelX, bottomPixel - 1 - ringPixels - borderPixel)))
    ...
    if (isSelectionRingColor(
            selectedNoteImage.pixel(centerPixelX, topPixel + ringPixels)))
  ...
  QImage ghostNoteRender(roll->size(), QImage::Format_ARGB32_Premultiplied);
  ghostNoteRender.fill(Qt::transparent);
  roll->render(&ghostNoteRender);

  const QRgb ghostTopEdge =
      ghostNoteRender.pixel(paintedNoteBox.center().x(), paintedNoteBox.top());
  const QRgb ghostTopInterior = ghostNoteRender.pixel(
      paintedNoteBox.center().x(), paintedNoteBox.top() + 2);
  const QRgb ghostBottomEdge = ghostNoteRender.pixel(
      paintedNoteBox.center().x(), paintedNoteBox.bottom());
  const QRgb ghostBottomInterior = ghostNoteRender.pixel(
      paintedNoteBox.center().x(), paintedNoteBox.bottom() - 2);
  ...
  if (doc.findNote(track, b.tick, uint8_t(b.key), &bNow)) {
    ...
    sendMouse(roll, QEvent::MouseButtonRelease, movedTop, Qt::LeftButton,
              Qt::NoButton);
    if (doc.findNote(track, b.tick, uint8_t(b.key), &bNow))
  ...
  {
    const SongView::DrawerPage drawerPageBeforeBand = view.drawerPage();
    const bool drawerVisibleBeforeBand = view.drawerVisible();
    ...
    const auto hasVelocitySelectionRing = [&](const Cell &cell) {
      if (!velocityArea)
        return false;
      DocNote note;
      if (!doc.findNote(track, cell.tick, uint8_t(cell.key), &note))
        return false;
      const SongView::NoteId id{uint32_t(cell.tick), uint8_t(cell.key)};
      const auto detents = view.velocityAxisDetents(track, {id});
      const songview::VelocityAxis axis(velocityArea->height(), detents);
      double nodeY = axis.velocityToY(note.velocity);
      if (detents) {
        const auto level =
            view.noteVelocityLevel(track, cell.tick, uint8_t(cell.key),
                                   note.velocity);
        if (!level)
          return false;
        nodeY = axis.levelToY(*level);
      }
      const qreal dpr = velocityArea->devicePixelRatioF();
      const qreal nodeX =
          view.displayX(double(cell.tick), songview::kGutterW, dpr);
      const QPixmap pixmap = velocityArea->grab();
      const QImage image = pixmap.toImage();
      const qreal imageDpr = pixmap.devicePixelRatio();
      for (int dx = -2; dx <= 2; ++dx) {
        for (int dy = -6; dy <= -3; ++dy) {
          const int x = qRound((nodeX + dx) * imageDpr);
          const int y = qRound((nodeY + dy) * imageDpr);
          if (image.rect().contains(x, y) &&
              isSelectionRingColor(image.pixel(x, y))) {
            return true;
          }
        }
      }
    ...
    if (!hasVelocitySelectionRing(a))
      fail("band entrant velocity node was not highlighted before release");
    ...
    if (hasVelocitySelectionRing(a))
      fail("departed band velocity node remained highlighted");
    ...
    if (!hasVelocitySelectionRing(a) || !hasVelocitySelectionRing(b))
      fail("additive band preview did not highlight the velocity union");
    ...
    const auto highlighted = [&](const Cell &cell) {
      const auto note = std::find_if(
          view.model().notes.begin(), view.model().notes.end(),
          [&](const ViewNote &candidate) {
            return candidate.track == track && candidate.startTick == cell.tick &&
                   candidate.key == cell.key;
          });
      return note != view.model().notes.end() &&
             view.isSelectionHighlighted(*note);
    };
    if (doc.engineTrackCount() > 1) {
      std::vector<int> switchOffKeys;
      const auto switchConn = QObject::connect(
          &view, &SongView::auditionNoteTimed, &view,
          [&](int, int key, int velocity, quint32) {
            if (velocity == 0)
              switchOffKeys.push_back(key);
          });
      view.setSelection({bId});
      sendMouse(roll, QEvent::MouseButtonPress, sweepStart, Qt::RightButton,
                Qt::RightButton);
      sendMouse(roll, QEvent::MouseMove, a.center + QPoint(4, 4), Qt::NoButton,
                Qt::RightButton);
      if (!highlighted(a) || view.selection() != std::vector{bId})
        fail("track-switch band fixture did not remain provisional");
      view.selectTrack((track + 1) % doc.engineTrackCount());
      sendMouse(roll, QEvent::MouseButtonRelease, a.center + QPoint(4, 4),
                Qt::RightButton, Qt::NoButton);
      if (!view.selection().empty())
        fail("track switch committed a provisional band to the new track");
      if (std::find(switchOffKeys.begin(), switchOffKeys.end(), a.key) ==
          switchOffKeys.end())
        fail("track switch left a band audition active");
      QObject::disconnect(switchConn);
      view.selectTrack(track);
    }
    std::vector<int> eventListOffKeys;
    const auto eventListConn = QObject::connect(
        &view, &SongView::auditionNoteTimed, &view,
        [&](int, int key, int velocity, quint32) {
          if (velocity == 0)
            eventListOffKeys.push_back(key);
        });
    view.setSelection({bId});
    sendMouse(roll, QEvent::MouseButtonPress, sweepStart, Qt::RightButton,
              Qt::RightButton);
    ...
    sendMouse(roll, QEvent::MouseButtonRelease, a.center + QPoint(4, 4),
              Qt::RightButton, Qt::NoButton);
    if (view.selection() != std::vector{bId} || highlighted(a) ||
        !highlighted(b))
      fail("event-list switch retained or committed a provisional band");
    if (std::find(eventListOffKeys.begin(), eventListOffKeys.end(), a.key) ==
        eventListOffKeys.end())
      fail("event-list switch left a band audition active");
    QObject::disconnect(eventListConn);
    view.setEventListVisible(false);
    view.clearSelection(); // the sections below manage their own
    if (hasSelectionRing(a) || hasSelectionRing(b) ||
        hasVelocitySelectionRing(a) || hasVelocitySelectionRing(b))
      fail("band preview highlight survived release and selection clear");
    view.setDrawerPage(drawerPageBeforeBand);
    view.setDrawerVisible(drawerVisibleBeforeBand);
  }
  ...
  // re-attacking it.
  {
    ...
    sendMouse(roll, QEvent::MouseButtonRelease, gliss, Qt::LeftButton,
              Qt::NoButton);
  ...
  // the hover mark pinned to the note's row. A stationary Ctrl+click keeps
  // its selection-toggle meaning (deferred to release), while one vertical
  // pixel is enough to begin a velocity drag.
  {
    ...
    sendMouse(roll, QEvent::MouseButtonPress, b.center, Qt::LeftButton,
              Qt::LeftButton, Qt::ControlModifier);
    sendMouse(roll, QEvent::MouseMove, b.center + QPoint(0, 1), Qt::NoButton,
              Qt::LeftButton, Qt::ControlModifier);
    sendMouse(roll, QEvent::MouseButtonRelease, b.center + QPoint(0, 1),
              Qt::LeftButton, Qt::NoButton, Qt::ControlModifier);
    if (view.selection().size() != 1 || !(view.selection().front() == bId))
      fail("one-pixel modifier velocity drag did not select its note");
    if (!doc.findNote(track, b.tick, uint8_t(b.key), &bMod) ||
        bMod.velocity != 77)
      fail("one-pixel modifier velocity drag did not change velocity");
    if (doc.undoStack()->count() != preCount + 2)
      fail("one-pixel modifier velocity drag did not push one command");
  }
  ...
  const int resizeHandleY =
      qRound(songview::velBarRect(rows.noteRect(0, 1, d.key), 100, rows.dpr())
                 .center()
                 .y());
  ...
  const QPointF narrowMiddle(
      songview::kKeyboardW + view.contentX(double(d.tick) + double(snapCell)) -
          6,
      rows.bottom(d.key) - 2);
  ...
  // same border its wide neighbors have instead of shedding it.
  {
    ...
    const int narrowRightX =
        songview::kKeyboardW + view.contentX(double(d.tick + snapCell));
    ...
    if (!isNarrowBorder(narrowImage.pixel(qRound(narrowBox.center().x()),
  ...
  const qreal nudgePx =
      view.contentX(double(nStart + nudgeStep)) - view.contentX(double(nStart));
  const qreal displayedEnd = view.displayX(double(nStart + snapCell), 0.0, dpr);
  const int rides =
  ...
  sendKey(roll, Qt::Key_Right, Qt::ControlModifier);
  if (!doc.findNote(track, timeNudgeStart, uint8_t(d.key - 10), &transposed))
  ...
  // Both velocity-axis modes share the font-scaled vertical inset, and every
  // integer MIDI velocity must survive a projection round trip.
  {
    constexpr double projectionHeight = 180.0;
    const double projectionInset = layout::space(layout::Space::Three);
    if (std::abs(songview::velocityToY(127, projectionHeight) -
                 projectionInset) > 1e-12 ||
        std::abs(songview::velocityToY(1, projectionHeight) -
                 (projectionHeight - projectionInset)) > 1e-12 ||
  ...
  if (doc.engineTrackCount() >= 2) {
    ...
    if (hiddenLaneCc >= 0) {
      ...
      movingLaneState.rowStates.insert(movingLaneRowId(0),
    ...
    if (!row0 || !row1) {
      ...
      if (hiddenLaneCc >= 0 && !view.laneHidden(0, uint8_t(hiddenLaneCc)))
        fail("canceling the reorder moved hidden-lane state");
      if (hiddenLaneCc >= 0) {
        const SongView::ViewState canceledMoveState = view.viewState();
        if (canceledMoveState.rowStates.value(movingLaneRowId(0)) !=
            movingLaneDisplayState) {
      ...
      if (!same) {
        ...
        if (hiddenLaneCc >= 0 && movedLaneState.rowStates.value(movingLaneRowId(
                                     1)) != movingLaneDisplayState) {
        ...
        if (hiddenLaneCc >= 0 &&
            undoneMoveState.rowStates.value(movingLaneRowId(0)) !=
                movingLaneDisplayState) {
    ...
    if (hiddenLaneCc >= 0) {
      ...
      if (addedEmptyLane) {
        view.removeEmptyLane(laneTrackAfterCheck, uint8_t(hiddenLaneCc));
      }
    }
  }
  ...
  const int expectedUndos = 20 + (reordered ? (dragRenamed ? 2 : 1) : 0);
  if (undos != expectedUndos) {
    fail(qUtf8Printable(
        QStringLiteral("gesture pass pushed %1 undo commands; expected %2")
            .arg(undos)
            .arg(expectedUndos)));
  }
