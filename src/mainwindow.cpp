...
public:
  VoiceEditCommand(MainWindow *window, SongSession *session,
                   const QString &loadName, int slot, const VgVoice &before,
                   const VgVoice &after, bool structural)
      : QUndoCommand(QObject::tr("edit voice %1").arg(slot)), m_window(window),
        m_session(session), m_loadName(loadName), m_slot(slot),
        m_before(before), m_after(after), m_structural(structural) {}
  ...
  void redo() override {
    m_window->applyVoiceEdit(*m_session, m_loadName, m_slot, m_after,
                             m_structural);
  }
  void undo() override {
    m_window->applyVoiceEdit(*m_session, m_loadName, m_slot, m_before,
                             m_structural);
  }
...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) {
  ...
  statusBar()->showMessage(
      m_audioOk
          ? tr("Audio ready (%1 Hz). Open a decomp project to get started.")
...
void MainWindow::buildUi() {
  ...
  openAction->setText(tr("&Open Project..."));
  connect(openAction, &QAction::triggered, this, &MainWindow::openProject);
  fileMenu->addAction(openAction);
  m_newSongAction = new QAction(this);
  m_newSongAction->setText(tr("&New Song..."));
  connect(m_newSongAction, &QAction::triggered, this, &MainWindow::newSong);
  fileMenu->addAction(m_newSongAction);
  m_newSongAction->setEnabled(false);
  m_importAction =
      fileMenu->addAction(tr("&Import MIDI..."), this, &MainWindow::importMidi);
  m_importAction->setEnabled(false);
  m_saveAction = new QAction(this);
  m_saveAction->setText(tr("&Save Song"));
  connect(m_saveAction, &QAction::triggered, this, &MainWindow::saveSong);
  ...
  m_exportWavAction =
      fileMenu->addAction(tr("Export &WAV..."), this, &MainWindow::exportWav);
  ...
  QAction *undoAction = m_undoGroup->createUndoAction(
      this, live_shortcuts::translatedLabel(live_shortcuts::Command::Undo));
  live_shortcuts::configureAction(*undoAction, live_shortcuts::Command::Undo);
  editMenu->addAction(undoAction);
  QAction *redoAction = m_undoGroup->createRedoAction(
      this, live_shortcuts::translatedLabel(live_shortcuts::Command::Redo));
  live_shortcuts::configureAction(*redoAction, live_shortcuts::Command::Redo);
  ...
  m_followPlaybackAction = new QAction(this);
  live_shortcuts::configureAction(*m_followPlaybackAction,
                                  live_shortcuts::Command::FollowPlayback);
  viewMenu->addAction(m_followPlaybackAction);
  m_followPlaybackAction->setCheckable(true);
  m_followPlaybackAction->setChecked(true);
  m_followPlaybackAction->setEnabled(false);
  connect(m_followPlaybackAction, &QAction::toggled, this, [this](bool on) {
    if (m_active)
      m_active->view->setFollowPlayback(on);
  });
  ...
  m_goToStartAction = new QAction(
      style()->standardIcon(QStyle::SP_MediaSkipBackward),
      live_shortcuts::translatedLabel(live_shortcuts::Command::GoToStart),
      this);
  live_shortcuts::configureAction(*m_goToStartAction,
                                  live_shortcuts::Command::GoToStart);
  ...
  m_playAction = new QAction(style()->standardIcon(QStyle::SP_MediaPlay),
                             live_shortcuts::translatedLabel(
                                 live_shortcuts::Command::ContinuePlayback),
                             this);
  live_shortcuts::configureAction(*m_playAction,
                                  live_shortcuts::Command::ContinuePlayback);
  ...
  // toolbar action is the resume-from-pause path.
  m_playPauseAction = new QAction(this);
  live_shortcuts::configureAction(*m_playPauseAction,
                                  live_shortcuts::Command::PlayPause);
  ...
  m_loopAction = new QAction(
      style()->standardIcon(QStyle::SP_BrowserReload),
      live_shortcuts::translatedLabel(live_shortcuts::Command::Loop), this);
  live_shortcuts::configureAction(*m_loopAction, live_shortcuts::Command::Loop);
  ...
  QAction *browserAction = dock->toggleViewAction();
  browserAction->setText(tr("Song Browser"));
  viewMenu->addAction(browserAction);

  auto *findAction = new QAction(this);
  findAction->setText(tr("Find Song"));
  ...
  connect(m_vgBrowser, &VoicegroupBrowser::auditionVoice, this,
          [this](int voice, int key, int velocity) {
            if (m_audioOk)
              m_audio.previewVoice(uint8_t(voice), uint8_t(key),
                                   uint8_t(velocity));
          });
...
SongSession *MainWindow::createSession() {
  ...
  connect(s->view, &SongView::auditionNote, this,
          [this, s](int track, int key, int velocity) {
            if (s == m_active && m_audioOk && m_audio.songLoaded())
              m_audio.previewNote(uint8_t(track), uint8_t(key),
                                  uint8_t(velocity));
          });
  ...
  connect(s->view, &SongView::statusMessage, this, [this](const QString &text) {
  ...
  connect(s->view, &SongView::revealVoiceRequested, this,
          [this, s](int program) {
            if (s != m_active)
              return;
            m_vgDock->show();
  ...
  connect(s->view, &SongView::eventListVisibilityChanged, this,
          [this, s](bool on) {
            if (s == m_active) {
...
void MainWindow::activateSession(SongSession *session, bool force) {
  ...
  m_followPlaybackAction->setEnabled(loaded);
  {
    QSignalBlocker blocker(m_followPlaybackAction);
    m_followPlaybackAction->setChecked(session &&
                                       session->view->followPlayback());
  }
...
void MainWindow::maybeRefreshVoicegroup(SongSession &session) {
  ...
  if (!session.vgSource || session.vgSource->dirty())
    return;
  const QDateTime onDisk =
      QFileInfo(session.vgSource->filePath()).lastModified();
...
void MainWindow::refreshSessionsAfterVgSave(const QString &filePath,
                                            SongSession *except) {
...
void MainWindow::tabChanged(int index) {
  if (m_tearingDown || m_restoringSession)
    return;
  activateSession(index >= 0 ? sessionForWidget(m_tabs->widget(index))
                             : nullptr);
}
...
void MainWindow::openProject() {
  ...
  const QString startDir =
      settings.value(QStringLiteral("lastProjectDir"), QDir::homePath())
          .toString();
  const QString dir = QFileDialog::getExistingDirectory(
      this, tr("Open Decomp Project"), startDir);
  if (dir.isEmpty())
    return;
  openProjectDir(dir);
}
...
void MainWindow::restoreSession() {
  QSettings settings;
  const QString dir =
      settings.value(QStringLiteral("lastProjectDir")).toString();
...
bool MainWindow::openProjectDir(const QString &dir, bool interactive) {
  ...
  statusBar()->showMessage(
      tr("Opened %1 — %2 songs, %3 with MIDI sources (%4 ms)")
...
void MainWindow::populateSongList() { m_songList->setSongs(m_project.songs()); }
...
LoadedVoiceGroup *MainWindow::loadVoicegroupFor(const SongCfg &cfg,
                                                QString *tried) {
  ...
  for (const QString &name : candidates) {
    LoadedVoiceGroup *vg = voicegroup_load(
        rootUtf8.constData(), name.toLocal8Bit().constData(), nullptr);
    if (vg)
      return vg;
  }
  return nullptr;
...
void MainWindow::onDocumentChanged(SongSession &session) {
  ...
  if (cfg.voicegroupArg != session.appliedVoicegroupArg) {
    ...
    if (LoadedVoiceGroup *vg = loadVoicegroupFor(cfg, &tried)) {
      ...
      statusBar()->showMessage(tr("Voicegroup not found (tried: %1) — keeping "
...
bool MainWindow::saveSession(SongSession &session) {
  ...
  if (vgWasDirty) {
    ...
    if (!newDefs.isEmpty()) {
      if (!VoicegroupSource::writeSynthDefinitions(m_project.root(), newDefs,
                                                   &error)) {
...
void MainWindow::exportWav() {
  ...
  auto updateDuration = [&] {
    ...
    const double scale =
        double(opts.sampleRate) / m_audio.timeline()->sampleRate;
    MidiTimeline scaled;
    scaled.lengthSamples =
        uint64_t(double(m_audio.timeline()->lengthSamples) * scale);
    if (hasLoop) {
    ...
    durationLabel->setText(QStringLiteral("%1:%2")
                               .arg(seconds / 60)
  ...
  if (path.isEmpty())
    return;
  appSettings.setValue(QStringLiteral("lastWavExportDir"),
                       QFileInfo(path).path());
...
void MainWindow::importMidi() {
  ...
  QSettings settings;
  const QString startDir =
      settings.value(QStringLiteral("lastImportDir"), QDir::homePath())
          .toString();
...
void MainWindow::finishCreateSong(const SmfFile &smf, const QString &label,
  ...
  const QString midiDir =
      m_project.root() + QStringLiteral("/sound/songs/midi");
  ...
  int songId = -1;
  if (!SongRegistry::registerSong(m_project.root(), label, constant, player,
                                  &error, &songId)) {
    // Keep the chosen constant/player so Register Song can retry later;
    // the song shows a badge in the browser until then.
    SongRegistry::saveRegistrationMeta(m_project.root(), label, constant,
                                       player);
...
void MainWindow::registerLoadedSong() {
  SongSession *session = m_active;
  if (!session || session->songId < 0 ||
      session->songId >= m_project.songs().size())
    return;
  ...
  QString error;
  int songId = -1;
  if (!SongRegistry::registerSong(m_project.root(), song.label, constant,
                                  player, &error, &songId)) {
...
void MainWindow::updateVoicegroupBrowser() {
  ...
  m_vgBrowser->setSource(
      ...
      m_pendingSynths, [this](const VgSynthDesc &desc) -> QString {
        ...
        for (int i = 2;
             synths.find(symbol) || vgCatalog().directSound.contains(symbol);
             i++)
...
void MainWindow::openVoicegroupSource(SongSession &session,
                                      const SongCfg &cfg) {
...
void MainWindow::onVoiceEditRequested(int slot, const VgVoice &voice,
                                      bool structural) {
  SongSession *session = m_active;
...
void MainWindow::applyVoiceEdit(SongSession &session, const QString &loadName,
                                int slot, const VgVoice &voice,
                                bool structural) {
...
void MainWindow::onVoiceEdited(SongSession &session, int slot,
                               bool structural) {
  if (!session.vgSource)
    return;
  if (structural) {
...
void MainWindow::reloadVoicegroupPreview(SongSession &session, int keepSlot) {
  ...
  QDir().mkpath(previewDir);
  {
    ...
    if (!out.open(QIODevice::WriteOnly)) {
      statusBar()->showMessage(tr("Cannot write voicegroup preview file."),
                               8000);
      return;
    }
  ...
  std::strncpy(config.voicegroupPaths[0], ".porydaw/vgpreview",
               VG_MAX_PATH_LEN - 1);
  config.voicegroupPathCount = 1;
  LoadedVoiceGroup *vg = voicegroup_load(
      m_project.root().toLocal8Bit().constData(),
      session.vgSource->loadName().toLocal8Bit().constData(), &config);
  if (!vg) {
    statusBar()->showMessage(
        tr("Edited voicegroup failed to load — keeping the previous sound."),
        8000);
    return;
  }
  swapVoicegroup(session, vg, keepSlot);
...
bool MainWindow::applyPendingSynthTones(SongSession &session,
                                        LoadedVoiceGroup *vg) {
  ...
  for (int slot = 0; slot < VOICEGROUP_SIZE; slot++) {
    const VgVoice *v = session.vgSource->voiceAt(slot);
    if (!v || (v->macro != VgMacro::DirectSound &&
...
void MainWindow::swapVoicegroup(SongSession &session, LoadedVoiceGroup *vg,
                                int keepSlot) {
...
void MainWindow::cleanupVgPreview() {
  if (!m_project.isOpen())
    return;
  QDir(m_project.root() + QStringLiteral("/.porydaw/vgpreview"))
      .removeRecursively();
}

void MainWindow::updateVgDockTitle() {
  const bool dirty =
      m_active && m_active->vgSource && m_active->vgSource->dirty();
...
void MainWindow::newVoicegroup() {
  ...
  if (activeSource)
    sourceCombo->addItem(
        tr("Copy of %1").arg(QFileInfo(activeSource->filePath()).fileName()),
  ...
  const QString sectionLabel = (!copyFrom.isEmpty() && activeSource &&
                                copyFrom == activeSource->filePath())
...
bool MainWindow::maybeSaveSession(SongSession &session) {
  ...
  const auto choice = QMessageBox::question(
      ...
      vgDirty
          ? tr("%1 has unsaved changes (including voicegroup edits). Save "
               "them?")
...
void MainWindow::updateWindowTitle() {
  ...
  if (m_active) {
    setWindowTitle(QStringLiteral("%1[*] — %2 — porydaw")
                       .arg(m_active->doc.label(), project));
...
void MainWindow::closeEvent(QCloseEvent *event) {
  ...
  if (m_persistSession) {
    ...
    settings.setValue(QStringLiteral("songFilterText"),
                      m_songList->searchText());
    settings.setValue(QStringLiteral("songFilterSort"),
                      m_songList->sortIndex());
...
void MainWindow::uiTick() {
  ...
  if (m_active && m_audio.songLoaded()) {
    ...
    m_timeLabel->setText(QStringLiteral("%1 / %2").arg(
        formatTime(m_audio.playheadSamples()), formatTime(length)));
    m_active->view->setPlayheadSample(
        m_audio.playheadSamples(), m_audio.transport() == Transport::Playing);
...
bool MainWindow::runSelfTest(const QString &projectRoot,
  ...
  qInfo("selftest: project opened, %lld songs",
        (long long)m_project.songs().size());
  ...
  qInfo("selftest: loaded %s (%zu events, %d tracks)",
        qUtf8Printable(target->label), m_audio.timeline()->events.size(),
        m_audio.timeline()->usedTrackCount);
    ...
    for (int i = 0; i < VOICEGROUP_SIZE; i++) {
      ...
      if (dsSlot < 0)
        dsSlot = i;
               ...
               v->symbol != tab->vgSource->voiceAt(dsSlot)->symbol)
        donorSlot = i;
    }
    if (dsSlot >= 0) {
  ...
  const double playedSeconds =
      double(m_audio.playheadSamples()) / m_audio.sampleRate();
  ...
  if (ok) {
    ...
    const uint64_t seekTick = std::min<uint64_t>(
        tl->lengthTicks / 2, uint64_t(tl->ticksPerBeat) * 16);
    ...
    if (ok)
      ...
      qWarning(
          "selftest: edit-cursor seek FAILED (cursor %.2fs, playhead %.2fs "
    ...
    if (ok) {
      ...
      ok = m_audio.transport() == Transport::Playing &&
           afterSpace >= seekSample && afterSpace < pausedAt;
      if (ok)
  ...
  if (ok) {
    ...
    if (ok) {
      ...
      ok = std::abs(restored.pxPerBeat - saved.pxPerBeat) < 0.001 &&
           ...
           restored.gridMinDenom == 8 && restored.gridTriplet &&
