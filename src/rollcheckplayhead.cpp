                                       ...
                                       PaintRegionProbe &paintProbe, QStringList &failures)
{
    ...
    const int cacheKeyboardPixelWidth =
        std::min(qCeil(pianoRoll.width() * dpr), qCeil(layout::editorGeometry().pianoKeyboardWidth * dpr));
    ...
    if (!firstRepainted || firstRepaintWidth > layout::editorGeometry().pianoKeyboardWidth) {
        failures.append(QStringLiteral("piano-roll hover repainted %1 px (budget %2)")
                            .arg(firstRepaintWidth)
                            .arg(layout::editorGeometry().pianoKeyboardWidth));
    }
    ...
    const int keyboardPixelWidth = std::min(
        afterFirstHover.width(), qCeil(layout::editorGeometry().pianoKeyboardWidth * afterFirstHover.devicePixelRatio()));
    ...
    if (!moveRepainted || moveRepaintWidth > layout::editorGeometry().pianoKeyboardWidth) {
    ...
    if (!clearRepainted || clearRepaintWidth > layout::editorGeometry().pianoKeyboardWidth) {
                                     ...
                                     QStringList &failures)
{
    ...
    const int plotLeft = layout::editorGeometry().plotOrigin;
                             ...
                             const QColor &playheadColor, QStringList &failures)
{
    ...
    if (playheadXInView < eventListArea.left() || playheadXInView > eventListArea.right()) {
    ...
    const auto hasLine = [&](const QRect &area) {
        return hasPlayheadRedLine(overlayImage, overlayDpr, stoppedMarkerCenter, area,
                                  playheadColor);
    };
    const auto redWidth = [&](int y) {
        return playheadRedWidth(overlayImage, overlayDpr, stoppedMarkerCenter, y, playheadColor);
    };
    ...
    const QRect eventListBodyArea(eventListArea.left(), eventListArea.top() + triangleHeight,
                                  eventListArea.width(), eventListArea.height() - triangleHeight);
...
{
    const int plotWidth = view.width() - layout::editorGeometry().plotOrigin;
    if (plotWidth <= 64) {
    ...
    songview::TimelineSurface *lanes =
        dynamic_cast<songview::TimelineSurface *>(&bands[2].widget);
    if (!pianoRoll || !lanes) {
    ...
    const QRect rulerArea(bands.front().widget.mapTo(&view, QPoint()),
                          bands.front().widget.size());
    ...
    checkEventListRendering(view, marker, stoppedMarkerCenter, rulerArea, playheadColor, failures);
